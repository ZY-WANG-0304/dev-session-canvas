...
发布到市场的 `template.json` 必须保留本地画布模板中的用户分组结构：`template.groups` 描述分组框，节点通过 `groupIndex` 关联分组，嵌套分组通过 `parentGroupIndex` 关联父分组。市场发布、包上传、安装和自动缩略图生成都应保留这些字段，并拒绝越界、自引用或循环父子关系。`file` / `file-list` 节点和多根 workspace root 分组不属于市场模板主体，不随模板分发。
...
- ✅ 下载安装到本地用户模板目录
- ✅ 匿名浏览和下载
- ✅ 官方精选模板可通过正常发布或受控运维流程写入 D1/R2；代码内 seed / preview 模板只作为开发、测试和调试 fixture，不作为生产预置内容
...
- ❌ 生产环境通过代码内 seed / preview fixture 预置官方模板；生产无 D1 数据时市场应保持空目录，正式模板必须通过发布流程或受控运维流程入库
...
### Phase 4：版本管理与治理
当前状态：Phase 4 已启动，第一切片完成本地实现的是治理后端与浏览器治理入口：登录用户举报、管理员举报队列、举报处理、模板下架/恢复、用户封禁/解封、封禁用户写接口拒绝和管理员审计日志均已有 Worker API 与本地自动化测试；Web 详情页举报表单与 `/templates/admin` 最小管理后台已接入。管理员全站数据统计已补齐 Worker API 与 Web 面板本地实现，统计读取累计计数、按天聚合、举报状态、用户封禁数、发布版本数和管理员审计数。VSCode 安装侧更新提醒、手动更新、历史版本回滚、插件内举报入口和真实 preview OAuth smoke 仍待完成。
...
- [ ] 管理后台可查看数据统计（Worker API 与 Web 面板已本地验证，preview OAuth smoke 待补）
...
### 8.1 已确认
- ✅ 市场形态：社区驱动公开市场 + 官方精选；官方精选模板也必须通过发布流程或受控运维流程写入 D1/R2，不通过代码内 seed fallback 作为生产内容
...
- `GET /api/v1/admin/stats` — 全站统计面板（管理员）
