...
5. 点击卡片进入模板详情页，查看 README、完整描述、版本历史、发布者信息；插件内市场列表行可以预选安装位置，并提供“查看并安装 / 查看更新 / 已安装 / 下载 JSON”等快捷动作，这些动作会自动打开 VSCode Webview 详情页并执行对应动作
6. 在详情页点击"安装/更新"将目标版本下载到本地用户模板目录；安装按钮使用 split button，主按钮安装/更新当前详情页选中的版本，右侧下拉可选择安装某个历史版本
7. 如只想下载 JSON 文件，在详情页点击"下载" split button 主按钮下载当前详情页选中的版本，右侧下拉可下载特定历史版本
8. 安装完成后，模板出现在侧边栏模板列表中，第二行以 `市场下载 · 本地` 或 `市场下载 · 工作区` 标记来源与保存位置
9. 浏览和下载无需登录
...
- [x] 可下载安装模板到本地（无需登录）
- [x] 插件内列表行允许安装位置预选和“查看并安装 / 查看更新 / 已安装 / 下载 JSON”等快捷动作；这些动作会打开 VSCode 内详情页并执行对应安装确认或下载流程
- [x] 插件内详情页安装按钮采用 split button：主按钮安装/更新当前详情页选中的版本，下拉可安装特定版本
- [x] 插件内详情页下载按钮采用 split button：主按钮下载当前详情页选中的版本，下拉可下载特定版本
- [x] 安装后模板出现在侧边栏，标记为 `市场下载 · 本地` 或 `市场下载 · 工作区`
...
- [x] 插件内安装 / 下载版本菜单在点击外部、搜索 / 排序变化、切换列表 / 详情或按 Escape 时关闭
