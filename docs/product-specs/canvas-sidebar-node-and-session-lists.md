...
- 从 workspace 移除 folder 使用 VS Code 原生 modal，可选择 `保留画板并移除` 或 `清空画板并移除`，默认保留画板；它只调用 VS Code workspace folder 移除语义，不删除目录或 git worktree。移除 worktree 会先确认该 folder 是 linked git worktree，再用 VS Code 原生 modal 提供 `移除 Worktree 并清空画板` 与 `移除 Worktree 但保留画板`，默认清空画板；确认后执行 `git worktree remove` 并从 workspace 移除对应 folder。
