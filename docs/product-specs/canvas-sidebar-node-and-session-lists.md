...
- **交互行为**：
  ...
  - workspace folder 分组行可以通过前置图标区分普通 folder、repo 与 worktree，并可以基于该 folder 创建 worktree 并加入 workspace、移除 worktree 并从 workspace 移除对应 folder，或从 workspace 移除该 folder；移除 folder 前用户可以选择保留画板并移除，移除 worktree 前用户可以选择清空或保留画板
- **同步机制**：
  - 监听画布节点变化事件
  - 实时更新列表内容
...
- 多根 workspace 的每个 workspace folder 分组行最前面显示 folder 类型图标，分别区分普通 folder、git repository 和 git worktree；行尾按新建 worktree、移除 worktree、移除 folder 的顺序显示 icon-only 操作；新建 worktree 使用专用 `worktree` Codicon；点击 workspace folder 行的新建 worktree 直接基于该 folder 创建 worktree 并加入 workspace；点击移除 folder 后，确认选择框提供保留画板和清空画板两条路径；点击移除 worktree 后，宿主先确认该 folder 是 linked git worktree，再由确认选择框提供“移除 Worktree 并清空画板”和“移除 Worktree 但保留画板”两条路径，并展示清空路径影响数量。
