...
5. 用户重新打开 VSCode 后：
   ...
   - 若 Supervisor 正在处理旧 registry / Journal metadata，系统通过不可取消的 VS Code 旋转进度通知明确显示“恢复中”、已完成会话数和剩余会话数；这不阻止用户创建新的 `Agent` 或 `Terminal`，也不在启动期回放完整 Journal，画板左上角不重复显示全局恢复提示。
