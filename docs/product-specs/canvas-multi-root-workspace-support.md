...
2026-07-02 补充：paneGallery root 标签常态取消 running / attention 静态状态染色后，attention 只保留标题栏闪烁动画；running 聚合提示迁移为 root 标签标题区域 Codex Working 风格宽 `/` 形纯色斜线扫描。扫描线只在 root 内存在精确 `running` 执行节点且无 attention 时显示，颜色使用 root 标签 border color 派生色，移动段使用 16px 宽度与 0.82 透明度，速度比上一版更慢，且两次扫描之间不保留空档，不使用渐变、外发光或强 halo，扫描范围裁切在 root 标题栏区域内，`launching` / `live` / `starting` / `resuming` / `reattaching` 不触发，`rootGroups` 模式不渲染对应 paneGallery 属性，reduced motion 下关闭扫描线并保留原始底部分隔线。
