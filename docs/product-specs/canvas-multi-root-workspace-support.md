...
- 在 `rootGroups` 或 `paneGallery` 中从某个 root 或用户分组内右键触发清空画板时，主菜单默认清空当前 root 子画布或当前分组内容；通过 `>` 进入二级菜单后，可以明确选择清空当前范围或清空整个 workspace，且所有清空动作都必须先确认。
...
- `paneGallery` 的 root title 在 `dynamic`、`grid`、主画板、顶部/右侧缩略图和 active-root 占位中都保持左上角轻量标签并按内容收缩，不能拉伸为占满 pane 的整栏标题。在 `topThumbnails` / `sideThumbnails` 布局中，一个 active root 以主画板展示；主画板右下角显示与单一画板一致的 MiniMap；rail 按 VSCode workspace folder 顺序保留所有 root 槽位，active root 在原槽位显示等尺寸轻微斜纹占位卡片，文案为“正在主画板”，不显示 status dot，但占位卡片左上角 root 标签继续显示 attention 闪烁与 running 竖向色块动画，状态摘要仍通过 `title` / `aria-label` 暴露；其他 root 以不可交互的画板缩略图保留在顶部或右侧 rail，缩略图复用左上角轻量 root 标签而不使用外部卡片标题；`sideThumbnails` 因 Webview 变窄而把 rail 退化到底部时，每个横向缩略图和 active-root 占位都在自身内部顶部保留 `24px` 标题可见区域和横向分隔线，但 root title 仍保持左上角轻量标签，真实缩略画板内容从标题区域下方开始且不能覆盖标题；rail 未溢出时保持居中，溢出时从第一张 root 槽位开始滚动且滚动终点可访问最后一张 root 槽位；单击不切换 active root，双击非 active root 缩略图才切换 active root。`dynamic` / `grid` 的多 root 子画板不显示 MiniMap。
...
- 在 multi-root workspace 中执行全局“清空画板”后，所有 root-local 节点、用户分组、root 内连线和文件活动记录被清空，当前 workspace 的系统 workspace-root section 仍按 overlay 位置可见；其他 root 内执行会话被终止，但 workspace folder、磁盘目录和 worktree 不被删除。
- 在 multi-root workspace 的某个 workspace-root section、`paneGallery` 可交互 root pane 或 thumbnail 模式 active root 主画板内右键清空画板时，默认只清空当前 root 的 root-local 子画布并保留该 root section；在普通用户分组内右键清空画板时，默认只清空该分组内容并保留当前分组框；二级菜单仍可选择清空整个 workspace。上述清空动作都必须先显示 VS Code 原生 modal 确认。
...
2026-07-17 补充：已修复 `sideThumbnails` 窄宽度底部 rail 的 root 标题区域不可辨认问题。底部横向缩略图和 active-root 占位保留 `24px` 标题可见区域和独立横向分隔线，真实缩略画板内容下移到标题区域之后；root title 本身继续遵循整个 `paneGallery` 的通用规范，保持左上角轻量标签并按内容收缩。新增从 `1000px` 缩窄到 `800px` 的 Webview Playwright 回归，验证宽窄两态标题均位于左上角且不占满 pane、底部 rail 位置、分隔线和缩略内容避让。
