...
2026-07-03 补充：paneGallery root 标签常态取消 running / attention 静态状态染色后，attention 只保留标题栏闪烁动画；running 聚合提示迁移为 root 标签 title 区域 Codex Working 风格 `|` 形竖向色块。色块只在 root 内存在精确 `running` 执行节点且无 attention 时显示在 root title 文字下方，颜色使用 root 标签 border color 派生色，宽度为 5px、透明度为 0.82，在 root title 区域内左右往返运动，不使用渐变、外发光或强 halo，运动范围裁切在 root title 区域内，`launching` / `live` / `starting` / `resuming` / `reattaching` 不触发，`rootGroups` 模式不渲染对应 paneGallery 属性，reduced motion 下关闭色块动画层并保留原始底部分隔线。
