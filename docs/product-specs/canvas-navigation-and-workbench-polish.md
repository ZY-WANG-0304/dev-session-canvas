...
8. 当节点分散到很远时，用户点击 fit view 仍能看到全部节点；如果概览配置为默认 `title`，低于可配置概览触发倍率时每个节点内容区域会显示节点标题；如果配置为 `none`，画布不会切换到概览显示。
...
- 全局动态概览缩放与概览模式配置：
  ...
  - `title` 表示低倍率进入概览后，在节点内容区域显示节点标题，帮助用户确认节点身份。
...
- 当概览模式为默认 `title`，且缩放低于概览触发倍率时，节点内容区域显示节点标题，用户能在低倍率下辨认节点。
