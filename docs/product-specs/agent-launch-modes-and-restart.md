...
6. 若通过命令面板或侧栏“创建节点”入口创建 Agent，先选对象/provider，再进入带输入框的第二步 Quick Input；输入框展示完整命令，标题栏 `默认 / Resume / YOLO / 沙盒` 快捷按钮只负责替换输入框，不直接创建。
...
- VSCode Quick Input 创建链路：
  ...
  - `默认 / Resume / YOLO / 沙盒` 快捷按钮只替换输入框内容，不直接创建。
- 默认启动参数设置：
...
- 命令面板 / 侧栏“创建节点”里的 Agent 入口会进入第二步 Quick Input；第二步输入框展示完整命令，点击标题栏 `默认 / Resume / YOLO / 沙盒` 快捷按钮只会替换输入框内容，不会直接创建。
