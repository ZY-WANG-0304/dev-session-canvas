...
当前状态：已确认。本文收口 Agent 节点在创建前选择 provider、启动方式、默认启动参数、停止后“恢复原会话 / 新会话”分流，以及从当前 Claude Code 会话 Fork 出新 Agent 节点的正式产品语义。`docs/product-specs/canvas-navigation-and-workbench-polish.md` 继续负责空白区右键入口与节点落点规则，但 Agent 相关的多级创建、重启与 Fork 交互以本文为准。
...
- 当用户想像网页对话里的 Fork 一样，从当前 Claude Code 会话上下文分出一个新窗口继续探索时，只能先去侧栏历史里手动恢复；这既不是当前节点的一键动作，也不会使用 Claude Code 原生 `--fork-session` 创建新的 session id。

## 2. 目标用户

目标用户是已经把 Dev Session Canvas 当作 VSCode 内多会话工作台使用的开发者。他们通常已经在本机安装并使用 `Codex` 或 `Claude Code`，希望在不离开画布的前提下，用更低摩擦的方式创建带启动偏好的 Agent，在会话结束后明确选择“继续原上下文”还是“开一条新会话”，并能从当前 Claude Code Agent 直接 Fork 出一个新节点继续探索不同方向。
...
9. 对已经持有可信 Claude Code session id 的 Agent 节点，用户可以在节点标题栏点击 `Fork`；扩展会创建一个新的 Claude Code Agent 节点并立即用 `claude --resume <session-id> --fork-session` 启动。旧节点保持不变，新节点通过标题弱提示来源，并自动用一条普通可编辑 `user` 边连接来源；这条边不引入正式父子分支树或机器可读 lineage。
...
- Claude Code Agent 的 `Fork` 动作：
  - `Fork` 只对 `Claude Code` provider 生效；第一版不为 `Codex` 暴露该能力。
  - 当前节点必须持有可信的 `claude-session-id`，来源沿用现有 Claude Code 恢复上下文确认规则。
  - 点击 `Fork` 后，宿主创建一个新的 Agent 节点并立即启动，不要求用户再点一次 `启动`。
  - 新节点启动命令必须使用 Claude Code 原生 fork 语义：`claude --resume <session-id> --fork-session`；它不是普通 `claude --resume <session-id>`。
  - 旧节点保持不变，用户仍可继续在旧节点对话；新节点通过标题弱提示来源，并自动创建一条普通可编辑 `user` 边；这条边不表示正式父子边、分支树或强持久化分支关系。
- 自定义启动输入约束：
  - 输入不能为空。
...
- 为 `Codex` 或其他 provider 实现 Fork；除非未来确认它们有等价的 provider 原生 fork 语义，否则第一版不提供。
...
- `重启` 按钮是否可执行 `Resume`
- `Fork` 按钮是否可执行 Claude Code 原生 fork；它依赖可信 `claude-session-id`，但执行命令必须追加 `--fork-session`
- 用户本次选择的是 `新建`、`重启` 还是 `Fork`
...
- 对持有可信 `claude-session-id` 的 Claude Code Agent，标题栏提供 `Fork` 动作；点击后创建新的 Claude Code Agent 节点并立即启动，启动命令包含 `--resume <session-id> --fork-session`，原节点状态不变化。
- 新 Fork 节点标题弱提示来源，例如以当前节点标题加 `Fork` 后缀表达；画布自动创建来源 Agent 指向新 Agent 的普通可编辑 `user` 边，但不新增正式父子边，也不要求用户区分哪个节点是“主分支”。
- `Codex` Agent、非 Claude provider、未持有可信 Claude session id 的 Agent 不会误触发 Fork 启动。
...
- `Fork` 当前只收口 Claude Code 的 `--fork-session` 语义；若未来 Codex 或其他 provider 提供等价能力，应另行补充 provider-specific 规则，而不是把普通 resume 当作 Fork。
