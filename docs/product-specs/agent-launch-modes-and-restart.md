...
6. 若通过命令面板或侧栏“创建节点”入口创建 Agent，先选对象/provider，再进入带输入框的第二步 Quick Input；第一项是 `确认创建`，默认按 Enter 使用输入框当前命令创建，下面的 `默认 / Resume / YOLO / 沙盒` 只负责替换输入框，不直接创建。
...
- VSCode Quick Input 创建链路：
  ...
  - 第二层顶部输入框始终代表本次真正要执行的完整命令；默认高亮第一项 `确认创建`，按 Enter 使用当前输入框创建。
  - `默认 / Resume / YOLO / 沙盒` 预设项排在确认项下方，只替换输入框内容，不直接创建。
- 默认启动参数设置：
...
- 命令面板 / 侧栏“创建节点”里的 Agent 入口会进入第二步 Quick Input；第二步输入框展示完整命令，第一项 `确认创建` 使用当前输入框创建 Agent。
- 第二步 Quick Input 的 `默认 / Resume / YOLO / 沙盒` 预设项只会替换输入框内容，不会直接创建；按 Enter 默认命中 `确认创建`，并按当前输入框内容创建 Agent。
