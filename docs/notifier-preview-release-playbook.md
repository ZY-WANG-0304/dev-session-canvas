...
- 统一 publish-tag dry-run 已在发布准备提交后用本地临时 `publish/v0.18.0` tag 执行 `npm run release:publish-tag -- --trigger-tag publish/v0.18.0 --dry-run --package-only --skip-origin-main-check`，确认计划产物包含 `extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.18.0.vsix` 与主扩展 VSIX，release ref 为 `c913ef9e69551cb994ba02733f48c5ca046e440a`；本地临时 tag 已删除。由于发布准备 head 尚未合入 `origin/main`，这里的 `--skip-origin-main-check` 只适用于本地预演，最终 `main` release ref 必须不带该参数复跑。
- `npm run -w extensions/vscode/dev-session-canvas-notifier package:vsix` 已通过，本地 notifier VSIX 为 10 files，约 145.15 KB，`VSCE README doc ref` 为当前发布准备提交 `c913ef9e69551cb994ba02733f48c5ca046e440a`；发布准备 MR 合并后，还需在最终 `main` ref 上复跑 notifier 打包，确认 VSIX 文件名、README doc ref、文件数与大小都来自最终发布 ref。
