...
- `0.24.4` 发布准备分支已同步 notifier manifest、notifier changelog 与本手册中的目标版本。notifier 本轮继续随主扩展对齐版本，但不引入新的通知投递行为。Node `25.6.0` / npm `11.8.0` 上的 typecheck、source、companion smoke 与英文 / 简体中文 locale smoke 均通过；候选 notifier VSIX 为 `14` files、`159,484 bytes`、`sha256=623019120ee262f8bd1ac4e3d7338652752c2505502b66e2293c323ca17291ab`，dirty-tree `VSCE README doc ref` 为 `69e74368a1f6765c9c6e4bcdb96ac916c8a3366b`。主扩展 / notifier clean-checkout packaged-payload 与完整 / production 审计验证均通过，候选 VSIX 与 README ref 均不得当作最终 Release asset 或最终 release ref。
