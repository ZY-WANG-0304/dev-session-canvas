...
- `paneGallery` 的 root 标签可在 root 内存在精确 `running` 执行节点且无 attention 时显示一条 `/` 形纯色斜线扫过整个 root 标题栏区域；该动效只属于 `paneGallery` root 标签，不用于 `rootGroups` 系统 root 分组，颜色取 root 标签当前 border color 的纯色扫描线，并采用类似 Codex Working 状态的扁平扫光质感：使用 16px 宽移动段且透明度为 0.82、无渐变和外发光；速度比上一版更慢但循环空档更短；扫描线必须裁切在 root 标题栏区域内，标题栏原有 1px 底部分隔线保持不变。
