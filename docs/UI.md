...
- `paneGallery` 的 root 标签可在 root 内存在精确 `running` 执行节点且无 attention 时显示一个 `|` 形竖向纯色色块在 root title 所在的小框区域内左右往返运动；该动效只属于 `paneGallery` root 标签，不用于 `rootGroups` 系统 root 分组，颜色取 root 标签当前 border color，采用类似 Codex Working 状态的扁平活性提示：色块宽度为 16px、透明度为 0.82、无渐变和外发光；运动必须裁切在 root title 所在的小框区域内，title 文本保持在色块上方，标题栏原有 1px 底部分隔线保持不变。
