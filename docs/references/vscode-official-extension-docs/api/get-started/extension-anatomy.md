...
- Registers the [`onCommand`](../references/activation-events.md#onCommand) [**Activation Event**](../references/activation-events.md): `onCommand:helloworld.helloWorld`, so the extension becomes activated when user runs the `Hello World` command.
...
- Uses the [`contributes.commands`](../references/contribution-points.md#contributes.commands) [**Contribution Point**](../references/contribution-points.md) to make the command `Hello World` available in the Command Palette, and bind it to a command ID `helloworld.helloWorld`.
- Uses the [`commands.registerCommand`](../references/vscode-api.md#commands.registerCommand) [**VS Code API**](../references/vscode-api.md) to bind a function to the registered command ID `helloworld.helloWorld`.

Understanding these three concepts is crucial to writing extensions in VS Code:

- [**Activation Events**](../references/activation-events.md): events upon which your extension becomes active.
- [**Contribution Points**](../references/contribution-points.md): static declarations that you make in the `package.json` [Extension Manifest](#extension-manifest) to extend VS Code.
- [**VS Code API**](../references/vscode-api.md): a set of JavaScript APIs that you can invoke in your extension code.

In general, your extension would use a combination of Contribution Points and VS Code API to extend VS Code's functionality. The [Extension Capabilities Overview](../extension-capabilities/overview.md) topic helps you find the right Contribution Point and VS Code API for your extension.
...
Each VS Code extension must have a `package.json` as its [Extension Manifest](../references/extension-manifest.md). The `package.json` contains a mix of Node.js fields such as `scripts` and `devDependencies` and VS Code specific fields such as `publisher`, `activationEvents` and `contributes`. You can find descriptions of all VS Code specific fields in [Extension Manifest Reference](../references/extension-manifest.md). Here are some most important fields:
...
- `activationEvents` and `contributes`: [Activation Events](../references/activation-events.md) and [Contribution Points](../references/contribution-points.md).
