...
> 视频资源已在本地瘦身副本中移除：Launch your first VS Code extension video。
...
> 视频资源已在本地瘦身副本中移除：Reload VS Code extension video。

Here are some ideas for things for you to try:

- Give the **Hello World** command a new name in the Command Palette.
- [Contribute](../references/contribution-points.md) another command that displays current time in an information message. Contribution points are static declarations you make in the `package.json` [Extension Manifest](../references/extension-manifest.md) to extend VS Code, such as adding commands, menus, or keybindings to your extension.
- Replace the `vscode.window.showInformationMessage` with another [VS Code API](../references/vscode-api.md) call to show a warning message.
...
> 视频资源已在本地瘦身副本中移除：Debug VS Code extension video。
...
In the next topic, [Extension Anatomy](extension-anatomy.md), we'll take a closer look at the source code of the `Hello World` sample and explain key concepts.

You can find the source code of this tutorial at: [https://github.com/microsoft/vscode-extension-samples/tree/main/helloworld-sample](https://github.com/microsoft/vscode-extension-samples/tree/main/helloworld-sample). The [Extension Guides](../extension-guides/overview.md) topic contains other samples, each illustrating a different VS Code API or Contribution Point, and following the recommendations in our [UX Guidelines](../ux-guidelines/overview.md).
...
This is also a good time to review our [UX Guidelines](../ux-guidelines/overview.md) so you can start designing your extension user interface to follow the VS Code best practices.
