...
The availability of commands and views and many other contributions can be controlled through context keys in [when clauses](../references/when-clause-contexts.md).
...
This separation also helps with [Untrusted Workspaces](workspace-trust.md) running in [Restricted Mode](https://code.visualstudio.com/docs/editor/workspace-trust#restricted-mode). Rich language extensions often require trust while basic language features can run in any setup.
