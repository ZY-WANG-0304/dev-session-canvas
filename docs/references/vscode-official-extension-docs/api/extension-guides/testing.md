...
The `testing/item/context` [menu contribution point](../references/contribution-points.md#contributes.menus) may be used to add menu items to Tests in the Test Explorer view. Place menu items in the `inline` group to have them inline. All other menu item groups will be displayed in a context menu accessible using the mouse right-click.

Additional [context keys](../references/when-clause-contexts.md) are available in the `when` clauses of your menu items: `testId`, `controllerId`, and `testItemHasUri`. For more complex `when` scenarios, where you want actions to be optionally available for different Test Items, consider using the [`in` conditional operator](../references/when-clause-contexts.md#in-and-not-in-conditional-operators).
