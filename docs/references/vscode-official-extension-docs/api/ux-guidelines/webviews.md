...
If you need to display custom functionality that is beyond what the VS Code API supports, you can use [webviews](../extension-guides/webview.md), which are fully customizable. It's important to understand that webviews should only be used if you absolutely need them.
...
* Ensure all elements in the view are themeable (see the [webview-view-sample](https://github.com/microsoft/vscode-extension-samples/blob/main/webview-view-sample/media/main.css) and [color tokens](../references/theme-color.md) documentation)
...
You can also place webviews into any view container (sidebar or panel) and these elements are called [webview views](../references/vscode-api.md#WebviewView). The same webview guidance applies to webview views.
...
* [Webview extension guide](../extension-guides/webview.md)
