...
Icons can be used in Markdown labels in hovers, in the  [StatusBarItem](vscode-api.md#StatusBarItem) text and [QuickPickItem](vscode-api.md#QuickPickItem) label API. The syntax for adding an icon in Markdown is `$(iconIdentifier)`:
...
[Product Icon Themes](../extension-guides/product-icon-theme.md) can replace each icon individually, as well as all icons from the codicon library.
