...
**Activation Events** is a set of JSON declarations that you make in the `activationEvents` field of `package.json` [Extension Manifest](extension-manifest.md). Your extension becomes activated when the **Activation Event** happens. Here is a list of all available **Activation Events**:

- [`onAuthenticationRequest`](activation-events.md#onAuthenticationRequest)
- [`onChatParticipant`](activation-events.md#onChatParticipant)
- [`onCommand`](activation-events.md#onCommand)
- [`onCustomEditor`](activation-events.md#onCustomEditor)
- [`onDebug`](activation-events.md#onDebug)
  - [`onDebugAdapterProtocolTracker`](activation-events.md#onDebugAdapterProtocolTracker)
  - [`onDebugDynamicConfigurations`](activation-events.md#onDebugDynamicConfigurations)
  - [`onDebugInitialConfigurations`](activation-events.md#onDebugInitialConfigurations)
  - [`onDebugResolve`](activation-events.md#onDebugResolve)
- [`onEditSession`](activation-events.md#onEditSession)
- [`onFileSystem`](activation-events.md#onFileSystem)
- [`onIssueReporterOpened`](activation-events.md#onIssueReporterOpened)
- [`onLanguage`](activation-events.md#onLanguage)
- [`onLanguageModelTool`](activation-events.md#onLanguageModelTool)
- [`onNotebook`](activation-events.md#onNotebook)
- [`onOpenExternalUri`](activation-events.md#onOpenExternalUri)
- [`onRenderer`](activation-events.md#onRenderer)
- [`onSearch`](activation-events.md#onSearch)
- [`onStartupFinished`](activation-events.md#onStartupFinished)
- [`onTaskType`](activation-events.md#onTaskType)
- [`onTerminal`](activation-events.md#onTerminal)
    - [`onTerminalProfile`](activation-events.md#onTerminalProfile)
    - [`onTerminalShellIntegration`](activation-events.md#onTerminalShellIntegration)
- [`onUri`](activation-events.md#onUri)
- [`onView`](activation-events.md#onView)
- [`onWalkthrough`](activation-events.md#onWalkthrough)
- [`onWebviewPanel`](activation-events.md#onWebviewPanel)
- [`workspaceContains`](activation-events.md#workspaceContains)
- [`*`](activation-events.md#Start-up)

We also provide a reference of all fields in the [`package.json` extension manifest](extension-manifest.md).
...
The `onLanguage` event takes a [language identifier](https://code.visualstudio.com/docs/languages/identifiers) value.
...
**Rule of thumb:** If activation of a debug extension is lightweight, use `onDebug`. If it is heavyweight, use `onDebugInitialConfigurations` and/or `onDebugResolve` depending on whether the `DebugConfigurationProvider` implements the corresponding methods `provideDebugConfigurations` and/or `resolveDebugConfiguration`. See [Using a DebugConfigurationProvider](https://code.visualstudio.com/api/extension-guides/debugger-extension#using-a-debugconfigurationprovider) for more details on these methods.

## workspaceContains

`workspaceContains:path` is emitted and interested extensions will be activated whenever a folder is opened and the folder contains at least one file that matches a [glob pattern](https://code.visualstudio.com/docs/editor/glob-patterns).
...
This activation event is emitted and interested extensions will be activated whenever VS Code needs to restore a [webview](../extension-guides/webview.md) with the matching `viewType`.
...
This activation event is emitted and interested extensions will be activated whenever VS Code needs to create a [custom editor](https://code.visualstudio.com/api/extension-guides/custom-editors) with the matching `viewType`.
...
will cause the extension to be activated when VS Code needs to restore a custom editor with the viewType: `catCustoms.pawDraw`. The viewType is set in the [`customEditors` contribution point](https://code.visualstudio.com/api/extension-guides/custom-editors#contribution-point) and bound to a provider with `registerCustomEditorProvider`.
