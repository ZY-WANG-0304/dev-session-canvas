...
- The badges provided in the `package.json` may not be SVGs unless they are from [trusted badge providers](../references/extension-manifest.md#approved-badges).
- Image URLs in `README.md` and `CHANGELOG.md` need to resolve to `https` URLs.
- Images in `README.md` and `CHANGELOG.md` may not be SVGs unless they are from [trusted badge providers](../references/extension-manifest.md#approved-badges).
...
A **publisher** is an identity that can publish extensions to the Visual Studio Code Marketplace. Every extension needs to include a `publisher` identifier in its [`package.json` file](../references/extension-manifest.md).
...
Both commands will first modify the extension's `package.json` [version](../references/extension-manifest.md#fields) attribute and then publish it with the updated version.
...
See more information in [Marketplace Presentation Tips](../references/extension-manifest.md#marketplace-presentation-tips).
...
Managing multiple platform-specific VSIXs might get overwhelming, so we suggest automating your extension's build process with [continuous integration](continuous-integration.md) (CI) tooling. For example, you can use [GitHub Actions](https://github.com/features/actions) to build your extensions. Our [platform-specific extension sample](https://github.com/microsoft/vscode-platform-specific-sample) can be used as a starting point for learning: its [workflow](https://github.com/microsoft/vscode-platform-specific-sample/blob/main/.github/workflows/ci.yml) enables the common scenario of using platform-specific extension support to distribute native node modules as dependencies across all supported VS Code targets.

## Next steps

- [Extension Marketplace](../../docs/configure/extensions/extension-marketplace.md) - Learn more about VS Code's public Extension Marketplace.
- [Testing Extensions](testing-extension.md) - Add tests to your extension project to ensure high quality.
- [Bundling Extensions](bundling-extension.md) - Improve load times by bundling your extension files with webpack.
...
Yes, see the [Automated publishing](continuous-integration.md#automated-publishing) section of the [Continuous Integration](continuous-integration.md) topic to learn how to configure Azure DevOps, GitHub Actions, and GitLab CI to automatically publish your extension to the Marketplace.

### I get "ERROR The extension 'name' already exists in the Marketplace" error when I try to publish my extension?

The Marketplace requires the [extension name](../references/extension-manifest.md) to be unique for every extension. If an extension with the same name already exists in the Marketplace, you will get the following error:

```
ERROR The extension 'name' already exists in the Marketplace.
```

The same rule applies for the [display name](../references/extension-manifest.md) of an extension.
