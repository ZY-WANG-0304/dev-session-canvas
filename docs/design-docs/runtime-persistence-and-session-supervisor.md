...
- 风险：历史 slot 里若残留旧版或损坏的 snapshot，且缺少可比较的时间戳，跨 slot 新鲜度判断仍会退回 recoverable-state fallback，不能像完整元数据那样可靠比较“谁更新”。
  当前缓解：当前主快照已经补上 `writtenAt` 与 `stateHash`，`workspaceStorage` 多 slot 恢复会优先按 snapshot 新鲜度选择 source；若 source 不是 current slot，会先把 `canvas-state.json`、`runtime-supervisor/` 与 `agent-runtime/` 迁回 current slot，再从 current slot 继续读写。测试态诊断现已包含 `storage/slotSelected`、`storage/stateMigratedToCurrentSlot`、`state/loadSelected`、`state/persistQueued` 与 `state/persistWritten`，便于直接比对 slot 选择与 `stateHash` 一致性。
...
- 已经被本地验证覆盖的行为包括：
  ...
  - `workspaceStorage` 多 slot 场景下按 snapshot 新鲜度选 source，并在必要时迁回 current slot 后继续读写
  ...
  - `npm run test:smoke-storage-slot` 覆盖 sibling fresher snapshot 迁回 current slot、`storage/slotSelected` / `state/loadSelected` `stateHash` 一致性，以及恢复后的写入仍留在 current slot
