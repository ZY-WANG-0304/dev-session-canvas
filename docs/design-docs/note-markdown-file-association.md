...
- `npm run test:note-markdown-file-association`、`npm run typecheck`、`node --check tests/vscode-smoke/extension-tests.cjs`、`git diff --check` 与 `npm run build` 通过；本轮覆盖 Host 从 Webview resource URI 推断完整 Remote authority、当前 Remote authority 可独立于 workspace containment 隐藏 `ssh:设备id` 前缀、不同 Remote authority 继续保留前缀，且不回退到只比较 `ssh-remote` 这类 remote kind；当时曾补充完整 authority 暂不可得时的临时 fallback，后续已由最新验证记录收敛为 fail closed。
...
- `npm run test:note-markdown-file-association`、`npm run typecheck`、`node --check tests/vscode-smoke/extension-tests.cjs`、`git diff --check` 与 `npm run build` 通过；本轮移除完整 authority 不可得时的 workspace-contained / same remote kind + filesystem existence fallback，改为 fail closed，并补充首次成功推断 current Remote authority 后主动触发 host-side refresh 与 watcher resync 的回归断言。
