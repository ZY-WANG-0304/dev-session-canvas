...
- `images/dev-session-canvas-icon.svg`
- `images/icon.png`
- `images/avatar.png`
...
- badge glyph 只表达 view 的差异语义，不重复产品主体语义；例如 notifier 用 bell、templates 用模板网格、nodes 用三行节点圆点与文本线、sessions 参考 VS Code Codicon `history` 的时钟指针部分。
...
- badge glyph 优先使用 1 到 3 个简单元素；禁止放入文字、品牌 logo 或多色填充。若语义已经有明确 VS Code Codicon 对应项，可参考对应单色路径并按 badge 内容外径圆简化，例如会话历史只保留 `history` 的时钟指针部分。
...
- `images/dev-session-canvas-sessions-activitybar.svg`：会话历史 view section，badge 内部参考 VS Code Codicon `history`，只保留时钟指针部分。
...
- `images/avatar.png` 是主扩展可用于圆形头像场景的 PNG 资产，要求可见像素保留在头像圆形安全区内。
- `images/dev-session-canvas-templates-activitybar.svg`、`images/dev-session-canvas-nodes-activitybar.svg` 与 `images/dev-session-canvas-sessions-activitybar.svg` 是主扩展 sidebar section 的专属 view icon。
...
- 未来新增专属 sidebar view icon 时，应优先复用 5.4 的 badge 约定：保留主 glyph、右上角单色圆形 badge、内部 glyph 用 mask 镂空；若 VS Code Codicon 已有强对应语义，优先参考其单色路径并按 24px 可读性简化。
- 其他历史 icon 草稿应从仓库中移除。
...
- 2026-06-15 新增 `images/dev-session-canvas-sessions-activitybar.svg`，用于会话历史 view section；badge 内部参考 VS Code Codicon `history` 的时钟指针部分，并已纳入 `npm run test:activitybar-badges` 的 badge 半径与简化形态检查。
- 2026-05-12 已新增主扩展头像图 `images/avatar.png`，并通过 `npm run test:avatar` 验证 `512x512` 头像可见像素位于半径 `244px` 的圆形安全区内。
