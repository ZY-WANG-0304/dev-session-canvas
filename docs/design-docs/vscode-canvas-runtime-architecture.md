...
要在 VSCode 内落地 DevSessionCanvas 当前定义的核心协作体验，至少要回答以下问题：
...
6. Agent 能力应如何被接入，才能既贴近当前产品定义里的真实对象语义，又不继续引入一套和 Terminal 平行的重型运行时模型。
...
- 因此主交互必须在节点内部承载，核心是 provider 标识、会话状态和嵌入式终端前端。
...
- 取舍：选择“嵌入式终端节点”会引入独立 PTY 后端和输出桥接复杂度。
  原因：但如果继续停留在原生终端代理节点，已经无法满足当前明确产品目标。
...
3. 嵌入式终端节点原型
   验证节点内输入输出、状态回传、最近输出摘要、重新附着与关闭回流。
