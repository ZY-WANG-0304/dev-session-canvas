...
- 媒体编排与原生输入都收口在 `scripts/media/recording-session.mjs` 与 `scripts/media/x11-native-input.py`；当前录制方案不需要再修改 `src/`。
...
- `scripts/media/recording-session.mjs record-start` / `record-stop` / `gif-frame` / `stop` 负责把真实窗口区域录成 MP4 clip、抓取 GIF storyboard frame，并在收尾时拼出 `images/marketplace/canvas-overview.mp4`、`images/marketplace/canvas-overview.gif` 与 `images/marketplace/canvas-overview.png`。
