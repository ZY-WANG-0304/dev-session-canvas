...
root-local 状态使用扩展 global storage 按 root 绝对路径稳定分桶保存，避免依赖 VSCode 未公开的“由任意 folder path 反查 workspaceStorage slot”能力。单根 workspace 加载时优先使用 root-local snapshot；如果 root-local 还不存在，则使用当前 workspace snapshot，并在持久化时镜像到 root-local。多根 workspace 只把 root 内状态写回 root-local storage，当前 multi-root workspace storage 只保存组合快照和 overlay。从 sidebar 移除 workspace folder 或 worktree 时，用户可以显式选择保留或清空对应 root-local canvas state；保留路径不修改该 root-local snapshot，后续重新加入同一路径仍可恢复画板内容。folder 的保留路径只从 Workspace 移除 folder；worktree 的保留路径仍会执行 `git worktree remove`，只是保留 root-local canvas state。清空路径的 modal 说明必须展示将被清空的节点、连线、分组数量，以及会停止多少个 Agent / Terminal，避免把“从 Workspace 移除”“清空画板”和“删除 worktree”混成一个不可感知的破坏性动作。sidebar 触发路径使用 sidebar 内 Webview modal 来固定三按钮布局：两个功能动作相邻、默认动作靠后、`取消` 放在末尾；非 sidebar 入口保留 VS Code modal fallback。
...
`paneGallery` 借鉴会议 gallery 的结构，而不是视觉风格。窗格不使用头像、视频控件、参会者计数或会议装饰；它们继续使用 `docs/UI.md` 的 VSCode theme token、紧凑 spacing 和状态色。每个 pane 左上角使用更轻、更浅的 multi-root root 标签，只显示 root 名称并通过 tooltip 暴露完整路径；nodes / waiting / attention 等汇总 subtitle 不常驻显示。root 标签 title 区域允许用弱背景色表达 root 内执行状态聚合提示：存在待确认 attention 时优先混入 attention 语义色；无 attention 但存在 starting / resuming / reattaching / running / live 执行节点时混入 running 语义色；无上述状态时保持普通 chrome 背景。该背景只负责 glance 级别提示，不闪烁、不扩大标题区域、不承担确认动作；可访问名称或 tooltip 需要补充“需要关注 / 正在运行”的文本信息，避免只靠颜色。active root 使用 `--vscode-focusBorder` 或状态 token 建立清晰边界；attention / error / waiting 等状态以节点自身状态、图标或 tooltip 辅助表达，不能只靠颜色。
...
2026-06-25 补充：workspace folder / worktree 移除确认选择框完成截图反馈优化。folder 使用“保留画板并移除 / 清空画板并移除”且默认保留画板；worktree 使用“移除 Worktree 并清空画板 / 移除 Worktree 但保留画板”且默认清空画板，两条 worktree 路径都会执行 `git worktree remove`。Host 通过 `getWorkspaceRootCanvasRemovalImpact(...)` 在 modal 说明中展示清空路径会影响的节点、连线、分组和 Agent / Terminal 数量，降低误清空风险。由于 VS Code 宿主 modal 的三按钮顺序会受平台控制，sidebar 触发路径改为 sidebar 内 Webview modal，固定为两个功能动作相邻、默认动作靠后、`取消` 在末尾；非 sidebar 命令入口继续使用 VS Code modal fallback。已补 `test:protocol-webview-messages` 源码断言、sidebar tooltip 断言与 sidebar modal 布局回归；已复跑 `npm run typecheck`、`npm run test:protocol-webview-messages`、`npm run test:sidebar-node-list`、`npm run test:extension-manifest`、`npm run test:canvas-multi-root-composition`、`npm run build` 与 `git diff --check`。

2026-06-24 补充：sidebar workspace folder / worktree 移除路径新增 root-local 画板清空选择。`src/panel/CanvasPanelManager.ts` 的 `clearWorkspaceRootCanvas(...)` 负责按 workspace root 收集所属节点、清理执行会话并写空 root-local snapshot；选择“仅从 Workspace 移除”时不触碰 root-local snapshot，因此后续重新加入同一路径仍可恢复画板。已补 `test:canvas-multi-root-composition` 覆盖 root-owned node 收集 helper，`test:protocol-webview-messages` 覆盖 Host 清空路径和 worktree workspace-only 分支，`test:sidebar-node-list` 覆盖移除按钮提示，并复跑 typecheck、manifest、sidebar、codicon、color、build 与 diff 检查。
