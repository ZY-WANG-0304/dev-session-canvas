...
`paneGallery` 借鉴会议 gallery 的结构，而不是视觉风格。窗格不使用头像、视频控件、参会者计数或会议装饰；它们继续使用 `docs/UI.md` 的 VSCode theme token、紧凑 spacing 和状态色。每个 pane 左上角使用更轻、更浅的 multi-root root 标签，只显示 root 名称并通过 tooltip 暴露完整路径；nodes / waiting / attention 等汇总 subtitle 不常驻显示。root 标签 title 区域不再用 running / attention 的静态状态背景或边框色表达聚合状态，常态只保持普通 chrome 背景与普通 border。存在待确认 attention 且 `devSessionCanvas.notifications.strongTerminalAttentionReminder` 会启用执行节点标题栏闪烁时，root 标签 title 区域进入同一套标题栏闪烁动画；该动画只用于 `paneGallery` 的 root 标签，不扩大标题区域、不承担确认动作，闪烁峰值与 reduced motion 静态高亮均使用 root 标签当前 border color，而不是重新取等待输入或原始 attention 状态色；动画峰值需要比常态状态染色更明确，通过更高占比背景混色、内外描边和轻量 halo 强化提示，并必须跟随 `prefers-reduced-motion` 退化为静态高亮。root 内存在精确 `running` 执行节点、且没有待确认 attention 时，root 标签标题区域可在文字下方显示一条 `/` 形纯色斜线背景扫过整个 root 标题栏区域；该扫描线只用于 `paneGallery` root 标签，不用于 `rootGroups` 系统 root 分组，触发条件不扩大到 `launching`、`live`、`starting`、`resuming`、`reattaching` 等生命周期状态。扫描线取 root 标签当前 border color 派生的纯色，并采用类似 Codex Working 状态的扁平扫光质感：移动段使用 16px 宽度与 0.82 透明度、无渐变和外发光，以避免脱离插件原生 chrome 风格；扫描速度比上一版更慢，且两次扫描之间不保留空档；扫描线只能裁切在 root 标题栏区域内扫过，不应越过标题栏边界，不覆盖 root title 文字，root 标签原有 1px `border-bottom` 保持不变，动画速度比 Agent 节点标题栏稍快以适应更短标题栏；attention 优先级更高，出现 attention 时隐藏 running 扫描线。`prefers-reduced-motion` 下关闭扫描线动画层，只保留原有底部分隔线。可访问名称或 tooltip 继续补充“需要关注 / 正在运行”的文本信息。active root 使用 `--vscode-focusBorder` 或状态 token 建立清晰边界；attention / error / waiting 等状态以节点自身状态、图标或 tooltip 辅助表达，不能只靠颜色。
