...
- `rootGroups` 中从某个 root 分组内右键触发“整理画布布局”时默认只整理当前 root；整理整个 workspace 画布进入同一菜单项的 `>` 二级菜单。
