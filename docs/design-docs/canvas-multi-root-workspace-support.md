...
2026-06-25 补充：workspace folder / worktree 移除确认选择框完成截图反馈优化。确认选择框改为纵向列表展示两条路径；folder 使用“保留画板并移除 / 清空画板并移除”且默认保留画板；worktree 使用“移除 Worktree 并清空画板 / 移除 Worktree 但保留画板”且默认清空画板，两条 worktree 路径都会执行 `git worktree remove`。Host 通过 `getWorkspaceRootCanvasRemovalImpact(...)` 在 QuickPick 说明中展示清空路径会影响的节点、连线、分组和 Agent / Terminal 数量，降低误清空风险。改用纵向 QuickPick 选择，取消由 Esc/点击外部完成，避免截图中的 `Cancel` 挤在两个动作按钮之间造成布局和阅读顺序割裂。已补 `test:protocol-webview-messages` 源码断言与 sidebar tooltip 断言；已复跑 `npm run typecheck`、`npm run test:protocol-webview-messages`、`npm run test:sidebar-node-list`、`npm run test:extension-manifest`、`npm run test:canvas-multi-root-composition`、`npm run build` 与 `git diff --check`。
