...
- 通过 Dev Session Canvas sidebar 的 `节点` section 添加或移除 folder / worktree，以及添加已有 worktree 到 workspace 时，仍复用同一套 root 分组组合语义与新增 root 聚焦规则；移除 folder / worktree 的二次确认使用 VS Code 原生 modal，并在移除前让用户选择是否清空对应 root 画板。
...
第一版将交互分成完整 root 子画板与缩略图。`dynamic` 和 `grid` 中每个 root pane 都是可交互子画板，支持在该 root 内创建 Note / Agent / Terminal、拖拽节点、输入终端、拖入 Markdown、应用模板和重置为模板；窗格边界直接决定目标 root。`topThumbnails` / `sideThumbnails` 中 active root 主画板同样承载深度交互，并尽量复用主线单一画板的控件、缩放体验和右下角 MiniMap；`dynamic` / `grid` 的多 root 子画板不渲染 MiniMap，避免每个 pane 同时出现导航小窗造成视觉噪音。缩略图仍渲染对应 root 的画板 fit view，而不是复用 MiniMap 或静态 SVG 预览；缩略图可以完整挂载对应 root 的子画板并进行 execution snapshot / live 状态 hydrate，以保证缩略图内容与切换为主画板时的连续性。这里的“不可交互 / 不承载终端输入”仅指缩略图不接受由缩略图内用户交互触发的 terminal input、节点拖拽、编辑、创建、drop 或 start / stop 等操作/消息；snapshot hydrate、attach 和正常执行生命周期同步不在禁用范围内。缩略图复用左上角轻量 root 标签，不在缩略图外再包一层标题卡片；缩略图单击只选中或预览缩略图，不切换主画板；只有双击缩略图才把对应 root 变成 active root 主画板，避免误触。缩略图 rail 的排列顺序始终跟随 VS Code multi-root workspace folder 顺序：active root 也保留在 rail 的原 workspace 顺序槽位，但渲染为等尺寸占位卡片而不是第二个真实 `ReactFlow` 缩略图；该占位使用轻微斜纹、普通 VSCode surface / border token 和“正在主画板”文案说明对应 root 已在主画板中显示，不显示 status dot，不承载单击或双击切换，不触发创建、拖拽、输入、drop 或 start / stop 等消息。占位的 `title` / `aria-label` 继续包含 root 名称、完整路径和 attention / running 文本摘要，使状态可通过 tooltip 与无障碍路径获取，但视觉状态仍优先由主画板 root 标签和节点自身表达。其他非 active root 保持真实不可交互画板缩略图并按 workspace 顺序排列；若某个 root 暂时无法匹配 workspace folder path，才保留 composed state 中的原相对顺序作为兜底。
