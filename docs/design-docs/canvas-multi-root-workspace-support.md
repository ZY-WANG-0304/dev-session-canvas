...
- `rootGroups` 或 `paneGallery` 中从某个 root 内右键触发“整理画布布局”时主菜单默认只整理当前 root；同一菜单项的 `>` 二级菜单提供当前 root 与整个 workspace 两个整理范围。
...
- 通过 Dev Session Canvas sidebar 的 `节点` section 添加或移除 folder / worktree，以及添加已有 worktree 到 workspace 时，仍复用同一套 root 分组组合语义与新增 root 聚焦规则；节点列表中的系统 workspace root 分组按 VS Code 当前 workspace folder 顺序排列，与 `paneGallery` root 稳定排序保持同一 root 心智；移除 folder / worktree 的二次确认使用 VS Code 原生 modal，并在移除前让用户选择是否清空对应 root 画板。
...
不做独立 app 式 workspace 管理、项目启动器或 root 切换器。不把多根窗格画廊做成多人实时协作、视频会议或远程共享白板；参考会议截图只用于说明 gallery / thumbnails 的空间组织方式。不把 multi-root workspace 做成完全隔离的第三份画布状态。不在第一版实现跨 root overlay Note、跨 root 连线或跨 root 模板捕获。不把 multi-root 组合视图整体保存或重置为模板；在跨 root 模板正式设计前，模板应用与重置都只作用于单个目标 root。不允许删除、取消分组或重命名系统 root 分组。不把跨 root 外层普通分组写入任一 root-local state。不把窗格内 viewport、active root 或缩略图顺序写入 root-local state。不承诺旧版本所有历史单根 VSCode workspaceStorage 都能被后台自动发现。不在同一个 Host 内支持多个 display node 同时呈现同一个 runtime；当前 `runtimeSessionBindings` 仍是一条 runtime key 对应一个 display node，未来如需同 Host 多视图呈现，应升级为 subscribers/list。
...
`paneGallery` 不增加第四份事实状态。它在 `extensions/vscode/dev-session-canvas/src/webview/main.tsx` 从同一份 composed state 派生 root pane view model：每个 view model 由一个 `role === 'workspace-root'` 的 `CanvasGroupSummary`、该 root 拥有的 nodes / groups / edges 和运行摘要派生而来，但 pane 内不渲染 `rootGroups` 模式的 workspace-root 分组框或水印，因为窗格边界已经承担 folder 区分。`extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts` 仍是宿主权威状态入口；`extensions/vscode/dev-session-canvas/src/common/canvasMultiRootComposition.ts` 仍负责纯数据组合/拆分；`extensions/vscode/dev-session-canvas/src/common/protocol.ts` 只需要新增运行时配置字段，不应让 Host 为窗格模式维护另一套节点图。
...
`rootGroups` 与 `paneGallery` 中的右键整理入口遵循 root-first 规则。`extensions/vscode/dev-session-canvas/src/webview/main.tsx` 在打开画布上下文菜单时，如果命中 workspace-root body、title/border chrome 或其内部用户分组，主菜单“整理画布布局”直接发送 `webview/arrangeCanvasLayout` 并携带所属 workspace-root group id；如果命中 `paneGallery` 的可交互 root pane，主菜单携带该 pane 对应的 workspace-root group id；同一行右侧 `>` 打开二级菜单，二级菜单同时提供“整理当前 root 内的节点”和“整理整个 workspace 的画布”，当前 root 选项发送带目标 group 的整理消息，整个 workspace 选项发送不带目标 group 的整理消息。`extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts` 收到带 `targetGroupId` 的消息后再次把普通用户分组解析到所属 workspace root，并只调用 `extensions/vscode/dev-session-canvas/src/common/canvasLayoutArrangement.ts` 的目标分组整理能力；文件活动 reconciliation 随后只能使用 scoped geometry repair，member expansion 与 sibling overlap repair 都限定在目标 root 子树所在 collection，非目标 root 和 workspace-level overlay 的节点/分组几何保持不变；如果目标 root 扩张后与其他 root 发生冲突，也仅平移目标 root 子树到非重叠位置。这样默认整理只影响当前 root 内节点和用户分组，不移动其他 root 分组，也不重排 multi-root overlay。入口位于所有 root section 外部或单根 workspace 时，仍沿用原来的直接整理当前可交互画布语义。
...
`paneGallery` 借鉴会议 gallery 的结构，而不是视觉风格。窗格不使用头像、视频控件、参会者计数或会议装饰；它们继续使用 `docs/UI.md` 的 VSCode theme token、紧凑 spacing 和状态色。每个 pane 左上角使用更轻、更浅的 multi-root root 标签，只显示 root 名称并通过 tooltip 暴露完整路径；nodes / waiting / attention 等汇总 subtitle 不常驻显示。root 标签 title 区域不再用 running / attention 的静态状态背景或边框色表达聚合状态，常态只保持普通 chrome 背景与普通 border。存在待确认 attention 且 `devSessionCanvas.notifications.strongTerminalAttentionReminder` 会启用执行节点标题栏闪烁时，root 标签 title 区域进入同一套标题栏闪烁动画；该动画只用于 `paneGallery` 的 root 标签，不扩大标题区域、不承担确认动作，闪烁峰值与 reduced motion 静态高亮均使用 root 标签当前 border color，而不是重新取等待输入或原始 attention 状态色；动画峰值需要比常态状态染色更明确，通过更高占比背景混色、内外描边和轻量 halo 强化提示，并必须跟随 `prefers-reduced-motion` 退化为静态高亮。root 内存在精确 `running` 执行节点、且没有待确认 attention 时，root 标签 title 所在的小框区域可在文字下方显示一个 `|` 形竖向纯色色块；该色块只用于 `paneGallery` root 标签，不用于 `rootGroups` 系统 root 分组，触发条件不扩大到 `launching`、`live`、`starting`、`resuming`、`reattaching` 等生命周期状态。色块取 root 标签当前 border color 派生的纯色，并采用类似 Codex Working 状态的扁平活性质感：宽度为 16px、透明度为 0.82、无渐变和外发光，以避免脱离插件原生 chrome 风格；色块在 root title 所在的小框区域内左右往返运动，运动范围只能裁切在 root title 所在的小框区域内，不应越过该小框边界，不覆盖 root title 文字，root 标签原有 1px `border-bottom` 保持不变，动画速度比 Agent 节点标题栏稍快以适应更短标题栏；attention 优先级更高，出现 attention 时隐藏 running 色块。`prefers-reduced-motion` 下关闭色块动画层，只保留原有底部分隔线。可访问名称或 tooltip 继续补充“需要关注 / 正在运行”的文本信息。active root 使用 `--vscode-focusBorder` 或状态 token 建立清晰边界；attention / error / waiting 等状态以节点自身状态、图标或 tooltip 辅助表达，不能只靠颜色。

第一版将交互分成完整 root 子画板与缩略图。`dynamic` 和 `grid` 中每个 root pane 都是可交互子画板，支持在该 root 内创建 Note / Agent / Terminal、拖拽节点、输入终端、拖入 Markdown、应用模板和重置为模板；窗格边界直接决定目标 root。`topThumbnails` / `sideThumbnails` 中 active root 主画板同样承载深度交互，并尽量复用主线单一画板的控件、缩放体验和右下角 MiniMap；`dynamic` / `grid` 的多 root 子画板不渲染 MiniMap，避免每个 pane 同时出现导航小窗造成视觉噪音。缩略图仍渲染对应 root 的画板 fit view，而不是复用 MiniMap 或静态 SVG 预览；缩略图可以完整挂载对应 root 的子画板并进行 execution snapshot / live 状态 hydrate，以保证缩略图内容与切换为主画板时的连续性。这里的“不可交互 / 不承载终端输入”仅指缩略图不接受由缩略图内用户交互触发的 terminal input、节点拖拽、编辑、创建、drop 或 start / stop 等操作/消息；snapshot hydrate、attach 和正常执行生命周期同步不在禁用范围内。缩略图复用左上角轻量 root 标签，不在缩略图外再包一层标题卡片；缩略图单击只选中或预览缩略图，不切换主画板；只有双击缩略图才把对应 root 变成 active root 主画板，避免误触。`paneGallery` 的 root pane/thumbnail 和 sidebar `节点` section 的系统 workspace root 分组都以 VS Code multi-root workspace folder 顺序作为稳定排序：thumbnail rail 中 active root 也保留在 rail 的原 workspace 顺序槽位，但渲染为等尺寸占位卡片而不是第二个真实 `ReactFlow` 缩略图；该占位使用轻微斜纹、普通 VSCode surface / border token 和“正在主画板”文案说明对应 root 已在主画板中显示，不显示 status dot，不承载单击或双击切换，不触发创建、拖拽、输入、drop 或 start / stop 等消息；占位的 `title` / `aria-label` 继续包含 root 名称、完整路径和 attention / running 文本摘要，使状态可通过 tooltip 与无障碍路径获取，但视觉状态仍优先由主画板 root 标签和节点自身表达。其他非 active root 保持真实不可交互画板缩略图并按 workspace 顺序排列；节点列表按分组树展示或平铺展示 root 分组时，也不能因标题字母序、节点数量、attention 状态或画布空间位置改变 workspace folder 顺序。若某个 root 暂时无法匹配 workspace folder path，才保留 composed state 中的原相对顺序作为兜底。


`paneGallery` 的每个可交互 root pane 内部仍使用命名空间化 display id。创建 Note / Agent / Terminal、拖入 Markdown、应用模板、重置为模板或右键创建时，Webview 发送的目标 group 是该窗格对应的 workspace-root group；宿主按现有 multi-root 规则写回对应 root-local state。重置为模板只清空并替换该 pane 所属 root 的 root-local 画布及该 root 内执行会话，不清空其他 root。缩略图不承载重置、应用、创建、拖拽或输入等写操作。跨 root 连线、跨 root 拖拽迁移和静默改写 `cwd` 仍被拒绝。用户如果需要整理 root 分组的空间位置、resize root 分组或创建包含多个 root 的 overlay 普通分组，应切回 `rootGroups`。
...
模板功能在支持跨 root 模板前只做 root-scoped 物化。`extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts` 的 `applyCanvasTemplateRecord()` 在 multi-root 中必须解析出目标 workspace-root group：右键落在 `rootGroups` 的 workspace-root body 或其内部用户分组时使用该分组所属 root；`paneGallery` 的可交互 root pane 或 thumbnail 模式 active root 主画板直接使用 pane 身份；命令面板、sidebar 或其他无法从位置推断 root 的入口由 Host 通过 QuickPick 选择 root。追加模板时，Host 先把目标 root 拆成 root-local state，在该 root-local state 内执行模板避碰和关联 Note 物化，再 namespacing 回 composed state。重置为模板时，Host 以空 root-local state 作为应用基底，调用 `prepareWorkspaceRootCanvasForTemplateReset()` 只终止目标 root 内执行节点、清理待写入 terminal 初始输入与关联 Note 编辑草稿，然后通过 `composeRootLocalCanvasStateIntoComposed()` 替换该 root；不得调用会清理整个 Host 的 `prepareForHostBoundary()`，也不得清空其他 root、multi-root overlay 或其他 root 的 live runtime。由于当前模板格式不表达跨 root 内容，`rootGroups` 中从所有 root section 外部触发“重置为模板”时，`extensions/vscode/dev-session-canvas/src/webview/main.tsx` 必须直接提示“多根 workspace 中请在目标 root section 内重置为模板。”并关闭菜单，不向 Host 发送 reset 请求。
...
已有 `rootGroups` 验证继续保留：新增 root composition 纯函数测试覆盖 ID 命名空间、root 分组 overlay、组合/拆分、root 内新增对象归属、overlay 外层分组重组，以及 Add Folder 新增 root 按可见中心就近避让并写回 overlay。扩展分组测试覆盖系统 root 分组不可删除/取消分组/重命名、root 内扩边、root-root 避让和 root 被外层分组包含。扩展协议、模板、Markdown 拖入和执行 cwd 测试；模板相关测试还必须覆盖 multi-root reset 只清理目标 root、`rootGroups` 外部 reset 提示且不发 Host 消息、root section 内部用户分组可解析到所属 root，以及 `paneGallery` 可交互 pane reset 携带 pane 对应 root。sidebar workspace 操作需要用 manifest 与节点列表 Webview 测试覆盖：`节点` view title 暴露添加 folder / worktree，workspace folder 分组行只在系统 root 上显示 folder kind 图标与 folder 级 worktree / remove-worktree / remove-folder 操作，worktree action 使用 `worktree` Codicon，并向 Host 发送 rootPath；节点列表 Webview 还必须覆盖系统 workspace root 分组按当前 VS Code workspace folder 顺序排列，且标题字母序与画布空间位置不能改变侧栏顺序；添加已有 worktree 的 `git worktree list --porcelain` 解析、不执行 `git worktree add` 路径，以及全局 worktree 入口按 git common dir 合并同一 repository 的选择逻辑由 `npm run test:git-worktrees` 覆盖；worktree QuickPick 在真实 VS Code 宿主中的完整交互仍需人工验证。导航与 MiniMap 需要保留 Webview Playwright：空 root 分组没有节点时仍被全局 fit view 纳入；多个 root 分组在 MiniMap 中可见且与普通用户分组可区分；`host/focusGroup` 能用缩放平移动画把新增 root 分组居中。live runtime 恢复需要覆盖 multi-root reload 后 Agent / Terminal 真实 reattach、离线输出可见、单根窗口与 multi-root 窗口同时 attach 同一 session、resize last-writer-wins，以及缺失 `runtimeStoragePath` 的迁移或显式降级回归。

新增 `paneGallery` 实现时必须补充验证：配置默认值为 `rootGroups`；将 runtime context 切到 `paneGallery` 后 Webview 渲染 root pane 数量等于 workspace root 数量；不出现整条 paneGallery 顶部 toolbar、filter roots 搜索框或右上角模式入口，但右上角使用提示入口仍可见；root 内精确 `running` 执行节点只让 `paneGallery` root 标签显示 Codex Working 风格的 `|` 形竖向色块在 root title 所在的小框区域内左右往返运动，`launching` / `live` / `starting` / `resuming` / `reattaching` 不触发色块，attention 存在时色块隐藏，`rootGroups` 模式不渲染 paneGallery root 运行色块属性；左下角画板控制区提供模式入口，全览态为 `eye`、thumbnail 态为 `globe`，点击执行粗切换，hover 后始终暴露四个具体模式，并验证 `dynamic` / `grid` / `topThumbnails` / `sideThumbnails` 选项分别使用 `layout` / `table` / `split-vertical` / `split-horizontal` 且菜单样式对齐画板控制按钮；粗切换默认从全览进入 `sideThumbnails`、从 thumbnail 返回 `dynamic`，并在用户选择后记住上次的全览/缩略图具体模式；`dynamic` / `grid` pane 显示轻量 root 标签和路径 tooltip，不显示 nodes / waiting / attention subtitle，默认 fit 当前 root 可见子图（`nodes + groups`）且不额外限制缩放比例，空 root 不 fallback 到 workspace-root 外框也不自动收紧 root overlay，per-pane overview 跟随各 pane 自己的 zoom，root 内容过大时 pane 内 pan/zoom 可用；overview viewport 与 thumbnail 主画板 main viewport 必须按 root 分离记忆，root 首次成为 thumbnail 主画板时自动 fit，后续切换主画板恢复 main viewport，切回 `dynamic` / `grid` 恢复 overview viewport，缩略图自身 fit view 不写入 viewport；root 数量超过同屏容量时 gallery 保持最小可交互 pane 尺寸并通过滚动/虚拟化或等价机制访问所有 root；`dynamic` / `grid` pane 内创建 Note / Agent / Terminal、拖拽节点、terminal input 和 Markdown drop 的目标 root 正确；切到 `topThumbnails` 或 `sideThumbnails` 后 active root 主画板可见且右下角 MiniMap 可见，active root 在 rail 原 workspace 顺序槽位显示等尺寸斜纹占位且不挂载第二份真实 `ReactFlow`，其他 root 进入不可交互的画板缩略图 rail，顶部 rail 未溢出时横向居中、右侧 rail 未溢出时纵向居中，溢出时滚动起点能看到第一张 root 槽位且滚动终点能看到最后一张 root 槽位；缩略图 rail 在 active root 切换前后都必须保留 VS Code workspace folder 顺序，不能把双击的缩略图与原 active root 做位置互换；`dynamic` / `grid` 子画板不显示 MiniMap；缩略图复用左上角轻量标签，不使用外部标题卡片或 MiniMap/SVG 预览；在 thumbnail 模式缩略图中允许只读预加载与 execution snapshot 同步，单击 root 不切换 active root，双击非 active root 缩略图只切换 active root，active root 占位不响应单击 / 双击切换，且缩略图和占位都不会发送由用户交互触发的 create / drag / edit / terminal input / start / stop / drop 等消息，也不会因缩略图内用户交互写入 root-local state；paneGallery 不渲染 workspace-root 分组框或水印；active root 主窗格创建 Note / Agent / Terminal 时目标 root 正确，单独打开该 root 可见对象；跨 root 连线和跨 root 拖拽仍被拒绝；切回 `rootGroups` 后 overlay 位置未被 paneGallery 改写。最终运行 `npm run typecheck`、`npm run build`、`npm run test:canvas-multi-root-composition`、相关 Playwright grep 和 `git diff --check`。

## 9. 当前验证状态

2026-07-02 补充：sidebar `节点` section 的系统 workspace root 分组排序已与 `paneGallery` root 稳定排序对齐，按当前 VS Code workspace folder 顺序呈现，不再受 root 标题字母序影响。本轮实现由 Host 为 root group snapshot 补 workspace order index，Webview 分组树和平铺 root 分组渲染共用该索引；已通过 `npm run test:sidebar-node-list` 与 `npm run typecheck` 验证。

2026-07-01 补充：模板“重置为模板”在 multi-root 中已从“不支持”收口为 root-scoped reset。`rootGroups` 的 root section 或其内部用户分组提供目标 root；在所有 root section 外部触发 reset 时，Webview 提示用户到目标 root section 内重置且不发送 Host reset 消息；`paneGallery` 的可交互 pane 和 thumbnail 模式 active root 主画板继续由 pane 身份提供目标 root。Host 侧只清理目标 root 内的执行节点和待写入初始输入，并以空 root-local state 套用模板后写回 composed state，其他 root 与 overlay 不被清空。本轮补充执行 `npm run typecheck`、`npm run test:canvas-templates`、`npm run build && node scripts/test/run-playwright-webview.mjs -g "template reset"`，均通过。

2026-06-25 补充：sidebar 的 workspace folder / worktree 移除确认选定 VS Code 原生 modal 方案，而不是 QuickPick 或 sidebar Webview 自绘 modal。移除 folder 的默认动作是 `保留画板并移除`，另一个动作是 `清空画板并移除`；移除 worktree 的默认动作是 `移除 Worktree 并清空画板`，另一个动作是 `移除 Worktree 但保留画板`；worktree 路径会先执行 `git worktree remove`，只有 Git 成功后才按选择清空对应画板，Git 拒绝移除时画板保持不变。扩展通过 action item 传入顺序表达默认动作，并把 `取消` 标记为 `isCloseAffordance`；最终按钮视觉顺序和横排/竖排由 VS Code / OS 原生 modal 决定，不在扩展层强行重排。

截至 2026-06-13，本设计的 `rootGroups` 主路径已完成自动化验证：composition、protocol、group policy、execution context、template、Markdown drop、typecheck、build 与针对 workspace root group 的 Playwright 用例均通过。Add Folder root placement 本轮新增 `test:canvas-multi-root-composition` 覆盖新增 root 使用当前可见中心就近放置、避让已有 root 并写回 overlay，新增 Playwright 用例覆盖 `host/focusGroup` 通过缩放平移动画把 workspace root 分组居中并选中；root body 平铺签名水印补充 Playwright 用例覆盖仅 workspace-root 渲染、展示 root title、重复铺排、横向样式、常规字号与 root title 同步缩放、title tab 被分组宽度压缩时水印独立反向缩放、path-like 长标题简化和普通长标题换行、低透明和疏密度不可交互、配置关闭、不拦截 body 选择、普通分组背景不遮挡以及节点层仍覆盖水印。review 修复已追加覆盖 live 文件活动按 owner root namespace 记录 file reference、旧 unnamespaced reference 在 root scope 内迁移、suppression 往返保留，以及旧 multi-root live runtime skip 不覆盖 root-local reattach 信号。root 分组参与全局 fit view 与 MiniMap 的导航增强已在 `docs/exec-plans/completed/canvas-spatial-fit-minimap.md` 中完成并记录定向验证。2026-06-06 已把 multi-root live runtime 从“整体跳过”修订并实现为“按 root-local runtime metadata 共享恢复”：Host 不再以 `multi-root-workspace` 整体 block，runtime binding key 纳入 `runtimeBackend + runtimeStoragePath + runtimeSessionId + executionKind`，对缺失 `runtimeStoragePath` 的旧 root-local live-runtime snapshot 显式降级，并让 supervisor delete 先向订阅者广播非 live 终态，避免误连当前 multi-root workspace storage 或让其他窗口停留在假 live。2026-06-07 针对同一个 root 可能存在多个 VS Code `workspaceStorage` slot 的风险补充验证：单根 root-local snapshot 缺少 `runtimeStoragePath` 也降级，不再由当前同 root slot 回填；`test:smoke-storage-slot` 覆盖 root-local snapshot 优先、旧 sibling slot 恢复和缺字段降级；real reopen smoke 记录并校验 Agent / Terminal 重连前后的 slot name 不变。2026-06-25 sidebar worktree 流程新增添加已有 worktree 到 workspace 分支，并修复同一 git repository 的多个 workspace folders / linked worktrees 被重复当作多个 repository 的选择问题，已补 `npm run test:git-worktrees` 覆盖 `git worktree list --porcelain` 解析、repository 去重、添加已有分支入口和不执行 `git worktree add` 的路径，并复跑 `npm run typecheck`、`npm run test:extension-manifest`、`npm run test:sidebar-node-list`、`npm run test:sidebar-codicon-bundle` 与 `git diff --check`。2026-06-16 sidebar workspace folder 操作已补 `npm run test:extension-manifest`、`npm run test:sidebar-node-list`、`npm run test:sidebar-codicon-bundle`、`npm run test:sidebar-list-colors`、`npm run typecheck`、`npm run build` 与 `git diff --check`，覆盖 title action、workspace folder 分组 action、worktree 专用 Codicon、VS Code 风格 QuickPick ref 选择、workspace folder 前置 kind 图标与行尾操作顺序；真实 VS Code 中的 folder picker、`git worktree add`、添加已有 worktree、workspace folder 移除、worktree 移除和新增 root 聚焦仍需人工验证。真实 VSCode smoke 已补充 `multi-root-real-reopen`、`single-to-multi-root-real-reopen` 与 `two-window-shared-runtime` 三条路径，覆盖 multi-root 窗口重启恢复、单根创建后 multi-root 重启恢复、离线输出可见、重连后输入继续作用同一 session、不使用当前 multi-root workspace storage 误连，以及两个独立 VS Code 窗口同时 attach 同一 Agent/Terminal runtime 后的双向 output 多播、双向 input、resize last-writer-wins、stop/delete 共享 session 终态同步。全量 Webview Playwright 仍有与本功能无直接关系或 lifecycle 断言口径相关的失败，需要后续单独收口。
...
2026-06-17 补充：根据新的视觉与交互反馈，`paneGallery` 不再采用整条顶部 toolbar、filter roots 搜索框或 `tiled` / `focus` 二态命名，但保留右上角使用提示入口；正式局部模式改为 `dynamic`、`grid`、`topThumbnails`、`sideThumbnails`。动态/宫格模式下每个 root 子画板应尽量与当前主线单一画板一致，并且不额外限制缩放比例；顶部/右侧缩略图模式下主画板与单一画板一致，缩略图使用对应 root 画板 fit view 且不限制比例；缩略图双击才切换 active root。第二轮进一步取消右上角模式按钮，把模式入口移入左下角画板控制区；缩略图改为不可交互的真实 React Flow 画板而非 MiniMap/SVG 预览；paneGallery 不渲染 workspace-root 分组框/水印；root 标签轻量化并删除 nodes / waiting / attention subtitle；动态布局改为非等尺寸窗格铺满区域；per-pane overview 按各自 zoom 判定。第三轮明确左下角入口图标和粗切换语义：全览态 `eye` 点击进入 thumbnail，thumbnail 态 `globe` 点击返回全览；hover 始终展示四模式，动态/宫格/顶部缩略图/右侧缩略图用 `layout` / `table` / `split-vertical` / `split-horizontal`，菜单样式贴近画板控制按钮。第四轮补充粗切换记忆与 MiniMap 规则：全览转 thumbnail 使用上次顶部/右侧缩略图，thumbnail 返回全览使用上次动态/宫格，默认分别为右侧缩略图和动态；thumbnail 主画板显示右下角 MiniMap，动态/宫格子画板不显示 MiniMap。本轮已补定向 Playwright 回归覆盖无 toolbar/filter/右上角入口、左下角模式入口、粗切换记忆、thumbnail 主画板 MiniMap、动态/宫格不显示 MiniMap、画板化缩略图、缩略图单击无副作用、缩略图双击切换、many-roots 滚动、动态布局非等尺寸、per-pane overview 和 Markdown drop 目标 root。2026-06-18 追加收口 thumbnail rail 溢出对齐：未溢出时通过内层 track 自身 margin 居中，溢出时不允许居中到滚动容器负偏移之外，顶部/右侧 rail 均必须能从第一张缩略图滚到最后一张缩略图；实现不得依赖 VS Code 1.80 / Chromium 108 不支持的 `safe center` 行为。同日补充 viewport 记忆拆分：`dynamic` / `grid` 使用 overview viewport，thumbnail 主画板使用 main viewport；root 首次成为 thumbnail 主画板时自动 fit 并写入 main viewport，后续恢复 main viewport；切回 overview 时恢复 overview viewport，缩略图自身始终 fit 且不写入 viewport。2026-06-23 补充缩略图排序收口：paneGallery view model 明确按 runtime context 中的 workspace folder path 排序；thumbnail 模式只从该稳定序列中排除 active root，双击切换 active root 后 rail 中其他 root 仍按 workspace 顺序排列，避免形成“被双击缩略图与原主画板互换”的心智模型。同日曾补 pane 顶部细状态线；2026-06-24 根据缩略图相邻细线归属不清与视觉效果反馈，取消顶部细线，曾改为 root 标签 title 区域弱背景色按 `attention > running > idle` 聚合 execution 状态，并继续通过 pane 可访问名称 / thumbnail tooltip 暴露文本摘要；2026-07-01 该静态状态染色被取消，root 标签常态回到普通 chrome 背景和普通 border，状态摘要只保留在可访问名称 / thumbnail tooltip 与节点自身状态中。2026-06-24 补充 fit view 边界：paneGallery 各 root 的自动 fit 与手动 fit view 只使用当前 root 可见子图 bounds，不用不可见 workspace-root 外框补位；空 root 没有可 fit 内容时保持当前 viewport，不自动写入新的 root-local viewport，也不收紧 multi-root overlay。2026-07-01 补充 root 标签 attention 动画：paneGallery root 内存在待确认 attention 且增强提醒模式包含标题栏闪烁时，左上角 root 标签 title 区域复用执行节点标题栏闪烁动画，并在 reduced motion 下退化为静态高亮；动画取色使用 root 标签当前 border color，并通过更高占比背景混色、内外描边和轻量 halo 强化闪烁峰值，避免回退到等待输入色或原始状态色导致状态语义不一致，同时避免仅通过弱背景色区分通知状态。2026-07-03 补充 root 标签 running 活性色块：paneGallery root 内存在精确 `running` 执行节点且无 attention 时，左上角 root 标签 title 区域在文字下方显示 Codex Working 风格的 `|` 形竖向色块；色块使用 root 标签 border color 派生色，宽度为 16px、透明度为 0.82，在 root title 所在的小框区域内左右往返运动，不显示渐变、外发光或强 halo，不覆盖 root title 文字或原始 `border-bottom`，运动范围裁切在 root title 所在的小框区域内，不用于 `rootGroups`，也不由 `launching` / `live` / `starting` / `resuming` / `reattaching` 触发。2026-07-02 补充 thumbnail 模式 active root 占位：active root 不再从 rail 中移除，而是在原 workspace 顺序槽位渲染等尺寸斜纹占位卡片，文案为“正在主画板”；占位不显示 status dot，状态摘要只进入 `title` / `aria-label`，不承载单击 / 双击切换或任何画板写操作，避免同一 root 同时挂载两份真实 `ReactFlow`。文档仍保持 `validation_status: 验证中`：真实 VSCode 宿主下 terminal input、复杂跨 pane 拖拽和大量 root 可见区虚拟化性能尚未端到端验证。
