...
- 风险：真实 Remote SSH / 历史 VSCode storage slot 里，第一个 root 的单根 `workspaceStorage/<id>/meta.json` 可能不存在，或 `meta.name` 与 `workspaceFolders[0].name` 不一致；如果 fork 只依赖 root 名称匹配，会找不到源，然后首次打开 Canvas 会应用内置使用说明模板，看起来像初始化了全新画布。当前缓解：Untitled 多根 fork 仍先用 `meta.name` 精确匹配第一个 root；当没有精确匹配时，再从非内置使用说明模板、节点 cwd 明确落在第一个 root 下的非空快照中选择证据候选。若证据候选属于 `workspaceStorage/<slot>-N` indexed sibling，且其 `runtimeStoragePath` / `resumeStoragePath` 指向同一个单根 canonical slot，则实际 fork 源优先切回该 canonical slot；只有 canonical slot 缺失或没有可恢复快照时才使用证据候选本身。这条 fallback 通常只在当前多根槽和 `workspaceState` 都没有有意义节点、且 `workspaceFile.scheme === 'untitled'` 时启用；唯一例外是当前 Untitled 多根槽已经有上一次错误 fork 复制进来的有意义快照时，可先把当前槽仅作为证据读取 runtime/resume canonical 指向并切回 canonical slot，且同一错误快照已经进入 `workspaceState` 时不能让 workspaceState guard 预先阻断这次纠偏。若当前槽没有这种 canonical 指向，仍保留当前多根快照，避免覆盖用户已经形成的多根画布。
...
8. reload 路径没有可用的内存 Canvas，因此启动时需要做受限补救：如果 `workspace.workspaceFile?.scheme === 'untitled'`、当前 workspace folders 多于一个，宿主先扫描同一个 VSCode `User/workspaceStorage` 下其他 slot 的 `meta.json` 和本扩展 `canvas-state.json`，再结合当前 `workspaceState` 决定是否允许 fork。常规 fork 要求当前 `context.storageUri` 下没有有意义 `canvas-state.json`、当前 `workspaceState` 也没有有意义节点。源选择分两层：第一层优先选择与当前第一个 workspace root 名称匹配的单根 slot；若同名候选有多个，选择最新快照。第二层只在没有任何 root 名称精确匹配时启用：读取候选 `canvas-state.json` 中 Agent / Terminal metadata 的 `cwd`、`runtimeStoragePath`、`resumeStoragePath` 等目录线索，选择明确落在第一个 root 路径下的非空证据候选；若证据候选属于同一 `workspaceStorage` canonical slot family 的 indexed sibling，例如 `<slot>-2`，或证据候选快照里的 runtime / resume 路径指向某个 canonical slot，则实际 fork 源优先使用该 canonical slot 的非空主快照。这样可以避免 Remote SSH 场景中陈旧 indexed sibling 的 cwd 线索盖过当前单根 canonical slot。只有 canonical slot 缺失、为空或仅为内置模板时，才退回证据候选本身。内置 `builtin-getting-started` 使用说明模板只代表首次打开的空画布，不可作为 fork 源。选中后只把源 `canvas-state.json` 复制到当前 Untitled 多根 slot，作为 `A + B` 的初始 fork，并清理旧 `workspaceState` 的 Canvas 兜底字段。若 selection 表明当前槽是 copied evidence 且实际源是 canonical slot，则即使 `workspaceState` 已经有同一错误复制节点，也允许应用该 selection；其他有意义 `workspaceState` 仍阻断 fork。
9. reload 补救不得导入新增 root `B` 的历史 Canvas，不得读取或 merge 已有有意义 `A + B` 快照，也不得复制旧单根 `agent-runtime`、`runtime-supervisor` 或 Note 草稿目录。若当前多根 slot 已有空快照，可先保留 `.bak` 或记录诊断后复制，避免旧版本已经写入的空快照遮蔽原单根画布；若当前多根 slot 已有非内置使用说明模板节点，则视为有意义快照并跳过 fork；只有内置使用说明模板时仍可被第一 root 的单根快照覆盖。
...
- 存储 helper 单元测试：构造一个 Untitled 多根当前 slot、一个匹配第一个 root 名称的单根 slot、一个匹配新增 root 名称的单根 slot，断言启动期 fork 选择第一个 root 的快照；同时保留 unrelated workspaceStorage hash 不通过普通 freshness 规则恢复的断言。另构造 first-root cwd 线索只出现在 `<slot>-N` indexed sibling、以及 runtime 路径指回 canonical slot 的复制快照场景，断言实际 fork 源优先使用 canonical slot，并在 canonical slot 缺失时才退回 indexed sibling。
