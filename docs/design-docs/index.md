...
| `docs/design-docs/runtime-persistence-and-session-supervisor.md` | 将运行时持久化正式拆成 `snapshot-only` 与 `live-runtime` 两档，并为真实跨 VSCode 生命周期的进程存活引入会话监督器路线 | VSCode 集成域、协作对象域、执行编排域、项目状态域 / 宿主集成层、共享模型与编排层、适配与基础设施层 | 已选定 | 已验证 | `docs/product-specs/runtime-persistence-modes.md`、`docs/product-specs/canvas-core-collaboration-mvp.md`、`docs/exec-plans/completed/runtime-persistence-and-supervisor-design.md`、`docs/exec-plans/completed/remote-ssh-runtime-persistence-automation.md` | 2026-04-10 |
