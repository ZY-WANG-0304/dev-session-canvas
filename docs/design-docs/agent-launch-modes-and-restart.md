...
- 第二层：只对 Agent 打开，输入框显示完整命令，列表第一项是 `确认创建`，下方 `默认 / Resume / YOLO / 沙盒` 项是快捷替换器。

正式规则：

- Enter 默认命中第一项 `确认创建`，并始终按输入框当前值创建。
- 点击或键盘确认预设项只改写输入框，不直接创建；改写后焦点回到 `确认创建`。
- 如果用户显式点击了某个预设，而最终输入框内容在语义上仍等价于该预设生成的完整命令，则节点 metadata 里的 `launchPreset` 保留这次显式选择，而不是仅靠字符串反推后回落成 `default`。
- 第二层允许通过 Back 返回第一层。
- 第二层继续使用 `QuickPick`，但把 `确认创建` 固定为第一项，而不是把第一个预设放在列表顶部；原因是 VSCode `QuickPick` 会在输入过滤时自动激活第一个可选项，让自动激活落在确认项上可以把 Enter 的默认行为变成“使用当前输入创建”，同时避免标题栏 icon-only 按钮难以理解。
...
- 2026-05-11：命令面板 / 侧栏创建 Agent 的第二步 Quick Input 改为 `QuickPick` 首项 `确认创建` + 下方预设替换项，利用 VSCode 自动高亮第一项的行为承载确认动作，避免默认高亮第一个预设，也避免标题栏 icon-only 预设按钮不易理解；Enter 仍始终按当前输入框内容创建。
