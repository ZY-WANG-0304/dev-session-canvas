...
- 第一版优先使用原生 View 能力承载，而不是直接上 `WebviewView`。
- 侧栏动作收口为：
  - 打开或定位画布
  - 创建对象
  - 重置宿主状态
...
- 当前推荐把“创建对象”收口为一个稳定入口，再让用户选择 `Agent`、`Terminal`、`Task`、`Note`。
- 这样可以避免侧栏一打开就被四个常驻动作按钮占满，也更符合“极简侧栏”的目标。
- 是否需要保留个别高频对象的快捷入口，留待原型验证后再决定。

### 7.4 与现有正式文档的关系

- `docs/design-docs/canvas-feedback-polish.md` 中关于“顶角 panel 作为过渡态存在”的结论，当前仍可视为已实现现状。
- 一旦侧栏化路线完成原型和验证，应由本主题文档接管相关结论，并回头收口旧文档中的过渡态描述。

## 8. 验证方法

至少需要完成以下验证，当前设计才适合从“比较中”推进为“已选定”：

1. 做一个最小侧栏原型，确认原生 View 是否足以表达“打开画布 + 创建对象 + 重置状态 + 最小状态摘要”。
