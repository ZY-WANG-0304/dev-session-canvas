...
- `概览` 视图标题行尾部额外提供一个 gear 按钮，作为进入扩展设置的稳定捷径，不把设置入口挤进状态树项本身；视图正文则额外暴露 `通知模式`、`文件功能` 与 `文件视图` 状态，避免用户把“提醒行为变化”或“过滤入口消失”误解成渲染故障。环境配置行由 `extensions/vscode/dev-session-canvas/src/sidebar/CanvasSidebarView.ts` 读取 `CanvasSidebarState` 中的 terminal / Agent CLI 配置快照，并通过 TreeItem `command` 接到 `extensions/vscode/dev-session-canvas/src/extension.ts` 中的选择命令；Codex / Claude Code 行通过 `contextValue` 贡献 inline tail action，分别打开 Codex `auth.json`、Codex `config.toml` 和 Claude Code `settings.json`。Codex 路径先按 Agent 实际执行环境解析 `CODEX_HOME`，再回退到当前执行宿主 home 下的 `.codex`；Claude Code 继续使用当前执行宿主 home 下的 `.claude/settings.json`。文件缺失时先弹出 modal 确认提示，再用本地文件 API 创建带最小默认内容的文件并在编辑器中打开；POSIX 上配置目录和文件会 best-effort 收紧到 `0700` / `0600`，并用不覆盖已有文件的 `wx` 语义避免 race 覆盖。Agent CLI 候选发现逻辑收口在 `extensions/vscode/dev-session-canvas/src/panel/agentCliSelection.ts`，只负责帮助用户选择当前执行宿主上的命令或路径；当候选列表没有任何可解析路径时，同一模块提供安装元数据，`extensions/vscode/dev-session-canvas/src/extension.ts` 在选择 QuickPick 顶部展示安装入口。命令行安装不改变 Agent 节点的 PTY 启动模型，而是通过 `extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts` 创建普通 Terminal 节点，并在该终端 live session 启动后写入一次性安装命令。
- `CanvasSidebarState.creatableKinds` 现在只表达“哪些节点类型属于当前正式支持的创建入口”，不再承担 untrusted 之类运行时受限场景的隐藏语义。当前实现中它固定保留 `agent`、`terminal`、`note` 三类，使侧栏按钮、命令面板 Quick Input 与画布右键菜单在 trusted / untrusted 下都展示同一套入口；真正的限制改由宿主单独判断，并在用户点击受限项时弹 modal 解释原因。
...
4. 在 workspace 未受信任场景下，确认侧栏与 `创建节点` 命令仍展示执行型入口，但点击后会弹出明确说明原因的 modal，而不是把入口静默隐藏。
