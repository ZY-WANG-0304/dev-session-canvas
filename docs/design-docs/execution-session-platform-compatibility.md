...
- 2026-05-08/2026-05-09 已按 review 补齐 `Terminal` target、相对 `terminal.shellPath` probe、非 Windows PowerShell probe 与 Agent CLI cache workspace 隔离回归，并在复盘 VS Code 原生 Terminal 后改为混合方案：`node scripts/test-shell-environment-resolver.mjs` 现已覆盖 Windows `Terminal` 跳过 patch、POSIX `Terminal` 默认继承 patch、inheritEnv opt-out、POSIX login-only probe、相对 shell 路径复用 workspace `cwd`，以及 fake `pwsh` 走 PowerShell probe；`node scripts/test-terminal-shell-configuration.mjs` 已覆盖 `shellArgs` 规范化；`tests/vscode-smoke/extension-tests.cjs` 也已要求裸命令名 cache key 按 workspace `cwd` 隔离。同轮 `npm run typecheck`、`npm run build`、`npm run test:agent-cli-resolver`、`node -c tests/vscode-smoke/extension-tests.cjs` 与 `git diff --check` 均已通过。
