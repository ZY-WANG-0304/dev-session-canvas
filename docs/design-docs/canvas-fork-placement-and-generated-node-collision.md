...
- 所有系统生成节点的初始落位都复用同一矩形碰撞语义，使用真实已有尺寸和新节点的预期尺寸，并保留统一安全间距。
...
同层槽位步长按“新节点在排列轴上的尺寸 + 通用碰撞间距”计算：向上 / 向下使用 `target.width + NODE_PLACEMENT_PADDING`，向右使用 `target.height + NODE_PLACEMENT_PADDING`。候选顺序为中心、正向一格、负向一格、正向两格、负向两格，以保持来源节点附近的视觉重心。
...
- 每次由系统物化新节点时，在提交宿主权威状态前，用目标节点的最终初始尺寸和已有节点的当前持久化尺寸做矩形检测。
...
- 自动文件活动节点、Explorer 资源创建、普通创建、历史恢复 / 历史 Fork 和当前节点 Fork 都应进入同一碰撞语义；已存在的自动文件节点在刷新时保留原位置，不因本规则周期性跳动。
