...
- 默认值：`true`
- 作用域：`window`
- 关闭时：现有启发式与诊断层继续解析这些信号，但不额外发 VSCode 通知
- 打开时：`Agent` 与 `Terminal` 节点都会把命中的 attention signal 桥接为 VSCode 工作台通知

这里默认打开，是为了让执行节点里的 attention signal 在开箱即用时就能回到 VSCode 工作台；`BEL` 噪音则依靠信号优先级与冷却去重控制，用户仍可按需关闭。
...
- `src/panel/CanvasPanelManager.ts`
  - 负责把命中的 signal 桥接成 VSCode 工作台通知
  - 统一覆盖本地 PTY 与 runtime supervisor 输出
  - 是当前唯一通知 UI 落点

这意味着“状态启发式”和“用户提醒”共用同一份底层解析器，但仍属于两条独立支路。
...
- 但当 `OSC 9` payload 呈现为 `4;...` 进度形态时，通知桥接层不把它弹成 VSCode 通知
...
- 2026-04-21 已完成代码落地，并通过：
  - `npm run typecheck`
  - `npm run test:execution-attention-signals`
  - `npm run test:smoke`
- 当前文档保持 `验证中`，因为本轮尚未在真实 Ghostty / kitty / iTerm2 / tmux 场景里做手工协议验证；但仓库内已补 VSCode 宿主级 smoke，覆盖配置开关、冷却抑制与点击通知后定位节点。
