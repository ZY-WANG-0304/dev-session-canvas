...
5. `节点` view 标题栏提供添加 workspace folder 与 worktree 的全局按钮；多根 workspace 下全局 worktree 流程先按 git common dir 合并同一 repository，只有存在多个不同 repository 时才选择基准 repository。
