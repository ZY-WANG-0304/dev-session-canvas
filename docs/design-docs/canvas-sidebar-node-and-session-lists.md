...
8. workspace folder 行新建 worktree 成功后，新 worktree 目录被加入当前 workspace；选择添加已有 worktree 后，已有 worktree 目录被加入当前 workspace 且不会执行 git 创建命令；workspace folder 行移除 folder 使用 VS Code 原生 modal，可选择保留画板或先清空画板再移除，默认保留画板且磁盘目录不被删除；移除 worktree 使用 VS Code 原生 modal，可选择先清空画板再移除或保留画板，默认清空画板，确认后删除对应 git worktree 目录并从 workspace 移除该 folder。
