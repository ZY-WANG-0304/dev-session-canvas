...
手动验证在英文 VS Code 和简体中文 VS Code 环境中分别打开扩展。英文环境应看到英文 command title、设置说明、view title、已迁移 Webview 文案和 sidebar 文案；简体中文环境应看到中文对应文案。未迁移的模板市场 panel/Webview、模板保存表单和共享 presentation/status/helper 模块继续记录为后续工作，不能把它们描述为已完成本地化。
