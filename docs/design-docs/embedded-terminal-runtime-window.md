...
- `host/executionOutput.outputSequence` 与 `host/executionSnapshot.outputSequence` 是 Webview backlog reset 的顺序边界，不是用户可见内容。Host 每次投递 output 时递增序号；Webview 丢弃旧 raw backlog 后请求 snapshot，snapshot 序号之前的旧 output 不得再写入 xterm，snapshot 序号之后已经收到的新 output 只能在 snapshot hydrate 完成后重放。`webview/attachExecutionSession.requestId` 与 snapshot 回包的 `requestId` 用于确认 reset 请求是否完成；`executionSessionId` 用于区分同一节点的新旧执行会话，避免新会话低序号 output 被上一轮 reset 边界误判为 stale。reset 已清空或 session 已结束后，带 `snapshot-reset-*` request id 的迟到 snapshot 不应覆盖当前终端画面。
