...
- serialized terminal state 不是独立权威数据；它必须携带与 raw output 相同的 `outputSequence`，且只有序号与 snapshot 边界一致时，Host / runtime supervisor / Webview 才能把它作为恢复源。若历史 state 缺少序号，或序号落后于 raw output，则必须回放 raw output 重建 headless terminal state，而不是依赖 resize、visibility redraw 或 TUI 自发重绘来“补齐”画面。
