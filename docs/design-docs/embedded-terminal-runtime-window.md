...
- 当前 `script` 路线只在会话启动前按首帧节点尺寸设置终端行列；运行中不再把 resize 命令注入前台程序输入流。
...
- 风险：当前 `script` 路线缺少稳定的运行中 PTY resize 控制通道。
  当前缓解：节点创建后先等待 Webview 首帧 fit，再按该尺寸启动 shell；活跃会话期间不再通过 stdin 注入 `stty`，避免污染前台程序输入。
