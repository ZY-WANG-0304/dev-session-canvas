...
- 对 Agent / Terminal，Webview 本地 resize 草稿也不得逐帧改写 PTY 字符网格；手势期冻结现有 xterm `cols/rows`，结束后只提交最终稳定尺寸。普通布局抖动、纯位置拖动与本地 redraw 的详细边界见 `docs/design-docs/execution-terminal-resize-coalescing.md`。
