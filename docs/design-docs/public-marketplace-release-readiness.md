...
>
> 2026-09-16 lineage 决策：用户确认从 `0.24.5` 到 `0.25.0` 有意回滚了一系列变更；因此当前 `origin/main` 是本轮 `0.25.0` 的 release input，不重新接回 `v0.24.5` release line。发布记录需要把两者差异拆为“有意回滚”“当前保留”“PTY title 重新实现”，不能把 `v0.24.5` 独有的 Runtime Supervisor recovery 能力自动写成 `0.25.0` 能力。

> 当前 `origin/main` 已重新实现 PTY title：`7fc63a7e` 新增解析、Host / Supervisor 协议接入和 Webview 展示，`e04e295b` 补充输出顺序断言，最终由 `44c02505` 合入。该能力是 `0.25.0` 的主要用户可见内容，但真实 Extension Development Host、provider spinner、Webview reload 与跨生命周期 reattach 仍处于验证中，不因本轮发布完成自动升级为宿主级已验证。

> 当前 `origin/main` 可纳入候选范围的内容分为：PTY terminal title 展示 / `CSI 21 t` 查询 / 控制序列脱敏与旧 session 消息隔离；Pane Gallery 缩略图标题栏可见性与层叠修复；多根 workspace Marketplace 双语宣传素材；以及依赖审计告警收口。前三项需要按用户可见性和最终验证决定如何进入 release notes，Marketplace 素材属于 listing 资产更新，依赖升级属于维护与验证证据，不能混写成统一功能亮点。该范围已在最终 `main` release ref `4d7f07e55461f414c570365136cc06ece6f18c64` 上完成 `v0.25.0` 发布。

> 2026-09-16 目标版本确认与同步：用户确认本轮目标为 `0.25.0`；发布准备 MR `#291` 已合入 `main`，根 workspace、主扩展、notifier 和 lockfile 已统一同步到 `0.25.0`。正式 `v0.25.0` 与 GitHub Release 已创建，临时 `publish/v0.25.0` 已按完成门禁删除。

> 2026-09-16 发布后复核：Actions run `35040928600` 从 release ref `4d7f07e55461f414c570365136cc06ece6f18c64` 完成 GitHub Release assets + Open VSX 兜底发布。最终 manifest 为 `complete-with-deferred-visual-studio`，主扩展与 notifier 在 Open VSX 均为 `verified`，Visual Studio Marketplace 双扩展均为 `publish-failed` / deferred；正式 `v0.25.0` tag 与 Release assets 指向同一 release ref，`publish/v0.25.0` 已删除。主扩展 VSIX SHA-256 为 `88eeef48707f69f760b65713f7571841161d7eb4742057dfdf3fcf3117a6b811`，notifier VSIX SHA-256 为 `862e0b90cc9c70fca0ef0053e9a6fd2cbc5e69f208efecc11162576449ebc543`；两个工件的 README doc ref 均为最终 release ref。VSM public gallery 仍不得对外宣称为已可用。
...
>
> 当前发布准备目标选择 `0.24.2`。相对 `v0.24.1` 的输入只包含已合入 `main` 的 #261 Fork 定向展开与生成节点避碰、#262 Runtime Supervisor 跨 Node 终态门禁收口，以及 #263 无损 journal 安全 compact。选择 `0.24.2` 而不是新的 `0.25.0`，是因为三项变化继续收口 `0.24.x` 已建立的 Agent / Terminal 无损恢复与 Fork 工作流，没有改变扩展身份、稳定支持承诺、provider 命令契约、通知协议或服务 API 主版本；但它们同时包含用户可见能力和持久化机制变化，因此 release notes 必须完整说明，而不能包装成纯打包修复。`0.24.2` 继续保持 Preview 定位、主扩展 / notifier 同版本、最终 `main` ref 发布、GitHub Release assets + Open VSX verified 完成门禁和 VSM deferred 约束。
>
> 本轮 release notes 必须保留以下验证边界：checkpoint 无法证明安全、超过 256 KiB、producer profile 缺少兼容 fallback 或恢复代不可用时，会停止 compact 并继续增长 journal；当前无固定磁盘上限、完整长期 retention 策略或跨版本回退保证；90000 行 completed stream 已出现 `89861`、`89960`、`89877` 三个间歇性短读样本且未定位，#262 只修复固定等待与重复 serialize，不关闭该风险；Fork 的 panel / editor 人工视觉验收尚未完成，自动 File 节点真实初始宽度仍可能超过 220 x 84 估算；`trusted` / `real-reopen` 各有一次非内容性 smoke harness 时序误报并已登记技术债。发布准备分支的实际自动化、双 VSIX、packaged-payload 与 dry-run 证据在命令完成后补写；最终 publish / tag 前仍需在 MR 合入后的 clean `main` ref 上重跑完整 gate 与 `publish/v0.24.2` dry-run。
...
>
> 当前候选主扩展 / notifier VSIX 分别为 `3,925,963` / `159,017` bytes，SHA-256 分别为 `f57db4d29706c123ee8cb59e591c3bdca24a84830cb6f71c3768e05dc54e704c` / `3e9a56d38586195032baf4d9117f0fea0923fe44006d29d375896aab0c609163`，dirty-tree README ref 为 `fc427c6ed38c97c25f183e95216d35b19b1bc311`。不带 skip 的 clean-checkout 已通过隔离 `npm ci`、0-vulnerability audit、`117`-file 主扩展打包与 VS Code `1.128.0` packaged-payload smoke；Node `22.23.1` 隔离临时 ref `3e5dc0417efe12a14cb8cc27bff90a32c07da242` 也通过 `publish/v0.24.2` package-only dry-run。候选工件与临时 ref 不作为最终 Release assets；发布准备 MR 合入后仍需在 clean `main` release ref 上重跑完整 gate。

> 2026-07-14 `0.24.2` 发布后验证补充：Actions run [`29309330723`](https://github.com/ZY-WANG-0304/dev-session-canvas/actions/runs/29309330723) 已从最终 `main` release ref `c1e13b754d6a1f7be85d14b5d908967d464e1c6a` 成功完成发布。正式 `v0.24.2` 指向同一 ref，临时 `publish/v0.24.2` 已删除；最终 manifest 为 `complete-with-deferred-visual-studio`，两个 artifact 的 `packagingDocRef` / `readmeDocRef` 均为最终 release ref。Open VSX 主扩展与 notifier 均为 `version=0.24.2`、`verified=true`；Visual Studio Marketplace 双扩展记录为 `publish-failed` / deferred，public gallery 对两个 extension id 的独立查询仍为 `count=0` / `TotalCount=0`。
>
> 正式主扩展 / notifier / manifest assets 分别为 `3,925,962` / `159,016` / `3,104` bytes，SHA-256 分别为 `8e5c125987b51eb815ac80bd138583f178100528653f8558ece0919b51673ac5`、`304f1917ffd17f4096b7391f62a3f02508de50c26315ff573e785b5fa5ba7bba` 与 `3b05f15c8a8dbb3923cc28bb8386324c66d7b338836adb5aabfcc208caec3c75`。最终 `main` ref 已通过 package-only dry-run、clean-checkout `npm ci`、0-vulnerability audit、`117`-file VSIX 与 VS Code `1.128.0` packaged-payload smoke。本次成功 run 对仍以 Node.js 20 为目标的官方 Actions 发出强制改用 Node.js 24 的 deprecation annotation；该 annotation 未改变发布成功事实，后续升级与完整 workflow 回归已登记到技术债。

> 2026-07-15 `0.24.3` 发布准备决策：相对 `v0.24.2` 的输入只包含已合入 `main` 的 #266 execution 媒体文件链接原生打开修复，以及 #267 execution 节点 resize 合并与终端重绘收口。选择 `0.24.3` 符合 `docs/workflows/VERSION.md` 的同里程碑 bugfix 规则：两项变化修复 `0.24.x` 执行面行为，没有改变扩展身份、稳定支持承诺、provider 命令契约、runtime journal 格式、通知协议或服务 API 主版本。`0.24.3` 继续保持 Preview 定位、主扩展 / notifier 同版本、最终 `main` ref 发布、GitHub Release assets + Open VSX verified 完成门禁和 VSM deferred 约束。
>
> 本轮 release notes 必须保留以下验证边界：PNG 已有真实 VS Code Host smoke，但 GIF / MP4 尚无独立 fixture；`vscode.open` resolve 只表示 editor service 受理，不保证 model 最终加载；resize 合并已有 Webview 回归与 trusted Host smoke，但尚未在真实 Codex / Claude TUI 上人工复核 journal；跨节点、跨 Pane Gallery surface 或多节点并发触控不受支持；一次真实最终尺寸变化仍会触发 provider 正常重排。`0.24.2` 的 unsafe / oversized journal 增长、无固定磁盘上限、90000 行间歇性短读、Fork 视觉验收与自动 File footprint 精度边界均继续保留。发布准备分支的实际自动化、双 VSIX、packaged-payload 与 dry-run 证据见下方验证补充；最终 publish / tag 前仍需在 MR 合入后的 clean `main` ref 上重跑完整 gate 与 `publish/v0.24.3` dry-run。

> 2026-07-15 验证补充：`release-0-24-3-prep` 已完成版本号、双 changelog、Marketplace / README、发布手册与正式设计发布记录同步。Node `25.6.0` / npm `11.8.0` 下，主扩展 / notifier build 与 typecheck、生产依赖 audit、发布 / manifest / VSIX / marketplace / workflow 守卫、媒体链接 helper、Webview protocol、output sequence、Runtime Supervisor protocol、notifier source、10-Agent 样本、trusted Host smoke、notifier companion smoke 与英文 / 简体中文 locale smoke 均通过。resize Webview 聚合首轮为 `19/21`：本轮相关 10 项全部通过，两项无关 harness timeout 隔离原样复跑为 `2/2 passed`，因此不把聚合首轮写成清洁通过。
>
> 当前候选主扩展 / notifier VSIX 分别为 `3,929,121` / `159,099` bytes，SHA-256 分别为 `5e2849e132449de7c110fd816e96ad7cf07ee092d7ffb22120c8c61df35f1089` / `f3df2a710e5e8c1feb773b7a63574d3096ada3311ef262ded9a9a1203c48907e`，文件数分别为 `117` / `14`，dirty-tree README ref 为 `2ddf0729b53993fcbe5120f2830a9b4f3a4a6885`。不带 skip 的 clean-checkout 已通过隔离 `npm ci`、`0 vulnerabilities` audit、`117`-file 主扩展打包与 VS Code `1.128.1` packaged-payload smoke；Node `22.22.2` 隔离临时 ref `12d686685046657db38f996ea577bac8926aead7` 在补齐 `npm ci` 后通过 `publish/v0.24.3` package-only dry-run。首次未安装依赖的 dry-run setup 因缺少 `jszip` 在发布断言前退出；临时 ref 与候选工件均不作为最终 release input。隔离安装暴露的 `@vscode/vsce@3.9.1 -> glob@11.1.0` deprecated warning 已登记技术债，当前 audit 未确认漏洞。2026-07-15 VSM public gallery 对主扩展 / notifier 的实时查询仍均为 `extensions_len=0`、`TotalCount=0`。发布准备 MR 合入后仍需在 clean `main` release ref 上重跑完整 gate 与不带绕过参数的 dry-run。
...
本节是本轮发布的正式范围记录。它描述最终 `main` release ref 的真实输入和相对 `v0.24.5` 的行为差异，不把未完成宿主验证写成已完成结论；版本同步后的 repo-local release gate、最终 release ref、渠道状态和 release-day dry-run 均已收口。

### 发布基线与版本定位

- `0.25.0` 的最终发布输入为 `main` release ref `4d7f07e55461f414c570365136cc06ece6f18c64`；发布准备最初以候选 ref `44c025056f3e63019b53f528951ea1d15ce1539d` 为基线，在 `release-0-25-0-prep` 分支完成收口。
- 上一公开 tag 是 `v0.24.5`（`a9e27873aa01c1d1f1e43b4303ff697ce618c8cf`），但它不是当前 `origin/main` 的祖先；共同祖先是 `7eb3864b855d85b7c18d0162b99d5896d13af4d6`。这是有意的版本演进 / 回滚策略，不是发布前要自动修复的历史分叉。
- 版本同步前根 workspace、主扩展和 notifier manifest 均为 `0.24.3`；发布准备分支已将它们和 lockfile 统一更新为 `0.25.0`。正式 `v0.25.0` 已指向最终 release ref，GitHub Release assets 与同一 ref 对齐，远端临时 `publish/v0.25.0` 已按完成门禁删除。

### 相对 v0.24.5 的有意回滚

当前主线没有纳入 v0.24.5 release line 的以下发布承诺：Supervisor 先监听并后台恢复、`recovering / ready` 状态与恢复进度通知；死亡 PTY 只读取有界 metadata、保留有界历史显示并要求用户显式 Resume；checkpoint 拒绝诊断与 bounded projection refresh；以及严格 FIFO / 单在途输入 RPC。当前代码分别体现为恢复前先 `loadRegistry()`、恢复时仍读取 journal candidates、协议没有 recovery / bounded checkpoint 类型、Host projection refresh 仍请求完整 snapshot，输入路径仍直接写 PTY / 发起 RPC。

因此，0.25.0 的 release notes 必须把这些内容作为“相对 v0.24.5 不再承诺的运行时边界”单独说明，而不能复用 0.24.5 的安全恢复、恢复期间新建会话不受影响、显式 Resume 或输入响应优化文案。`systemd-user` 的 `WorkingDirectory=` 专用序列化和 transport / readiness / execution-spawn 错误分层仍需单独复验，不能作为本轮已完成的宿主级结论。

v0.24.5 中的依赖声明修复没有原样保留在当前 manifest，但最终 release ref 的 clean install、`npm audit` 和 `npm audit --omit=dev` 均报告零漏洞。依赖部分按安全门禁和维护记录处理，不作为用户功能亮点。

### PTY title 重新实现

当前主线通过 `7fc63a7e` 和 `e04e295b` 引入并合入了新的 PTY title 实现（最终合并提交 `44c02505`）。`extensions/vscode/dev-session-canvas/src/common/executionTerminalTitle.ts` 提供共享解析和脱敏；Local Host / Runtime Supervisor 处理 OSC 0 / OSC 2，支持 `CSI 21 t` 查询并由 PTY owner 回写 `OSC l`；Webview 在 Agent / Terminal context row 展示动态 title，同时保留节点标题、Agent 命令副标题、Terminal shell 副标题和 workspace root。

title 控制序列和 payload 不进入终端可见输出、recent output、terminal stream、checkpoint 或 journal；title 只作为 live session 投影，会话结束清除；新 execution session 建立后，旧 session 的迟到 title / output / 终态消息不能覆盖新 session。当前 parser、Supervisor protocol、Webview Playwright 和 `node-pty` fixture 已有定向证据，但真实 Extension Development Host 手工设置 / 清空、真实 provider spinner、Webview reload 和跨 VS Code 生命周期 reattach 仍为验证中。

### 对外 release notes 的固定结构

0.25.0 的主扩展和 notifier CHANGELOG、Marketplace listing、根 README 与发布手册按以下结构保持一致：

1. 版本定位：公开 `Preview`、当前 release input、与 `v0.24.5` 的有意回滚关系；不要写成线性累积升级。
2. PTY title：动态展示、支持的控制序列、TUI 查询回包、输出脱敏、session 隔离和验证状态。
3. 运行时边界：明确不承诺的 v0.24.5 recovery / checkpoint / input 优化，以及 Linux / Remote SSH 的复验状态。
4. 保留能力与升级：现有 Agent / Terminal、journal、扩展 ID、notifier 自动安装和配置沿用规则；只有最终回归通过的行为才能列为已保留。
5. 渠道和限制：Marketplace / Open VSX / GitHub Release assets 实际状态、Preview 支持矩阵、已知风险、升级与回退建议。

### 0.25.0 发布说明矩阵（当前审计版）

| 范围 | 应该怎么写 | 对外分类 | 当前证据与缺口 |
| --- | --- | --- | --- |
| PTY title 重新实现 | Agent / Terminal 节点可展示 PTY 通过 OSC 0 / OSC 2 设置的动态标题；支持 `CSI 21 t` 查询并由实际 PTY owner 回写 `OSC l`；标题控制序列和 payload 不进入可见输出、recent output、stream、checkpoint 或 journal；新旧 execution session 保持隔离。 | 主要用户可见亮点 | `executionTerminalTitle.ts`、Host / Supervisor 接入和 Webview 定向测试已存在；真实 Extension Development Host、真实 provider spinner、Webview reload、跨生命周期 reattach 仍需补证或保留为已知限制。 |
| 相对 `v0.24.5` 的 Runtime Supervisor 回滚 | 明确 0.25.0 不延续 `v0.24.5` release line 中的后台恢复 / `recovering` 状态、恢复进度通知、死亡 PTY 有界恢复与显式 Resume-only、bounded projection / checkpoint 诊断、严格 FIFO 输入等承诺；不要把这些内容写成 0.25.0 能力。 | 必须说明的行为边界 | `runtimeSupervisorMain.ts`、`protocol.ts`、`runtimeSupervisorProtocol.ts`、`CanvasPanelManager.ts` 和已删除的对应测试/设计文档能证明当前边界；Linux / Remote SSH 的 systemd `WorkingDirectory` 与运行时失败分层仍需单独复验，不写成 0.25.0 已验证能力。 |
| 当前仍保留的基础能力 | 只在最终回归通过后说明 Agent / Terminal 生命周期、terminal stream、authority / revision、journal / checkpoint 基础机制、Fork、resize、multi-root、notifier 安装拓扑和既有配置沿用。 | 兼容性与无回归说明 | 最终 release ref 的 repo-local 分层 gate 和 packaged-payload smoke 已通过；这些能力不能因重新打包就全部算作 0.25.0 新增，真实宿主和跨平台缺口仍按已知限制表达。 |
| Pane Gallery 视觉修复 | 底部缩略图标题栏可见性、层叠或定位修复如果相对最终公开 `v0.24.5` 没有新的行为差异，不单列为 0.25.0 runtime 亮点；只在视觉回归或素材说明中记录。 | 非新增功能 | 当前主线包含相关修复，但 `v0.24.5` 侧已有对应输入，不能仅按 commit 新旧判断为本版本新增。 |
| 多根 workspace Marketplace 素材 | 说明 listing 素材已更新、包含双形态多根 workspace 展示；不暗示扩展运行时因此新增 multi-root 能力。 | 发布资产 | 素材和剧本位于 Marketplace media 相关提交；它们不改变 VSIX runtime 行为。 |
| 依赖审计与 lockfile | 记录为维护 / 安全门禁：当前 `npm audit` 与 `npm audit --omit=dev` 为零漏洞，不作为用户功能宣传。 | 维护与验证证据 | 最终 release ref 的本地审计、隔离 clean-checkout 的 `npm ci` 和发布 workflow 均通过；正式工件与 manifest 以 GitHub Release assets 为准。 |
| 升级、回退与渠道 | 主扩展和 notifier 同版本；继续保持 Preview、`extensionPack` / `extensionDependencies` 拓扑和无新增设置；升级重要会话前先停止/备份；不承诺 Preview journal 的跨版本回退兼容。Marketplace、Open VSX、GitHub Release assets 的状态按本轮实际结果表达。 | 安装与支持说明 | 发布手册、双 changelog、README 和 manifest 已同步到 0.25.0；Open VSX 主扩展 / notifier 为 verified，Visual Studio Marketplace 为 deferred，GitHub Release assets 已上传。 |

### 0.25.0 发布验证与发布后复核（2026-09-16）

版本同步后的 repo-local release gate 已通过：主扩展 / notifier typecheck、PTY title parser / protocol / Webview 定向测试、manifest / package / publish workflow 守卫、双扩展 build、notifier source / companion / locale smoke、PTY title Playwright 定向回归、两次 npm audit、生产模板市场 workflow 守卫和 packaged-payload smoke 均通过；PTY title Playwright 定向回归为 `1 passed`，两次 audit 均为 `0 vulnerabilities`。`npm run validate:clean-checkout:vsix -- --source working-tree` 还在隔离目录中完成独立 `npm ci`、117-file 主扩展 VSIX 打包和 packaged-payload smoke，并在完成后清理临时目录。

发布前 working-tree 候选工件仅用于分支验证，不是最终 Release assets：主扩展为 `3,933,659` bytes、`sha256=a1c23550467b002c3abfce1672f1586a6e1a195691eedd6a6d0ed2f78de488eb`；notifier 为 `159,279` bytes、`sha256=ac3f8fb59681fa081f52fd25907d8030797029f1578495fa3b8b375b175553d8`；README doc ref 为 `44c025056f3e63019b53f528951ea1d15ce1539d`。这些数据保留为发布前候选物料的历史证据，不能与下方最终 Release assets 的 SHA 混用。package-only dry-run 已在最终 release ref 的无历史 tag 冲突 clean clone 中通过。
...
- GitHub Release assets 是 marketplace 同批 VSIX 的镜像与兜底下载入口；`0.16.0` 起的当前完成门禁允许在 Visual Studio Marketplace public gallery 仍不可见时，仅以 GitHub Release assets + Open VSX verified 作为可完成状态；`0.25.0` 沿用该门禁。因为 workflow 会先创建正式 tag 以绑定 Release assets，单看 `vX.Y.Z` 存在不再足以证明整轮发布完成；必须结合最终 manifest、Open VSX verified 状态、Visual Studio Marketplace 是否 deferred，以及 `publish/vX.Y.Z` 是否已删除判断。同版本重跑必须复用并校验既有 Release assets，不能重新打包覆盖 VSIX，因为当前 VSIX 打包不承诺 byte-for-byte 可复现。Release notes 也必须随最终 manifest 更新，显式展示版本亮点、渠道状态、残余风险和发布证据，不能只保留通用安装模板。
...
- `vX.Y.Z` 是正式 lightweight release tag，也是 GitHub Release 绑定的 tag；workflow 会在首次打包后、marketplace 发布验证前创建或确认它，以便先上传 Release assets；后续同版本重跑会复用并校验该 Release 上的既有 VSIX / manifest。单看该 tag 存在不再足以证明整轮发布完成；当前完成条件是 Release assets 已上传、Open VSX 已发布并验证、Visual Studio Marketplace 状态已明确记录为 verified 或 deferred、最终 manifest 已更新，且 `publish/vX.Y.Z` 已删除；`0.25.0` 继续沿用该条件。
