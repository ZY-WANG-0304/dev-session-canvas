...
- `workspaceState` 额外记录“当前 workspace 是否已经完成首次模板初始化”。首次打开画布时，如果当前宿主状态为空且该标记尚未置位，则宿主会先应用当前默认模板，再把结果作为首次画布状态发送给 Webview。
- 显式执行“重置画布状态”为纯空画布时，会把首次初始化标记保留为已完成，避免下次打开又自动回填默认模板。
...
- 侧栏列表按“内置模板固定在前 + 用户模板按创建时间倒序”展示；每项显示：
  - 模板标题行按 `图标 / 模板名称 / 默认标签 / 行内动作` 排布，按钮始终和标题保持在同一行
  ...
  - 右侧轻量行内动作：追加到当前画布、重置当前画布为该模板、设为默认、导出；用户模板额外可删除
