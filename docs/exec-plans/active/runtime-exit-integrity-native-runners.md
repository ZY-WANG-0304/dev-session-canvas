...
- [ ] 主代理基于整理后的最新 HEAD 完成复审，确认允许推送和创建 PR。
- [ ] 推送前重新 fetch/rebase main，推送专用 runner 分支，创建 main PR，不合并。
- [ ] 取得三平台首次 Actions 工件，区分基线失败与基础设施错误，保留全部首次失败并记录实际版本。
...
当前本地诊断、自校验及独立 Linux 普通 Node 25/Electron-as-Node 22 复核通过，runner 增量已与重构隔离。尚未推送或运行托管 Actions，也没有托管普通 Node 22、macOS/Windows 通过证据，计划保持 active。完成本轮后仍不代表 source-drain 契约、VS Code/Host/Webview、真实 Agent、packaged 或全部支持平台通过；这些缺口必须保留为 repo-local 后续项。
...
最后一条预期为空。证据目录须不存在；换新目录重试，不能清空失败工件。获准后执行 `git fetch origin main`、`git rebase origin/main`、`git push -u origin runtime-exit-integrity-platform-runners-agent`，再用 `gh pr create --base main --head runtime-exit-integrity-platform-runners-agent` 提供中文背景、改动、验证和残余风险。用 `gh run list --workflow runtime-exit-integrity-native.yml --branch runtime-exit-integrity-platform-runners-agent` 查看运行；下载时以实际 run id 创建独立目录。

## 验证与验收

每平台固定自然 exit 0、自然 exit 7、12000 行编号输出、分片 UTF-8、收到开头后主动取消各三轮，共 15 个结果。完整场景须 writer receipt 成功、exit code 正确、xterm 内容与光标终点一致，Unix 还须 CRLF 规范化后逐字相同；取消仅记录取消和退出，不能归入完整。进程返回非零时也应保存可复查工件。事件记录的 socket close 或 onExit 不单独证明源已排空。
...
语法、自校验及 workflow YAML 结构检查通过，内容权限只读、矩阵 fail-fast 关闭、上传 `if: always()`、无 secret。以上不替代 GitHub 服务器语义检查。托管运行 URL、run/attempt、各平台版本与结果在实际执行后补入，不能预填成功。

## 接口与依赖

CLI 提供 `--self-test`，或 `--runs N --timeout-ms N --output DIR`。结果输出 `environment.json`、汇总 `summary.json` 和每场景每轮目录中的 `output.txt`、`rendered.txt`、`child-script.js`、`writer-receipt.json`（完整写场景）与 `summary.json`。不增加业务接口、协议或 package scripts；依赖保持 main 原样。
