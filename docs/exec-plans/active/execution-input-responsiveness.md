...
如果 bounded snapshot reset 生效，新的现场诊断中 `diagnosticsSchema.executionPerformance` 应为 8。一次 reset 应能看到 `snapshot-reset-requested`，随后看到 `snapshot-reset-applied`；如果 Host 或 bridge 卡住，最多先出现 `snapshot-reset-timeout` 并重试，而不是长期只有 `output-deferred-until-snapshot-reset`。等待 snapshot 期间 `pendingOutputLength` 不应再增长到 MB 级；超过 256KB 时应出现 `deferred-output-budget-reset` 并重新请求 snapshot。
