...
当前文档必须按 `docs/PLANS.md` 的要求持续维护。本计划实现已提交设计 `b3ef9cf docs(canvas): define Claude Code Agent Branch semantics` 中的 Claude Code Branch 方案，相关正式规格在 `docs/product-specs/agent-launch-modes-and-restart.md`，相关设计在 `docs/design-docs/agent-launch-modes-and-restart.md`。

## 目标与全局图景

完成后，用户在 DevSessionCanvas 里看到一个已经持有可信 Claude Code session id 的 Agent 节点时，可以点击标题栏里的 `Branch`。画布会新增一个 Agent 节点，标题弱提示它来自原节点，并立即启动 Claude Code，启动命令包含原 session id 和 `--fork-session`。原节点不停止、不改 metadata，用户可以继续在原节点或新节点里独立对话。

从代码路径看，Webview 只表达“对 nodeId 执行 Branch”的意图；Host 重新读取权威节点 metadata，确认 provider 是 `claude` 且 `resumeStrategy === 'claude-session-id'` 且 `resumeSessionId` 非空。Host 然后构造 Branch 命令，创建新 Agent 节点，依赖已有 `pendingLaunch: 'start'` 自动启动机制让新节点进入执行路径。
...
`scripts/test/test-agent-launch-presets.mjs` 是命令层回归测试。新增断言覆盖 Branch 命令生成、默认参数目标剥离、空 session id 报错、命令格式化保留可用默认参数。
...
`scripts/test/test-protocol-webview-messages.mts` 是协议解析回归。新增有效 Branch 消息和无效 Branch 消息断言。

`src/webview/main.tsx` 负责 Agent 节点标题栏按钮和 postMessage。新增 `onBranchAgentSession` 回调和 Agent 节点内 `Branch` 按钮。按钮只在 provider 为 `claude` 且 `canResumeAgentFromMetadataForWebview(agentMetadata)` 为真时显示；点击只发送 nodeId，不发送 session id。

`tests/playwright/webview-harness.spec.mjs` 负责 Webview 行为回归。新增测试确认 Claude resumable 节点显示 `Branch` 并 post `webview/branchAgentSession`，Codex resumable 节点不显示 `Branch`，Claude 不可恢复节点不显示 `Branch`。
...
- 决策：Branch 消息 payload 只携带 `nodeId`，不携带 provider 或 session id。
  理由：Webview 是非权威 UI，session id 必须从 Host 当前状态重新读取；这样能避免伪造消息用任意 session id 启动 CLI。
  日期/作者：2026-06-06 / Claude

- 决策：Branch 第一版的新节点使用 `agentLaunchPreset: 'custom'` 和 `agentCustomLaunchCommand`，复用现有 fresh-start 启动链路。
  理由：Branch 命令是一次明确的 provider-specific 启动命令，现有 custom preset 已经能持久化完整命令并走统一校验、resolver、pendingLaunch 自动启动流程。
  日期/作者：2026-06-06 / Claude
...
尚未实现。完成后记录实际改动、验证命令输出、未覆盖的手动场景和后续技术债。

## 上下文与定向

当前 Agent 启动/恢复能力已经分三层：共享命令层在 `src/common/agentLaunchPresets.ts`，Webview 消息协议在 `src/common/protocol.ts`，Host 执行编排在 `src/panel/CanvasPanelManager.ts`。已有历史恢复入口 `CanvasPanelManager.restoreAgentSessionFromHistory()` 会调用 `buildHistoryResumeCommandLine()`，然后创建一个 custom Agent 节点；Branch 要复用这个结构，但命令必须是 Claude Code 的 fork 语义。

`AgentNodeMetadata` 在 `src/common/protocol.ts` 中定义，包含 `provider`、`launchPreset`、`customLaunchCommand`、`resumeStrategy`、`resumeSessionId` 等字段。Host 端 `canResumeAgentFromMetadata()` 在 `src/panel/CanvasPanelManager.ts` 文件底部附近，Webview 端 `canResumeAgentFromMetadataForWebview()` 在 `src/webview/main.tsx` 文件底部附近。Branch 的可信条件必须是 provider 为 `claude`，resume strategy 为 `claude-session-id`，session id 非空；不能对 Codex 的 `codex-session-id` 启用。

`applyCreateNode()` 在 `src/panel/CanvasPanelManager.ts` 中负责创建节点。创建 Agent 成功后，它会把新节点置为 `starting` 并写 `pendingLaunch: 'start'`。这意味着 Host 的 Branch 方法只需要创建新节点并 post state；Webview 收到新节点后会自动启动。为了让启动发生，Branch 新节点必须使用有效 custom command，并且 workspace 必须 trusted。
...
### Task 1: Add Claude Branch command builder
...
- [ ] **Step 3: Implement minimal command builder**
  ...
  Do not change `buildAgentResumeArgv()` yet. Branch is a separate path, not a new launch preset.
...
- [ ] **Step 5: Commit Task 1**
  ...
  Run:
      ...
      feat(canvas): build Claude Agent Branch command

      Add a dedicated command builder for Claude Code Branch so fork-session launches cannot be confused with ordinary resume commands.
...
- [ ] **Step 6: Commit Task 2**
  ...
  Run:
      ...
      git commit -m "$(cat <<'EOF'
      feat(canvas): add Agent Branch webview protocol

      Let Agent nodes request a Branch action while keeping provider and session identity checks on the host side.

      Co-Authored-By: Claude Opus 4.7 <noreply@anthropic.com>
      EOF
      )"

### Task 3: Add Webview Branch button
...
- [ ] **Step 2: Run targeted Webview tests and verify failure**
  ...
  Run:
      ...
      npm run test:webview -- --grep "Agent Branch"

  Expected result: FAIL because no Branch button exists.
...
- [ ] **Step 4: Add Agent Branch button rendering**
...
- [ ] **Step 5: Run targeted Webview tests**
  ...
  Run:
      ...
      npm run test:webview -- --grep "Agent Branch|agent restart actions"
...
- [ ] **Step 6: Commit Task 3**
  ...
  Run:
      ...
      git commit -m "$(cat <<'EOF'
      feat(canvas): show Claude Agent Branch action

      Add a Claude-only Branch button for nodes with a trusted resumable session and route the intent to the host.

      Co-Authored-By: Claude Opus 4.7 <noreply@anthropic.com>
      EOF
      )"

### Task 4: Implement Host Branch behavior
...
- [ ] **Step 7: Add Host Branch command builder wrapper**
...
- [ ] **Step 1: Update design validation status section**
  ...
  In `docs/design-docs/agent-launch-modes-and-restart.md`, append a new line in `## 9. 当前验证状态` after the 2026-06-06 design note:
      ...
      - 2026-06-06：已实现 Claude Code Agent `Branch`：Webview 只发送当前 node id，Host 重新校验 Claude provider 与可信 session id 后创建 custom Agent 节点，并用 `--resume <session-id> --fork-session` 启动；已补充命令层、协议、Webview 与 Host smoke 回归。
...
- [ ] **Step 4: Manual verification in Extension Development Host**
  ...
  Start the extension development host using the repository's normal VS Code debug configuration or ask the user to run it if this session cannot launch the UI. Verify:
      ...
      2. Confirm a `Branch` button appears in the Agent title bar.
      3. Click `Branch`.
      4. Confirm a new Agent node appears near the source node with a `Branch` title suffix.
...
- [ ] **Step 6: Commit Task 5**
  ...
  Run:
      ...
      git commit -m "$(cat <<'EOF'
      docs(canvas): record Claude Agent Branch verification

      Capture the implemented Branch behavior, verification commands, and remaining manual validation notes.
...
3. A resumable Claude Agent node renders `Branch`; a Codex node or non-resumable Claude node does not.
4. Clicking `Branch` posts only `{ nodeId }` to Host.
5. Host Branch from a trusted Claude node with `resumeStrategy: 'claude-session-id'` and a non-empty `resumeSessionId` creates exactly one new Claude Agent node with custom launch command containing `--resume <source id> --fork-session`.
6. Host rejects Branch for Codex, missing node, missing trusted Claude session id, or untrusted workspace without creating a node.
7. Source node is unchanged after Branch.
