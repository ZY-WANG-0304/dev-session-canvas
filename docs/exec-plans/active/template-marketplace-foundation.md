...
当前仓库是一款 VSCode workspace extension 的 monorepo，仓库根目录是 private npm workspace root；主扩展包位于 `extensions/vscode/dev-session-canvas/`，其中 `package.json` 是主扩展 manifest，扩展入口是 `extensions/vscode/dev-session-canvas/src/extension.ts`，Webview 入口是 `extensions/vscode/dev-session-canvas/src/webview/main.tsx`，本地模板模型位于 `extensions/vscode/dev-session-canvas/src/common/canvasTemplates.ts` 和 `extensions/vscode/dev-session-canvas/src/panel/CanvasTemplateStore.ts`。模板市场设计已经确认新增两个主要落点：`packages/marketplace-shared/` 作为纯 TypeScript 合约包，`apps/template-marketplace/` 作为浏览器市场和 Cloudflare Worker API 的应用包。
