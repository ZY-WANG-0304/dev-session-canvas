...
- [x] (2026-06-23 22:28Z) 已梳理现有文本粘贴链路：`extensions/vscode/dev-session-canvas/src/webview/executionTerminalNativeInteractions.ts` 拦截平台粘贴快捷键，`extensions/vscode/dev-session-canvas/src/webview/main.tsx` 发起 `webview/requestExecutionPaste` 并等待 Host 回包，`extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts` 用 `vscode.env.clipboard.readText()` 读取文本并执行多行安全处理。
...
- 观察：VS Code Extension Host 官方剪贴板 API 当前只在仓库里通过 `vscode.env.clipboard.readText()` / `writeText()` 用于文本；如果要拿到截图字节，必须在 Webview 侧处理浏览器剪贴板事件或 `navigator.clipboard.read()`，不能只改 Host 侧现有文本粘贴 handler。
  证据：仓库当前只有 `extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts` 使用 `vscode.env.clipboard.readText()`，没有图片剪贴板 API 调用。
...
- 观察：当前 xterm key handler 会在识别粘贴快捷键后 `preventDefault()` 并返回 `false`，这会让浏览器默认 paste 事件不可靠；因此实现不能只依赖 `paste` 事件，还需要在快捷键分支中主动尝试 `navigator.clipboard.read()`，失败或无图片时再走原文本粘贴请求。
  证据：`extensions/vscode/dev-session-canvas/src/webview/executionTerminalNativeInteractions.ts` 中现有 `action === 'paste'` 分支先 `event.preventDefault()` / `event.stopPropagation()`，再调用 `onRequestPaste(...)`。
...
当前执行节点由 Webview 和 Host 两侧组成。Webview 的 `extensions/vscode/dev-session-canvas/src/webview/main.tsx` 创建 `@xterm/xterm` 终端实例，并把输入通过 `webview/executionInput` 发给 Host。`extensions/vscode/dev-session-canvas/src/webview/executionTerminalNativeInteractions.ts` 负责 xterm 里的原生交互，包括链接、拖拽、复制、文本粘贴快捷键和剪贴板诊断。Host 的 `extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts` 解析 Webview 消息，管理 Agent / Terminal 运行时会话，并通过 `vscode.env.clipboard` 处理文本剪贴板。
...
- `extensions/vscode/dev-session-canvas/src/common/protocol.ts`：新增图片粘贴消息类型和 validator。
- `extensions/vscode/dev-session-canvas/src/common/executionTerminalClipboard.ts`：新增图片粘贴 MIME、base64、文件名和路径引用 helper。
- `extensions/vscode/dev-session-canvas/src/webview/executionTerminalNativeInteractions.ts`：新增图片剪贴板读取和 paste 事件处理。
- `extensions/vscode/dev-session-canvas/src/webview/main.tsx`：新增图片 paste request 发送，并复用 pending paste response 路由。
- `extensions/vscode/dev-session-canvas/src/panel/CanvasPanelManager.ts`：处理图片粘贴消息、保存图片文件、返回路径文本，并调度临时截图缓存后台维护。
- `extensions/vscode/dev-session-canvas/src/panel/executionImagePasteCacheMaintenance.ts`：独立执行临时截图缓存 TTL 判断、分片扫描与删除预算。
