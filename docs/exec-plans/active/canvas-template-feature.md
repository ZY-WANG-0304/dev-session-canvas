...
- [x] (2026-05-06 18:05 +0800) 实现模板领域模型、3 个内置模板资源、用户模板目录扫描、默认模板持久化和首次打开自动应用逻辑。
...
第一，第一次打开新 workspace 的画布时，会直接出现默认“使用说明”模板，而不是空白画布；如果用户已经把别的模板设为默认，首次打开和“重置为默认模板”都应改用新默认模板。
...
第四，模板侧栏可以看到 3 个内置模板和新增的用户模板，能执行应用、设为默认、导出、删除等动作；删除当前默认用户模板后，会自动回退到“使用说明”。
