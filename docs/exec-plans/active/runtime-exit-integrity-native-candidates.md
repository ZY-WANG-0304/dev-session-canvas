...
- [ ] 推送独立输入并完整执行18项，下载首次工件、同步主线结论及剩余Windows/长驻资源边界。
...
当前增量里程碑：先实现设计第15节的新诊断与只读helper，记录编译来源和独占读取权的转移，完成所有权/假EOF/错误分账负对照及本地完整9项；然后专用workflow两平台完整18项，下载复算并以首次结果回答取消前提是否成立和owned bytes是否完整。完成本阶段不关闭生产退出完整性或长驻资源债务。
