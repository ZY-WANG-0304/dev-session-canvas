...
- [ ] 实现新 Unix 原生尾部/在途取消诊断、Windows 实际 bridge 启动链与 DLL 对照及独立 workflow，完成自校验和本地 Linux 全量。
...
新阶段已冻结但尚未执行；目标是补原生尾部、在途取消的证明边界与 Windows 启动链证据，不再重复注入模型。各阶段实际结果在完成后补录，不提前登记平台通过。
