...
- [ ] 推送隔离输入，完成三平台新 84 项并下载复核所有工件；记录首次失败、同步运行时主线，Windows 在途取消及长驻资源仍开放。
...
新阶段已完成实现及 Linux 三版各 21 项，每版 15 次自然 EIO、6 次明确中断并独立记录 64/1984 bytes 分割。Windows 本地仅自校验，不冒称 ConPTY。完整结果及取证加固来源见设计第 10–11 节；即将推送独立输入执行三平台 84 项，不提前登记 runner 通过。旧 294 项失败及断言不改。
