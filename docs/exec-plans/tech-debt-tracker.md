...
| 2026-04-06 | Webview Playwright harness 当前不运行在真实 VS Code Webview 容器里 | 为了尽快把 Webview UI 纳入自动化回归，本轮让 Playwright 直接加载真实 `dist/webview.js` bundle，并通过 stub `acquireVsCodeApi()` 驱动。这条路线已经足够覆盖 DOM、交互和截图，但它仍不是 VS Code 的真实 Webview 容器。 | Webview UI 对真实容器特性的依赖，例如 CSP、容器级样式差异、真实宿主注入环境与少量可访问性行为 | 当前保留一层 `@vscode/test-electron` smoke test 负责宿主级闭环；Playwright 明确只承担 Webview 专项 UI 回归，不把它写成完整宿主替代。 | 下一轮如果要覆盖更深的 Webview 容器差异或发布前做 UI 风险收口时 | `tests/playwright/harness/webview-harness.html`、`tests/playwright/webview-harness.spec.mjs`、`scripts/run-playwright-webview.mjs`、`docs/design-docs/development-debug-automation.md`、`docs/exec-plans/completed/extension-debug-automation.md` |
