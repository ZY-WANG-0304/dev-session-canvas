...
| 2026-04-05 | DevSessionCanvas 命名空间迁移后仍保留旧 view/container/publisher 标识 | 本轮已把命令、配置与 workspace 状态键迁移到 `devSessionCanvas.*`，但 VS Code 的 view/container/webview panel 标识与扩展 `publisher` 直接关系到视图恢复和扩展身份，缺少低风险别名机制。 | Activity Bar 容器、Panel 视图恢复、扩展身份和后续更彻底的命名统一 | 当前保留 `opencove` 相关 view/container ID 与 `publisher` 不变，并在 README 与设计文档中显式说明这是兼容边界，而非漏改。 | 下一轮如果需要彻底收口扩展身份时 | `package.json`、`src/common/extensionIdentity.ts`、`docs/design-docs/dev-session-canvas-namespace-migration.md`、`docs/exec-plans/completed/dev-session-canvas-namespace-migration.md` |
