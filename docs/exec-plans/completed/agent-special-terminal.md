...
把当前画布里的 `Agent` 从“输入 prompt，宿主执行一次 CLI，再把 stdout 聚合成 transcript”的模型，改成“画布里的特殊终端窗口”。用户创建 `Agent` 节点后，应看到一个真正的嵌入式会话窗口；它默认停留在未运行态，允许先切换 provider，再显式启动 `codex` 或 `claude` CLI，后续交互直接在终端内完成，而不是继续依赖节点外的调用模型。

完成后，用户应能亲眼验证以下行为：新建 `Agent` 节点后可以先切换 provider 再启动对应 CLI；节点内可以像终端一样收发输入输出；停止、重启、切换 provider 都遵循“会话窗口”语义；宿主状态里只保留 provider、最近输出、退出信息和会话摘要，而不再维护独立 transcript 状态机。
...
- [x] (2026-03-30 01:45+08:00) 根据 MR `!4` 复审结果取消 Agent 创建后自动启动，保留 provider 切换窗口，并补齐旧 placeholder 迁移与文档收口。
...
- 决策：新建 `Agent` 节点默认不自动启动，用户应先有机会切换 provider，再显式启动会话。
  理由：默认 provider 仍可能与本机可用 CLI 不一致；如果创建即自动运行，会在 claude-only 等环境里引入确定性首次失败回归。
...
这里的“默认启动行为”指默认启动命令和 provider 语义，而不是“创建即自动运行”；节点创建后仍先停留在可切换 provider 的未运行态。
