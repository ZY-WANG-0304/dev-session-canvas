...
- `npm run release:publish-tag -- --trigger-tag publish/v0.17.0 --dry-run --package-only --skip-origin-main-check`（发布准备 head 尚未合入 `origin/main` 时仅用于本地预演；最终 `main` release ref 必须不带该参数复跑）
- `npm run typecheck`
