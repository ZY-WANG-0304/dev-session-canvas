...
本文用于收口当前公开 `Marketplace Preview` 版本的发布素材、发布前复核、安装/升级说明、验证记录、发布命令与回退口径；当前发布准备目标为 `0.24.5`，上一已发布版本为 `0.24.4`。本轮范围是“把 Runtime Supervisor restart 与 Window Reload 分离：新 Supervisor 以空 namespace 启动，死亡 Agent 只保留显式 Resume identity、死亡 Terminal 只使用已有 durable history 与 Restart；同一存活 Supervisor 的 Window Reload 按 surface / 节点分块恢复完整 retained projection；输入 FIFO 在固定 socket dispatch 后推进，并把 control lifecycle 与 bulk terminal stream 隔离”。它不是对外宣传页，而是 release-day 执行与复核手册。
...
- 当前更新依据：`docs/design-docs/runtime-control-and-projection-isolation.md`、`docs/design-docs/agent-terminal-lossless-io-and-recovery.md`、`docs/design-docs/runtime-supervisor-recovery-readiness.md`、`docs/design-docs/execution-lifecycle-and-recovery.md`、`docs/exec-plans/completed/runtime-recovery-projection-isolation.md`、`docs/product-specs/runtime-persistence-modes.md`、`docs/UI.md`、`docs/FRONTEND.md`；`docs/exec-plans/completed/runtime-supervisor-dead-pty-bounded-recovery.md` 与 `docs/exec-plans/completed/terminal-checkpoint-input-responsiveness.md` 只记录历史实现，其中 namespace hydrate、全局 hydrate 通知与 response-gated FIFO 已被当前设计取代
...
本轮 listing 必须明确：`0.24.5` 是 `0.24.x` Preview 线内的修复更新，不是稳定版承诺。Supervisor instance 改变时，新进程使用空 namespace，不扫描或打开旧 registry/journal，也不发布整画布 hydrate 通知；死亡 Agent 只有在保留可信 provider session id 且用户点击 `Resume` 后才创建新 PTY，死亡 Terminal 提供已有 durable history（若存在）与 `Restart`，screen snapshot / recent output 只是可选提示。Window Reload 若仍连接同一健康 Supervisor，则每个 `(surface,node)` 独立通过有界 bulk stream 恢复完整 retained projection，收到 chunk 即显示，选中节点优先且后台节点有界推进。节点内输入保持 FIFO，但前一项写入固定 socket 后即可派发下一项；每条 response 独立观察，Supervisor 先写 PTY、再返回 response、最后发布 compact lifecycle，bulk 不驱动共享 lifecycle。升级窗口中旧 Supervisor 真正持有的 live session 可与当前 generation 并行 drain。Codex 风格 OSC 10/11 query 与 current / previous 安全 compact 继续保留；完整 journal 浏览、固定磁盘上限、跨版本回退、物理重启 / 长时间 Remote SSH 断开验证、90,000 行短读与 VSM 可见性均不得误写为已完成承诺；模板市场仍是 `Preview` 能力，生产服务部署版本与插件 SemVer 分离。
...
- 顶部版本标题为 `0.24.5 - Runtime Recovery and Responsive Terminal Input`
- 当前已包含实际版本差异、安装/升级说明与回退建议
- release notes 应覆盖以下当前已确认范围：新 Supervisor 空 namespace、不扫描旧 registry/journal、无整画布 hydrate 通知；Agent 以可信 resume id 进入 `resume-ready` 并显式 `Resume` 新 PTY；Terminal 使用已有 durable history 与 `Restart`；同 instance Window Reload 按 surface / 节点分块恢复完整 retained projection；输入 FIFO 在固定 socket dispatch 后释放、每条 response 独立完成；response-before-lifecycle、bulk 不驱动共享 lifecycle；旧 Supervisor live session 并行 drain；OSC 10/11 query 的 checkpoint 资格和主扩展 / notifier 版本对齐
- 已知边界必须保留 screen snapshot / recent output 只是可选帮助、Host 级 fault-model smoke 未点击真实 Resume/Restart 且不等于物理重启或长时间 Remote SSH 断开、旧 PTY 不提供跨重启存活保证、完整 journal 尚无通用分页浏览、unsafe / oversized journal 仍可增长、90,000 行短读、Fork、媒体 opener、resize 与 VSM deferred；不得用定向通过样本覆盖残余风险
...
5. 停止后的 Agent 只有在保留可信 provider session id 时才提供 `Resume / 恢复`；该动作以 provider resume identity 创建新 PTY，`New / 新建` 则启动新 provider 会话。Terminal 的 `Restart / 重启` 始终启动新的 shell 进程。当前 Agent 恢复 / 分叉仍只继承节点自身最近一次实际启动命令或长期启动偏好，不合并当前 Default args。
6. Supervisor instance 改变时，新 Supervisor 从空 namespace 启动，不扫描或打开旧 registry/journal，也不发整画布 hydrate 通知。死亡 Agent 停在显式 `resume-ready`；死亡 Terminal 提供已有 durable history（若存在）与 `Restart`。screen snapshot 和 recent output 只是可选上下文，缺失不影响 Resume identity。
7. Window Reload 若仍连接同一个健康 Supervisor，则每个 surface / 节点独立请求完整 retained projection，以有界 chunk 边接收边显示；选中节点优先，后台节点有界推进，一个 restoring 节点不阻塞其他 ready 节点、Note 或创建操作。普通输入只在该节点 projection ready 后开放。
8. 节点内输入保持 FIFO，但前一项写入固定 socket generation 后即释放下一项，不等待前一条 response；每条 response 仍独立观察、诊断和报错。Supervisor 先写 PTY、再返回 compact response、最后发布 lifecycle；bulk projection 只承载 terminal stream event，不驱动共享 lifecycle。
9. 升级窗口中，仍由旧 Supervisor 真正持有的 live session 通过原 runtime 继续 output、input、resize、stop 与 delete，当前 generation 可并行服务新会话。runtime persistence 只在 `devSessionCanvas.runtimePersistence.enabled` 与后端能力成立时生效；合格 checkpoint 可通过 current / previous generation 安全 compact 完整 prefix，不合格时保留所需 journal。local PTY 不获得跨 instance 存活承诺，journal 也不承诺固定磁盘上限、完整长期 retention 策略、通用完整历史浏览或跨版本回退兼容。
10. `devSessionCanvas.canvas.forkPlacementDirection` 默认 `up`，也可设为 `down` 或 `right`；设置热生效于后续当前节点 Fork，不重排既有节点 / 连线，历史会话 Fork 继续使用通用邻近避碰。
11. 模板市场生产入口默认为 `https://dscanvas.dev/templates`；生产环境不会把代码内 seed 模板暴露为正式内容，初始空目录属于当前受控状态，不代表扩展安装失败。
12. 若用户此前显式配置过 `devSessionCanvas.runtimePersistence.enabled`、`devSessionCanvas.notifications.attentionSignalBridge`、`devSessionCanvas.notifications.enabledAttentionSignals`、`devSessionCanvas.notifications.strongTerminalAttentionReminder`、`devSessionCanvas.notifications.agentAbnormalOutputTextNotifications`、`devSessionCanvas.canvas.linkOpenMode`、`devSessionCanvas.canvas.workspaceRootWatermarks.enabled`、`devSessionCanvas.canvas.multiRootPresentationMode` 或 `devSessionCanvas.canvas.forkPlacementDirection`，升级到 `0.24.5` 后会继续沿用该明确选择；未配置 `enabledAttentionSignals` 时继续使用默认 allow-list，未配置 `multiRootPresentationMode` 时继续使用默认 `rootGroups`。
13. 当前仍为 `Preview`，不承诺跨版本 workspace / runtime journal 状态完全兼容；若涉及关键工作区，建议升级前先停止重要会话、备份画布状态，并在非关键环境验证。
...
1. Supervisor 重启后 Agent 显示 `Resume` 且点击前没有 provider 进程、Terminal 显示已有 history 与 `Restart`、同时没有整画布 hydrate 通知的对照
2. Window Reload 时选中节点优先、多个节点逐 chunk 出现 retained history，其他 ready 节点、Note 与新建操作仍可用的对照
3. 健康 Agent 快速输入保持顺序，response 先于 compact lifecycle，bulk 恢复不阻塞 control 的诊断对照
4. 旧 Supervisor live session 与当前 generation 新会话并行 drain 的升级对照
5. Codex 风格 OSC 10/11 query 后 checkpoint 前进、实际颜色修改仍被保守拒绝的诊断对照
...
`0.24.5` 的发布输入在 `v0.24.4` 之后继续修正 #276--#278 的恢复与输入路径：保留可信 Agent resume identity、显式 Resume、OSC query checkpoint 资格和输入保序意图；取代旧 namespace hydrate、全局 hydrate 通知与 response-gated FIFO，并加入同 instance Window Reload 的 surface-local bulk projection、completed history archive/ref 和 control/lifecycle 隔离。它符合 `docs/workflows/VERSION.md` 的同里程碑 bugfix 规则：本轮修复既有恢复和输入响应行为，不改变扩展身份、provider 命令契约、服务 API 主版本或稳定支持承诺。
...
- release notes 同步：主扩展 `0.24.5` 顶部条目说明 Supervisor restart 空 namespace、显式 Resume/Restart、同 instance Window Reload 分块投影、input dispatch/response 边界、control/bulk lifecycle 隔离和旧 Supervisor live drain；notifier changelog 仅对齐版本
- Marketplace / README 文案同步：主扩展 Marketplace 中英文 listing 与仓库中英文 README 已更新为 `0.24.5` 发布准备口径，并保留 optional screen/recent output、物理重启 / 长断开、通用 journal 浏览、90,000 行短读、Fork、媒体 opener、resize 与 VSM deferred 边界
- 发布手册与设计结论同步：`docs/public-preview-release-playbook.md`、`docs/design-docs/runtime-control-and-projection-isolation.md`、`docs/design-docs/agent-terminal-lossless-io-and-recovery.md` 与本轮 completed ExecPlan 已更新当前 runtime 语义；更早的 completed plans 已标明 superseded 边界
...
残余风险基线：`0.24.4` 的最终 Release 将 Visual Studio Marketplace 记录为 deferred；本轮不能把该渠道写成已恢复。`0.24.5` 的 Supervisor-restart smoke 只验证 Host 启动、Agent 显式 Resume action 与 Terminal history 状态，没有点击真实 Resume/Restart 按钮；旧 local PTY 不承诺跨 instance 存活，screen/recent output 不作保证，真实物理重启与长时间 Remote SSH 断开仍不是已完成的验收。完整 journal 的显式、分页、有容量预算查看尚未实现；unsafe / oversized journal 增长、无固定磁盘上限、90,000 行间歇性短读、双 surface GUI、Host/Webview event-loop lag、Fork 视觉验收、自动 File footprint 精度、媒体 opener 和 resize 的既有边界也均未关闭。最终验证必须如实区分定向测试、真实 Host smoke、packaged-payload smoke 与未执行门禁。
...
6. 定向复核 `0.24.5` 用户可见主路径：新 Supervisor 启动不扫描旧 registry/journal、不显示整画布 hydrate 通知且可立即创建会话；具备可信 provider session identity 的 Agent 显示 `Resume` 且点击前不启动 provider 进程，Terminal 提供已有 durable history 与 `Restart`；同 instance Window Reload 按节点逐 chunk 显示完整 retained projection；健康节点快速输入严格保序且 response 先于 lifecycle；旧 Supervisor live session 可并行 drain。与此同时确认 `0.24.4` 的恢复 transport 错误归因与 `0.24.3` 的媒体链接、resize、journal compact、Fork 定向落位与终态门禁主路径未回归。
