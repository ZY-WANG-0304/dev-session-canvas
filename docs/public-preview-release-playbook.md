...
本文用于收口当前公开 `Marketplace Preview` 版本的发布素材、手工发布步骤、安装/升级说明与回退口径；当前目标版本为 `0.2.2`。当前仓库内先完成版本与文案预留，具体修复项仍待版本范围冻结后补齐。它不是对外宣传页，而是发布当天的执行与复核手册。
...
当前 `0.2.2` 的 release notes 统一以 `CHANGELOG.md` 为准。当前仓库中的 `0.2.2` 条目仍是发布准备草稿；发布前只允许做事实性修订，不应再引入与版本范围无关的新能力描述。

发布前应确认以下内容在 `CHANGELOG.md` 中保持一致：

- 顶部版本标题与 `CHANGELOG.md` 保持一致；当前草稿标题为 `0.2.2 - Preview Hotfix Update (Draft)`
- 当前已包含安装/升级说明与回退建议；实际 hotfix 列表应在版本范围冻结后补齐
