...
本文用于收口当前公开 `Marketplace Preview` 版本的发布素材、发布前复核、安装/升级说明、验证记录、发布命令与回退口径；当前发布准备目标为 `0.24.4`，上一已发布版本为 `0.24.3`。本轮范围是“修复 Runtime Supervisor 重启后旧 journal 恢复阻塞新会话与错误归因失真，修复 Pane Gallery 底部缩略图标题栏，并同步已审阅的中英文 multi-root Marketplace 素材”。它不是对外宣传页，而是 release-day 执行与复核手册。
...
本轮 listing 必须明确：`0.24.4` 是 `0.24.x` Preview 线内的修复更新，不是稳定版承诺；Supervisor 会先提供可连接的控制面并报告 `recovering`，旧历史在后台 hydrate，新会话不因该过程被全局阻塞；重启前已终止的 PTY 不会被复活，恢复的 non-live snapshot 只作为历史显示；socket/readiness 错误与 PTY spawn 错误必须保持区分。Pane Gallery 底部缩略图标题栏已修复，新的中英文媒体只展示已存在的 Root Groups / Pane Gallery 能力，不能误包装成产品新增模式。`0.24.3` 已登记的 journal、90,000 行短读、Fork、媒体 opener 与 resize 验证边界继续保持；模板市场仍是 `Preview` 能力，生产服务部署版本与插件 SemVer 分离。
...
- release notes 应覆盖以下当前已确认范围：Supervisor 先 listen 后异步恢复、`recovering` / `ready` phase、恢复期对 persisted attach 的重试、旧 PTY 不复活、新会话可创建、transport/readiness 与 spawn 错误归因、systemd `WorkingDirectory=` 修复、Pane Gallery 底部标题栏修复、中英文素材刷新，以及主扩展 / notifier 版本对齐
...
`0.24.4` 的发布输入严格限于 `v0.24.3` 之后已合入 `main` 的 #270 Pane Gallery 底部缩略图标题栏修复、#271 中英文多根目录 Marketplace 素材制作，以及 #272 Runtime Supervisor 重启恢复与错误归因修复。它符合 `docs/workflows/VERSION.md` 的同里程碑 bugfix 规则：本轮修复既有恢复、呈现与发布素材，不改变扩展身份、provider 命令契约、journal 格式、通知协议或服务 API 主版本。
...
- release notes 同步：主扩展与 notifier changelog 新增 `0.24.4` 顶部条目，说明 recovery phase、错误归因、old-PTY 边界、Pane Gallery 标题栏和 companion 仅对齐版本
...
本轮在 Node `25.6.0` / npm `11.8.0` 上已通过 `git diff --check`、extension manifest、publish-tag、双 marketplace、workflow、VSIX command / file-list 守卫，主扩展与 notifier typecheck、Runtime Supervisor protocol、Runtime Host backend、terminal journal、UI copy / localization、notifier source，以及 `npm audit --omit=dev`（`0 vulnerabilities`）。`runtime-supervisor-reboot-recovery`、`systemd-user-real-reopen` 与 `systemd-fallback-real-reopen` 真实 VS Code smoke 均通过；notifier companion 与英文 / 简体中文 locale smoke 也均通过。

当前准备分支候选主扩展为 `dev-session-canvas-0.24.4.vsix`（`117` files，`3,933,884 bytes`，`sha256=2f42104db70c5f5ce9c23aefd2625c80ab3313cbbe222b5bd31a2cdeb004c1d8`），notifier 为 `dev-session-canvas-notifier-0.24.4.vsix`（`14` files，`159,454 bytes`，`sha256=10104b203a36f38388291d76a5cf7fe55edae28e319d2b2dcf36557ccf960403`）。dirty-tree 打包日志的 `VSCE README doc ref` 为 `57eecf51c771990116b0f49913de27384d8d1731`；`npm run validate:clean-checkout:vsix -- --source working-tree` 已通过隔离安装、打包和 packaged-payload smoke。候选工件、README ref 与其 checksum 仅是准备分支证据，不能作为最终 Release assets；MR 合入后仍须在 clean `main` release ref 上重跑双扩展打包、clean-checkout 与 `publish/v0.24.4` dry-run。
