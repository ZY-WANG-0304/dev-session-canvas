...
本文用于收口当前公开 `Marketplace Preview` 版本的发布素材、发布前复核、安装/升级说明、验证记录、发布命令与回退口径；当前目标版本为 `0.23.0`。当前版本范围收口为“相对已发布 `0.22.0` 的新里程碑更新：Dev Session Canvas Notifier 英文默认版与简体中文本地化、真实 VS Code notifier locale smoke，以及 VS Code smoke 宿主稳定性”。它不是对外宣传页，而是发布当天的执行与复核手册。
...
- 当前更新依据：`docs/design-docs/ui-copy-localization.md`、`docs/design-docs/notifier-companion-architecture.md`、`docs/design-docs/canvas-multi-root-workspace-support.md`、`docs/design-docs/agent-launch-modes-and-restart.md`、`docs/design-docs/agent-running-state-detection.md`、`docs/UI.md`、`docs/FRONTEND.md`
...
本轮 listing 必须明确：`0.23.0` 是新的公开 `Preview` 里程碑，不是稳定版承诺；notifier companion 的产品自有 UI 文案默认英文并提供简体中文本地化，但 backend 名称、路径、配置片段、诊断事实、用户环境数据、终端输出、Agent/provider 原始输出、模板 README/CHANGELOG 和市场数据保持原样；主扩展继续保留 `0.22.0` 已发布的主画布本地化和当前 Agent 启动参数边界；模板市场仍是 `Preview` 能力，生产目录可能在真实模板发布前为空；生产服务部署版本与插件 SemVer 分离，不把 `deploy/template-marketplace/prod/*` 当作插件版本。

## release notes 定稿口径

当前 `0.23.0` 的 release notes 统一以 `extensions/vscode/dev-session-canvas/CHANGELOG.md` 为准；发布前只允许做事实性修订，不应再引入与版本范围无关的新能力描述。docs-only 变化不进入用户可见更新说明，除非它改变安装、升级、回退或发布判断。

发布前应确认以下内容在 `extensions/vscode/dev-session-canvas/CHANGELOG.md` 中保持一致：

- 顶部版本标题为 `0.23.0 - Notifier Localization and Smoke Stability Update`
- 当前已包含实际版本差异、安装/升级说明与回退建议
- release notes 应覆盖以下当前已确认范围：`Dev Session Canvas Notifier` 英文默认版与简体中文本地化、真实 VS Code notifier locale smoke、notifier VSIX 本地化资源打包守卫、两条真实 development extension 的 companion smoke、Linux headless smoke 稳定性参数、terminal viewport marker-line 断言稳定性，以及主扩展 / notifier 版本对齐
- 安装/升级与回退口径需要继续与 `README.marketplace.md` 保持一致
- 不把 `Preview`、notifier 本地化覆盖范围、模板市场、生产服务、当前 Agent 分叉可靠性或 Visual Studio Marketplace 可见性误写成稳定正式版承诺

## 安装与升级说明口径

当前对外统一使用以下安装与升级说明：

1. 当前目标版本为 `0.23.0`，扩展身份保持 `devsessioncanvas.dev-session-canvas`；`0.1.0` 仍是首个公开 `Preview` 基线版本。
2. 首次安装与从 `0.22.0` 升级到 `0.23.0` 的目标仍是通过当前宿主配置的公开扩展市场常规安装 / 升级完成。Open VSX 侧应继续同版本公开发布；官方 VS Code 的 `Visual Studio Marketplace` 仍是目标主路径，但当前 public gallery 仍不可见时允许延期补发，不阻塞本轮 `0.23.0` 以 GitHub Release assets + Open VSX verified 完成。对外宣称 VSM 安装路径前仍必须先完成 release-day visibility check，确认主扩展与 notifier 均公开可见。
...
若 `0.23.0` 对当前工作流形成 blocker，当前统一建议是：

1. 先禁用或卸载当前扩展，避免继续影响当前 workspace。
2. 关注后续更高的 `0.23.x` hotfix；当前默认优先通过修复版升级解决，而不是承诺平滑降级兼容。
...
1. 优先评估能否在短时间内发布后续更高的 `0.23.x` hotfix。
...
当前 `0.23.0` 发布不以额外截图为 blocker。当前已经具备：
...
1. 英文 / 简体中文 locale 下的 notifier sidebar、view title 与手动测试通知代表性文案对照
2. notifier 诊断输出与真实系统通知弹出 / 点击回跳平台状态
3. 主扩展 attention bridge 调用 notifier 后回到画布居中的端到端链路
...
7. 确认发布 workflow 会先把同一批 VSIX 镜像到 GitHub Release assets，并以 Open VSX 主扩展 / notifier 发布验证作为本轮完成门禁；Visual Studio Marketplace 仍会尝试发布 / 验证并写入 manifest，但当前允许 deferred，不阻塞 `0.23.0` 完成。仓库 Actions 必须具备 `contents: write` 权限，且 `GITHUB_TOKEN` 可创建 / 更新 Release、上传 assets、创建正式 tag 与按完成门禁删除临时 tag。
...
截至 `2026-07-07`，上一轮 `0.22.0` 已从最终 `main` release ref `b56fce2c74b2aefe383c1cb03fdfea522079b3cc` 完成 GitHub Release assets + Open VSX 兜底发布；正式 `v0.22.0` tag 指向同一 ref，远端 `publish/v0.22.0` 临时 tag 已由 workflow 删除。GitHub Release `v0.22.0` 位于 `https://github.com/ZY-WANG-0304/dev-session-canvas/releases/tag/v0.22.0`，包含 `dev-session-canvas-0.22.0.vsix`、`dev-session-canvas-notifier-0.22.0.vsix` 与 `release-manifest-0.22.0.json`。Release manifest 记录 Open VSX 主扩展与 notifier `0.22.0` 均 verified，Visual Studio Marketplace 主扩展与 notifier 均为 `publish-failed` / deferred；因此 VSM 继续作为 deferred channel，不得对外宣称为已可用。2026-07-07 发布准备复核时，GitHub Release latest 为 `v0.22.0`，Open VSX 主扩展与 notifier latest 均为 `0.22.0`，Visual Studio Marketplace item 页面两个 extension id 仍返回 404，public gallery `extensionquery` 返回空列表。

`0.23.0` 当前发布准备输入来自 `v0.22.0` 之后合入 `origin/main` 的功能、修复与文档 / 测试工作：#247 技术债清理流程规则、#248 live-runtime scrollback smoke 稳定性、#249 外部控制面设计边界，以及 #250 notifier 英文默认 / 简体中文本地化与真实宿主 locale smoke。本轮用户可见 release notes 聚焦 #250 和 smoke 稳定性；docs-only 变化不作为用户功能宣传。本轮发布准备分支只处理版本号、CHANGELOG、Marketplace / README 文案、发布手册和发布验证，不把未合并的功能分支 head 直接当作发布输入。最终 release ref 以发布准备 MR 合入 `main` 后的 commit 为准。

本轮发布准备分支已完成以下 repo-local 同步：

- 版本号同步：`package.json`、主扩展 manifest、notifier manifest、`package-lock.json` 根版本、主扩展 package entry 与 notifier package entry 均更新为 `0.23.0`
- release notes 同步：`extensions/vscode/dev-session-canvas/CHANGELOG.md` 与 notifier changelog 已新增 `0.23.0` 顶部条目，并明确 notifier 本轮新增本地化资源但不改变通知协议、后端选择或点击回跳语义
- Marketplace / README 文案同步：`extensions/vscode/dev-session-canvas/README.marketplace.md`、`extensions/vscode/dev-session-canvas/README.marketplace.zh-CN.md`、`README.md`、`README.zh-CN.md` 已更新为 `0.23.0` 发布准备口径，并保留 VSM deferred 约束
...
截至 `2026-07-07`，`release-0-23-0-prep` 已完成以下 repo-local 验证：

- 外部事实复核：GitHub Release latest 为 `v0.22.0`，Open VSX 主扩展与 notifier latest 均为 `0.22.0`，`release-manifest-0.22.0.json` 记录 Open VSX 双扩展 verified、Visual Studio Marketplace 双扩展 `publish-failed` / deferred，且 VSM public gallery 对两个 extension id 仍返回空列表
- 版本 / 发布脚本守卫：自定义 Node 版本与文本一致性检查、`npm run test:extension-manifest`、`npm run test:package-vsix-file-list`、`npm run test:publish-tag-release`、`npm run test:publish-marketplaces`、`npm run test:publish-marketplace-workflow`
- 构建 / 类型 / source 测试：`npm run build`、`npm run build:notifier`、`npm run typecheck`、`npm run typecheck:notifier`、`npm run test:notifier-source`、`npm audit --omit=dev`（`found 0 vulnerabilities`）
- 打包 / smoke：`npm run package:vsix`、`npm run -w extensions/vscode/dev-session-canvas-notifier package:vsix`、`npm run test:notifier-smoke`、`npm run test:notifier-locale-smoke`、`npm run test:vsix-smoke` 均通过；`test:notifier-locale-smoke` 覆盖英文与简体中文真实 VS Code 宿主，`test:vsix-smoke` 完成 packaged-payload smoke
- clean checkout：`npm run validate:clean-checkout:vsix -- --source working-tree --skip-vsix-smoke` 通过，可在隔离目录中基于当前 working tree 快照执行 `npm ci` 并重新生成主扩展 VSIX；该步骤显式跳过 clean-checkout 内的 VSIX smoke，因为本分支已单独运行当前工作区的 packaged-payload smoke
- 格式守卫：`git diff --check` 已在验证过程中通过；最终提交前仍需再次执行，避免后续文档补写引入空白问题

当前分支产物仅是发布准备验证证据，不是最终 Release assets：

| 产物 | 文件数 | 大小 | sha256 |
| --- | ---: | ---: | --- |
| `dev-session-canvas-0.23.0.vsix` | `117` | `3.66 MB` / `3,833,233 bytes` | `77ad1a207d259e9f8382701eeb502944ef00b8408f512a8a02a215ef6d84884e` |
| `extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.23.0.vsix` | `14` | `154.97 KB` / `158,689 bytes` | `5aa861f57d55b81712bfe11da57df2da6e4f54477f96a7e52c64f5e788b633b7` |

主扩展与 notifier 打包日志在发布准备 dirty worktree 中使用的 `VSCE README doc ref` 为 `b7149132b37c11f6a92912ed0cd24f11924f781f`，只能证明当前分支内 README 资源改写路径可解析；发布准备 MR 合入后，最终 `main` release ref 必须重新执行 clean-checkout VSIX、主扩展 / notifier 打包、packaged-payload smoke 与 `publish/v0.23.0` dry-run，并以最终 manifest 中的 `readmeDocRef` / `sha256` 作为正式 Release 证据。

残余风险：Visual Studio Marketplace public gallery 当前仍不可见，manifest / Release notes 必须继续把 VSM 写成 deferred / `publish-failed`；不得把官方 VS Code Marketplace 安装路径宣称为已可用。Notifier 本地化已有 source tests 与真实宿主 locale smoke 覆盖，但不等于所有平台真实 OS 通知弹出与点击回跳均已在本轮人工逐项验收；真实 GitHub OAuth template publish smoke 仍是发布前 / 升级验证状态前 follow-up；当前 clean-checkout 验证只做到主扩展打包且跳过 clean-checkout 内 VSIX smoke，最终发布日仍需在合并后的 clean `main` ref 上补齐完整 gate；生产服务与插件发布版本线分离，不能把服务 deploy tag 写成插件版本，也不能把插件 `v0.23.0` tag 当成当前线上服务版本。
...
推送 `publish/v0.23.0` 会触发 `.github/workflows/publish-marketplace-release.yml`。该 workflow checkout 临时 tag 指向的 commit，先执行：
...
随后 workflow 会创建或确认正式 `v0.23.0` tag 指向同一 release ref，创建或更新 `v0.23.0` 对应的 GitHub Release，并上传以下 Release assets：

    release-artifacts/release-manifest-0.23.0.json
    dev-session-canvas-0.23.0.vsix
    extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.23.0.vsix

Release assets 上传后，workflow 会继续复用同一批 manifest / VSIX 分别执行 Open VSX 与 Visual Studio Marketplace 发布、验证。如果同一 `publish/v0.23.0` 因任一 marketplace 失败而重跑，workflow 会先下载并校验 `v0.23.0` GitHub Release 中已有的 manifest / VSIX assets，不会重新打包或覆盖 VSIX；若既有 Release 缺少任一必需 asset，则直接失败并要求人工修复不完整状态。
...
注意：`--skip-package` 不再只检查 VSIX 文件存在；它要求已有 `release-artifacts/release-manifest-0.23.0.json`，并校验当前 VSIX sha256 与 manifest 一致，避免复用不属于本次 release ref 的旧包。

若最终版本号不是 `0.23.0`，统一替换 tag、manifest 文件名、release notes 与验证记录中的版本号。
...
`vX.Y.Z` 是正式 lightweight release tag，也是 GitHub Release 绑定的 tag。为了让 Release assets 在 marketplace 暂时不可用时仍能先提供手动安装兜底，workflow 会在打包后、marketplace 发布验证前创建或确认该 tag。单看 `vX.Y.Z` 存在不再足以判断整轮发布完成；`0.23.0` 当前完成条件是 GitHub Release assets 已上传、Open VSX 已发布并验证、Visual Studio Marketplace 已记录为 verified 或 deferred、最终 manifest 已更新，且 `publish/vX.Y.Z` 已删除。如果 Open VSX 发布或验证失败，`vX.Y.Z` 与 Release assets 可能已经存在，但 workflow 会失败并保留临时 tag。需要人工删除临时 tag 时，应先确认两个 tag 指向同一 commit、Release assets 已存在，且 Open VSX 发布验证已经完成：

    git push origin :refs/tags/publish/v0.23.0
    git tag -d publish/v0.23.0
...
1. 确认对应版本的 lightweight tag 已存在于远端，且指向本次实际发布的 commit；若 Open VSX 已验证完成，确认远端 `publish/v0.23.0` 已删除。
...
3. 打开 GitHub Release 页面，确认 `dev-session-canvas-0.23.0.vsix`、`dev-session-canvas-notifier-0.23.0.vsix` 与 `release-manifest-0.23.0.json` 都存在于 Assets 中。
...
6. 定向复核 `0.23.0` 用户可见主路径：主扩展激活、主画布打开、英文 / 简体中文 locale 下的 notifier manifest / sidebar / 手动测试通知代表性文案、主扩展 attention bridge 到 notifier callback 再回到画布居中的链路，以及模板市场打开 / 空目录读取。
7. 复核生产服务状态时使用 `/api/v1/meta`、deploy tag、Cloudflare deployment id 和 production smoke 证据；不要把插件 `v0.23.0` tag 当成服务当前运行版本。
