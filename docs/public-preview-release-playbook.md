...
本文用于收口当前公开 `Marketplace Preview` 版本的发布素材、手工发布步骤、安装/升级说明、验证记录与回退口径；当前目标版本为 `0.14.1`。当前版本范围收口为“发布 `0.14.x` Preview 补丁：Explorer Markdown 文件右键创建关联 Note、创建类入口复用当前已打开的 Canvas surface、分组 body 空白区拖动画板，以及 multi-root / 双窗口 shared live runtime 恢复验证与硬化；继续保留 `0.14.0` 的空间级 fit view、初始自动 fit、动态最小缩放、右下角 MiniMap、workspace-root section 可见性、双市场发布边界、安装拓扑和 Preview 支持边界”。它不是对外宣传页，而是发布当天的执行与复核手册。
...
当前 `0.14.1` 的 release notes 统一以 `CHANGELOG.md` 为准；发布前只允许做事实性修订，不应再引入与版本范围无关的新能力描述。docs-only 变化不进入用户可见更新说明。

发布前应确认以下内容在 `CHANGELOG.md` 中保持一致：

- 顶部版本标题与 `CHANGELOG.md` 保持一致；当前标题为 `0.14.1 - Preview Markdown Note Shortcut and Shared Runtime Patch`
- 当前已包含实际版本差异、安装/升级说明与回退建议
- release notes 应覆盖以下当前已确认范围：`0.14.1` 补齐 Explorer Markdown 文件右键创建关联 Note、创建类入口复用已打开 Canvas surface、分组 body 空白区拖动画板，以及 multi-root / 双窗口 shared live runtime 恢复验证与硬化；保留 `0.14.0` 的空间级 fit view、MiniMap、双市场同版本同步策略，以及 `Dev Session Canvas Notifier` companion 版本对齐
...
1. 当前目标版本为 `0.14.1`，扩展身份保持 `devsessioncanvas.dev-session-canvas`；`0.1.0` 仍是首个公开 `Preview` 基线版本。
2. 首次安装与从 `0.14.0` 升级到 `0.14.1` 将通过当前宿主配置的公开扩展市场常规安装 / 升级完成；官方 VS Code 仍以 `Visual Studio Marketplace` 为主路径，`Open VSX` 作为 VS Code 兼容宿主的补充渠道。后续 `0.14.x` 更新应保持两个公开市场同版本发布。
...
4. 若用户此前显式配置过 `devSessionCanvas.notifications.attentionSignalBridge`、`devSessionCanvas.notifications.strongTerminalAttentionReminder` 或 `devSessionCanvas.notifications.agentAbnormalOutputTextNotifications`，升级到 `0.14.1` 后会继续沿用该明确选择；默认安装路径仍优先使用 `system` 桥接并在必要时回退到工作台消息。
...
若 `0.14.1` 对当前工作流形成 blocker，当前统一建议是：
...
当前 `0.14.1` 发布不以额外截图为 blocker。当前已经具备：
...
1. Explorer Markdown 文件右键创建关联 Note 后，Canvas 中出现已关联 Markdown 文件的 Note
2. 当前窗口已打开 panel/editor Canvas 时，创建类入口复用该承载面
3. 双窗口 shared live runtime 恢复验证中的 Agent / Terminal 同步状态
...
当前功能输入已有 repo-local 证据：`docs/product-specs/explorer-markdown-create-note.md`、`docs/design-docs/note-markdown-file-association.md` 与 `docs/design-docs/canvas-surface-placement.md` 记录了 Explorer Markdown 创建关联 Note 和创建类入口复用已打开 Canvas surface 的边界；`docs/product-specs/canvas-node-groups.md` 与 `docs/design-docs/canvas-node-groups.md` 记录了分组 body 空白区左键拖动画板的语义；`docs/product-specs/canvas-multi-root-workspace-support.md` 与 `docs/design-docs/canvas-multi-root-workspace-support.md` 记录了 multi-root / 双窗口 shared live runtime 的 runtime binding、storage slot 与 smoke 验证口径。

本轮发布准备分支已刷新并通过以下验证：

- 版本一致性检查：`package.json`、notifier manifest、`package-lock.json` 根版本、root package entry 与 notifier package entry 均为 `0.14.1`
- `git diff --check`
...
- 主扩展 VSIX：`dev-session-canvas-0.14.1.vsix`，114 files，约 3.41 MB（本地文件大小 3,576,529 bytes）
- Notifier VSIX：`extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.14.1.vsix`，10 files，约 144.04 KB（本地文件大小 147,501 bytes）
- 两个 VSIX 的 `VSCE README doc ref` 均为 `0a7dc0587be868a5c32eab860e61eef79ae79668`
- `npm run validate:clean-checkout:vsix -- --source working-tree --skip-vsix-smoke` 已在隔离 checkout 中复核主扩展打包路径，clean checkout VSIX 约 3.41 MB，README doc ref 同为 `0a7dc0587be868a5c32eab860e61eef79ae79668`
...
残余风险：发布准备 MR 合并后，仍必须在最终 `main` release commit 上重新执行 `npm run validate:clean-checkout:vsix -- --ref <final-ref>`、`npm run package:vsix`、`npm run -w extensions/vscode/dev-session-canvas-notifier package:vsix` 与必要的 packaged-payload smoke，确认 README 相对链接改写、VSIX 文件数 / 大小、`VSCE README doc ref` 和 packaged-payload smoke 均与最终发布 ref 一致。真实 publish 与 `v0.14.1` tag 仍只能在发布准备 MR 合并后的最终 `main` ref 上执行。
...
3. 直接调用 Open VSX API 复核主扩展 `0.14.1` 的 `files.icon`、`files.license`、`files.vsixmanifest` 与 `files.sha256` 都存在，避免只验证 VSIX 本体而漏掉 registry asset metadata。
