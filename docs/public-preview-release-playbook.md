...
本文用于收口当前公开 `Marketplace Preview` 版本的发布素材、发布前复核、安装/升级说明、验证记录、发布命令与回退口径；当前发布准备目标为 `0.24.4`，上一已发布版本为 `0.24.3`。本轮范围是“修复 Runtime Supervisor 重启后旧 journal 恢复阻塞新会话与错误归因失真，修复 Pane Gallery 底部缩略图标题栏，同步已审阅的中英文 multi-root Marketplace 素材，并收口已报告的 npm audit 依赖问题”。它不是对外宣传页，而是 release-day 执行与复核手册。
...
- 当前更新依据：`docs/design-docs/runtime-supervisor-recovery-readiness.md`、`docs/exec-plans/completed/runtime-supervisor-reboot-recovery.md`、`docs/design-docs/canvas-multi-root-workspace-support.md`、`docs/design-docs/marketplace-readme-media-automation.md`、`docs/marketplace-media-scenario.md`、`docs/product-specs/runtime-persistence-modes.md`、`docs/product-specs/canvas-multi-root-workspace-support.md`、`docs/UI.md`、`docs/FRONTEND.md`
...
本轮 listing 必须明确：`0.24.4` 是 `0.24.x` Preview 线内的修复更新，不是稳定版承诺；Supervisor 会先提供可连接的控制面并报告 `recovering`，旧历史在后台 hydrate，新会话不因该过程被全局阻塞；重启前已终止的 PTY 不会被复活，恢复的 non-live snapshot 只作为历史显示；socket/readiness 错误与 PTY spawn 错误必须保持区分。Pane Gallery 底部缩略图标题栏已修复，新的中英文媒体只展示已存在的 Root Groups / Pane Gallery 能力，不能误包装成产品新增模式。本轮也更新了主扩展运行时和 VSIX / 部署工具链依赖，且不使用强制解析绕过依赖约束。`0.24.3` 已登记的 journal、90,000 行短读、Fork、媒体 opener 与 resize 验证边界继续保持；模板市场仍是 `Preview` 能力，生产服务部署版本与插件 SemVer 分离。

## release notes 定稿口径

当前 `0.24.4` 的 release notes 统一以 `extensions/vscode/dev-session-canvas/CHANGELOG.md` 为准；发布前只允许做事实性修订，不应再引入与版本范围无关的新能力描述。`v0.24.3` 之后的合并说明与协作过程不包装成用户功能。

发布前应确认以下内容在 `extensions/vscode/dev-session-canvas/CHANGELOG.md` 中保持一致：

- 顶部版本标题为 `0.24.4 - Supervisor Reboot Recovery and Multi-root Preview Refresh`
- 当前已包含实际版本差异、安装/升级说明与回退建议
- release notes 应覆盖以下当前已确认范围：Supervisor 先 listen 后异步恢复、`recovering` / `ready` phase、恢复期对 persisted attach 的重试、旧 PTY 不复活、新会话可创建、transport/readiness 与 spawn 错误归因、systemd `WorkingDirectory=` 修复、Pane Gallery 底部标题栏修复、中英文素材刷新、已报告 npm audit 依赖问题的收口，以及主扩展 / notifier 版本对齐
- 已知边界必须保留 Host 级故障模型不等于真实物理重启或长时间 Remote SSH 断开、旧 PTY 不提供跨重启存活保证，以及 `0.24.3` 延续的 journal / 90,000 行短读 / Fork / 媒体 opener / resize 边界；不得用定向通过样本覆盖残余风险
...
1. 当前发布准备目标为 `0.24.4`，上一已发布版本为 `0.24.3`，扩展身份保持 `devsessioncanvas.dev-session-canvas`；`0.1.0` 仍是首个公开 `Preview` 基线版本。
2. 首次安装与从 `0.24.3` 升级到 `0.24.4` 的目标仍是通过当前宿主配置的公开扩展市场常规安装 / 升级完成。Open VSX 侧应继续同版本公开发布；官方 VS Code 的 `Visual Studio Marketplace` 仍是目标主路径，但当前 public gallery 仍不可见时允许延期补发，不阻塞本轮 `0.24.4` 以 GitHub Release assets + Open VSX verified 完成。对外宣称 VSM 安装路径前仍必须先完成 release-day visibility check，确认主扩展与 notifier 均公开可见。
...
6. 若升级后 Supervisor 正在恢复旧 journal，它会先报告 `recovering` 并在后台 hydrate 历史；新会话不等待旧历史恢复。不同 generation 不迁移 PTY 所有权，重启时已结束的 PTY 不会被伪造为 live，恢复为 non-live snapshot 的节点只作为历史显示。
...
若 `0.24.4` 对当前工作流形成 blocker，当前统一建议是：
...
当前 `0.24.4` 发布不以额外截图为 blocker。当前已经具备：
...
1. Supervisor 已监听且处于 `recovering` 时，旧节点保持 reattaching、同时新 Agent / Terminal 可用的对照
2. 同一 namespace 进入 `ready` 后旧节点重新 attach 并在 non-live snapshot 下如实收口为历史的对照
3. Pane Gallery 底部缩略图 root 标题栏可见，以及中英文 Marketplace 资产在公开路径下正常加载的对照
...
7. 确认发布 workflow 会先把同一批 VSIX 镜像到 GitHub Release assets，并以 Open VSX 主扩展 / notifier 发布验证作为本轮完成门禁；Visual Studio Marketplace 仍会尝试发布 / 验证并写入 manifest，但当前允许 deferred，不阻塞 `0.24.4` 完成。仓库 Actions 必须具备 `contents: write` 权限，且 `GITHUB_TOKEN` 可创建 / 更新 Release、上传 assets、创建正式 tag 与按完成门禁删除临时 tag。
...
截至 `2026-07-17`，上一轮 `0.24.3` 已从最终 `main` release ref `478587ab0c40b13edb9f0a731cd524b910eda354` 完成 GitHub Release assets + Open VSX 兜底发布；正式 `v0.24.3` 指向同一 ref，`publish/v0.24.3` 已删除。GitHub Release 已上传主扩展、notifier 和 manifest 三个 assets；最终 Release notes 记录 Open VSX 双扩展 `verified`，Visual Studio Marketplace 双扩展 `publish-failed` / deferred。因此不得对外宣称 VSM 已可用。

`0.24.4` 的发布输入严格限于 `v0.24.3` 之后已合入 `main` 的 #270 Pane Gallery 底部缩略图标题栏修复、#271 中英文多根目录 Marketplace 素材制作、#272 Runtime Supervisor 重启恢复与错误归因修复，以及 #274 npm audit 依赖问题修复。它符合 `docs/workflows/VERSION.md` 的同里程碑 bugfix 规则：本轮修复既有恢复、呈现与发布素材，并升级运行时与发布工具链依赖，不改变扩展身份、provider 命令契约、journal 格式、通知协议或服务 API 主版本。

本发布准备分支已完成以下 repo-local 同步：

- 版本号同步：`package.json`、主扩展 manifest、notifier manifest 与 `package-lock.json` 的 workspace package entries 均为 `0.24.4`
- release notes 同步：主扩展与 notifier changelog 新增 `0.24.4` 顶部条目，说明 recovery phase、错误归因、old-PTY 边界、Pane Gallery 标题栏、依赖审计收口和 companion 仅对齐版本
- Marketplace / README 文案同步：主扩展 Marketplace 中英文 listing 与仓库中英文 README 已更新为 `0.24.4` 发布准备口径，并保留真实重启与长断开未完成、journal / 90,000 行短读 / Fork、媒体 opener / resize 和 VSM deferred 边界
- 发布手册与设计结论同步：`docs/public-preview-release-playbook.md`、`docs/notifier-preview-release-playbook.md` 与 `docs/design-docs/public-marketplace-release-readiness.md` 更新目标版本、发布输入、安装/升级、回退与 tag 命令

本轮在 Node `25.6.0` / npm `11.8.0` 上已通过 `git diff --check`、extension manifest、publish-tag、双 marketplace、workflow、VSIX command / file-list 守卫，主扩展与 notifier typecheck、构建、Runtime Supervisor protocol、Runtime Host backend、terminal journal、UI copy / localization、notifier source，以及完整 `npm audit` 与 `npm audit --omit=dev`（均为 `0 vulnerabilities`）。`runtime-supervisor-reboot-recovery`、`systemd-user-real-reopen` 与 `systemd-fallback-real-reopen` 真实 VS Code smoke 均通过；notifier companion 与英文 / 简体中文 locale smoke 也均通过。模板市场 shared / API / Web 测试分别通过 `28`、`99` 与 `45` 项。

当前准备分支候选主扩展为 `dev-session-canvas-0.24.4.vsix`（`117` files，`3,934,207 bytes`，`sha256=c8e7ca03f6795cc45f8b7a05d721a315037de2ff52bae5e6ee5e3c75cb64b27a`），notifier 为 `dev-session-canvas-notifier-0.24.4.vsix`（`14` files，`159,484 bytes`，`sha256=623019120ee262f8bd1ac4e3d7338652752c2505502b66e2293c323ca17291ab`）。dirty-tree 打包日志的 `VSCE README doc ref` 为 `69e74368a1f6765c9c6e4bcdb96ac916c8a3366b`；`npm run validate:clean-checkout:vsix -- --source working-tree` 已通过隔离安装、完整审计、打包和 packaged-payload smoke。候选工件、README ref 与其 checksum 仅是准备分支证据，不能作为最终 Release assets；MR 合入后仍须在 clean `main` release ref 上重跑双扩展打包、clean-checkout 与 `publish/v0.24.4` dry-run。

残余风险基线：`0.24.3` 的最终 Release 将 Visual Studio Marketplace 记录为 deferred；本轮不能把该渠道写成已恢复。`0.24.4` 的 restart-recovery 只在可重复 Host 级故障模型中验证，旧 local PTY 不承诺跨重启存活，真实物理重启与长时间 Remote SSH 断开仍不是已完成的验收。unsafe / oversized journal 增长、无固定磁盘上限、local PTY 非跨 Host、90,000 行间歇性短读、Fork 视觉验收、自动 File footprint 精度、媒体 opener 和 resize 的既有边界也均未关闭。最终验证必须如实区分定向测试、真实 Host smoke、packaged-payload smoke 与未执行门禁。
...
推送 `publish/v0.24.4` 会触发 `.github/workflows/publish-marketplace-release.yml`。该 workflow checkout 临时 tag 指向的 commit，先执行：
...
随后 workflow 会创建或确认正式 `v0.24.4` tag 指向同一 release ref，创建或更新 `v0.24.4` 对应的 GitHub Release，并上传以下 Release assets：

    release-artifacts/release-manifest-0.24.4.json
    dev-session-canvas-0.24.4.vsix
    extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.24.4.vsix

Release assets 上传后，workflow 会继续复用同一批 manifest / VSIX 分别执行 Open VSX 与 Visual Studio Marketplace 发布、验证。如果同一 `publish/v0.24.4` 因任一 marketplace 失败而重跑，workflow 会先下载并校验 `v0.24.4` GitHub Release 中已有的 manifest / VSIX assets，不会重新打包或覆盖 VSIX；若既有 Release 缺少任一必需 asset，则直接失败并要求人工修复不完整状态。
...
注意：`--skip-package` 不再只检查 VSIX 文件存在；它要求已有 `release-artifacts/release-manifest-0.24.4.json`，并校验当前 VSIX sha256 与 manifest 一致，避免复用不属于本次 release ref 的旧包。

若最终版本号不是 `0.24.4`，统一替换 tag、manifest 文件名、release notes 与验证记录中的版本号。
...
`vX.Y.Z` 是正式 lightweight release tag，也是 GitHub Release 绑定的 tag。为了让 Release assets 在 marketplace 暂时不可用时仍能先提供手动安装兜底，workflow 会在打包后、marketplace 发布验证前创建或确认该 tag。单看 `vX.Y.Z` 存在不再足以判断整轮发布完成；`0.24.4` 当前完成条件是 GitHub Release assets 已上传、Open VSX 已发布并验证、Visual Studio Marketplace 已记录为 verified 或 deferred、最终 manifest 已更新，且 `publish/vX.Y.Z` 已删除。如果 Open VSX 发布或验证失败，`vX.Y.Z` 与 Release assets 可能已经存在，但 workflow 会失败并保留临时 tag。需要人工删除临时 tag 时，应先确认两个 tag 指向同一 commit、Release assets 已存在，且 Open VSX 发布验证已经完成：

    git push origin :refs/tags/publish/v0.24.4
    git tag -d publish/v0.24.4
...
1. 确认对应版本的 lightweight tag 已存在于远端，且指向本次实际发布的 commit；若 Open VSX 已验证完成，确认远端 `publish/v0.24.4` 已删除。
...
3. 打开 GitHub Release 页面，确认 `dev-session-canvas-0.24.4.vsix`、`dev-session-canvas-notifier-0.24.4.vsix` 与 `release-manifest-0.24.4.json` 都存在于 Assets 中。
...
6. 定向复核 `0.24.4` 用户可见主路径：重启恢复期间旧 persisted 节点保持恢复中、新 Agent / Terminal 可创建、namespace ready 后会重试 attach，恢复为 non-live 的旧 session 只显示为历史；socket/readiness 错误不再显示为缺少命令或 shell；Pane Gallery 底部缩略图继续显示 root 标题栏。同时确认 `0.24.3` 的媒体链接、resize、journal compact、Fork 定向落位与终态门禁主路径未回归。
7. 复核生产服务状态时使用 `/api/v1/meta`、deploy tag、Cloudflare deployment id 和 production smoke 证据；不要把插件 `v0.24.4` tag 当成服务当前运行版本。
