...
本文用于收口当前公开 `Marketplace Preview` 版本的发布素材、发布前复核、安装/升级说明、验证记录、发布命令与回退口径；当前目标版本为 `0.21.1`。当前版本范围收口为“相对已发布 `0.21.0` 的同线修复更新：多根布局整理默认限定当前 root、Pane Gallery active root 缩略图占位和 running 扫描连续化、serialized terminal supervisor snapshot 新鲜度、侧栏 root 顺序，以及当前 Agent 恢复 / 分叉启动意图继承”。它不是对外宣传页，而是发布当天的执行与复核手册。
...
- 当前修复依据：`docs/design-docs/canvas-layout-arrangement.md`、`docs/design-docs/canvas-multi-root-workspace-support.md`、`docs/design-docs/embedded-terminal-runtime-window.md`、`docs/design-docs/agent-launch-modes-and-restart.md`、`docs/design-docs/canvas-sidebar-node-and-session-lists.md`
...
本轮 listing 必须明确：`0.21.1` 是 `0.21.x` Preview 线内修复，不是稳定版承诺；多根布局整理默认只整理当前 root，整 workspace 整理必须显式选择；Pane Gallery active root 缩略图 rail 使用占位而不是第二份真实画板；当前 Agent 恢复 / 分叉会继承节点启动意图，但历史会话入口不伪造历史 argv；模板市场仍是 `Preview` 能力，生产目录可能在真实模板发布前为空；生产服务部署版本与插件 SemVer 分离，不把 `deploy/template-marketplace/prod/*` 当作插件版本。

## release notes 定稿口径

当前 `0.21.1` 的 release notes 统一以 `extensions/vscode/dev-session-canvas/CHANGELOG.md` 为准；发布前只允许做事实性修订，不应再引入与版本范围无关的新能力描述。docs-only 变化不进入用户可见更新说明，除非它改变安装、升级、回退或发布判断。

发布前应确认以下内容在 `extensions/vscode/dev-session-canvas/CHANGELOG.md` 中保持一致：

- 顶部版本标题为 `0.21.1 - Multi-root Arrangement and Resume Reliability Patch`
- 当前已包含实际版本差异、安装/升级说明与回退建议
- release notes 应覆盖以下当前已确认范围：多根布局整理默认限定当前 root、Pane Gallery active root 缩略图占位、Pane Gallery running 扫描连续化、serialized terminal supervisor snapshot trust / tracker 保留、侧栏 root 顺序、当前 Agent 恢复 / 分叉启动意图继承，以及 `Dev Session Canvas Notifier` companion 版本对齐
...
1. 当前目标版本为 `0.21.1`，扩展身份保持 `devsessioncanvas.dev-session-canvas`；`0.1.0` 仍是首个公开 `Preview` 基线版本。
2. 首次安装与从 `0.21.0` 升级到 `0.21.1` 的目标仍是通过当前宿主配置的公开扩展市场常规安装 / 升级完成。Open VSX 侧应继续同版本公开发布；官方 VS Code 的 `Visual Studio Marketplace` 仍是目标主路径，但当前 public gallery 仍不可见时允许延期补发，不阻塞本轮 `0.21.1` 以 GitHub Release assets + Open VSX verified 完成。对外宣称 VSM 安装路径前仍必须先完成 release-day visibility check，确认主扩展与 notifier 均公开可见。
...
4. 多根 workspace 中的“整理画布布局”在 root 内触发时默认只作用于目标 root；如需整理所有 root，需要在菜单中显式选择“整理整个 workspace 的画布”。
5. 当前 Agent 节点的 `重启` / `分叉` 会继承节点自身的启动意图；历史会话列表的恢复 / 分叉仍只继承历史 session id、cwd 和当前默认参数。
6. 模板市场生产入口默认为 `https://dscanvas.dev/templates`；生产环境不会把代码内 seed 模板暴露为正式内容，初始空目录属于当前受控状态，不代表扩展安装失败。
7. 若用户此前显式配置过 `devSessionCanvas.notifications.attentionSignalBridge`、`devSessionCanvas.notifications.enabledAttentionSignals`、`devSessionCanvas.notifications.strongTerminalAttentionReminder`、`devSessionCanvas.notifications.agentAbnormalOutputTextNotifications`、`devSessionCanvas.canvas.linkOpenMode`、`devSessionCanvas.canvas.workspaceRootWatermarks.enabled` 或 `devSessionCanvas.canvas.multiRootPresentationMode`，升级到 `0.21.1` 后会继续沿用该明确选择；未配置 `enabledAttentionSignals` 时继续使用默认 allow-list，未配置 `multiRootPresentationMode` 时继续使用默认 `rootGroups`，默认安装路径仍优先使用 `system` 桥接并在必要时回退到工作台消息。
...
若 `0.21.1` 对当前工作流形成 blocker，当前统一建议是：
...
当前 `0.21.1` 发布不以额外截图为 blocker。当前已经具备：
...
1. Pane Gallery thumbnail rail 的 active root 占位与 root running 扫描提示
2. 多根 root 内整理画布布局的范围菜单
3. 当前 Agent `YOLO` / 沙盒节点恢复或分叉时保留启动意图的诊断或命令展示
4. 插件内模板市场列表和详情页，展示安装 / 更新 / 版本菜单

若来不及补截图，不阻塞当前公开 `Preview` 修复更新。
...
6. 复核以下文件与当前版本事实一致：
   ...
   - `docs/support.md`
   - `docs/SECURITY.md`
   - `docs/design-docs/canvas-layout-arrangement.md`
...
7. 确认发布 workflow 会先把同一批 VSIX 镜像到 GitHub Release assets，并以 Open VSX 主扩展 / notifier 发布验证作为本轮完成门禁；Visual Studio Marketplace 仍会尝试发布 / 验证并写入 manifest，但当前允许 deferred，不阻塞 `0.21.1` 完成。仓库 Actions 必须具备 `contents: write` 权限，且 `GITHUB_TOKEN` 可创建 / 更新 Release、上传 assets、创建正式 tag 与按完成门禁删除临时 tag。
...
截至 `2026-07-03`，上一轮 `0.21.0` 已从最终 `main` release ref `64df87fb08cb3940b32f0b3104b8d408d837c940` 完成 GitHub Release assets + Open VSX 兜底发布；正式 `v0.21.0` tag 指向同一 ref，远端 `publish/v0.21.0` 临时 tag 已由 workflow 删除。GitHub Release `v0.21.0` 位于 `https://github.com/ZY-WANG-0304/dev-session-canvas/releases/tag/v0.21.0`，包含 `dev-session-canvas-0.21.0.vsix`、`dev-session-canvas-notifier-0.21.0.vsix` 与 `release-manifest-0.21.0.json`。Release manifest 记录 Open VSX 主扩展与 notifier `0.21.0` 均 verified，Visual Studio Marketplace 主扩展与 notifier 均为 `publish-failed` / deferred；因此 VSM 继续作为 deferred channel，不得对外宣称为已可用。

`0.21.1` 当前发布准备输入来自 `v0.21.0` 之后合入 `origin/main` 的功能、修复与文档 / 测试工作：#236 serialized terminal snapshot 旧 supervisor raw tail 降级与可信 tracker 保留、#237 Pane Gallery root running 扫描连续化、#233 多根 root 整理默认限定当前 root、#238 Pane Gallery active root 缩略图占位、#239 侧栏 root 分组按 workspace 顺序排列，以及 #240 当前 Agent 恢复 / 分叉继承启动意图。本轮发布准备分支只处理版本号、CHANGELOG、Marketplace / README 文案、发布手册和发布验证，不把未合并的功能分支 head 直接当作发布输入。最终 release ref 以发布准备 MR 合入 `main` 后的 commit 为准。

本轮发布准备分支已完成以下 repo-local 同步：

- 版本号同步：`package.json`、主扩展 manifest、notifier manifest、`package-lock.json` 根版本、主扩展 package entry 与 notifier package entry 均更新为 `0.21.1`
- release notes 同步：`extensions/vscode/dev-session-canvas/CHANGELOG.md` 与 notifier changelog 已新增 `0.21.1` 顶部条目，并明确 notifier 不引入新的通知投递行为变更
- Marketplace / README 文案同步：`extensions/vscode/dev-session-canvas/README.marketplace.md`、`extensions/vscode/dev-session-canvas/README.marketplace.zh-CN.md`、`README.md`、`README.zh-CN.md` 已更新为 `0.21.1` 发布准备口径，并保留 VSM deferred 约束
...
截至 `2026-07-03`，`release-0-21-1-prep` 已执行并通过以下 repo-local 验证：
...
- 本轮能力相关回归：`npm run test:agent-launch-presets`、`npm run test:canvas-layout-arrangement`、`npm run test:sidebar-node-list`、`npm run test:execution-output-sequence`
...
当前 repo-local VSIX 产物记录如下：

- 主扩展：`dev-session-canvas-0.21.1.vsix`，`115` files，约 `3.6 MB`（`3,774,429` bytes），`sha256=f455252e437f88d60e5712ed1a9b473f47709b44fa56cb7eabc34a0628478adb`
- notifier：`extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.21.1.vsix`，`10` files，约 `145.93 KB`（`149,435` bytes），`sha256=8f5c8fe51e6887067c3c2bd0fdd93d4aea7aa08244ebe5cf6fc7cddf078bbf34`
- `npm audit --omit=dev` 输出 `found 0 vulnerabilities`
- 主扩展打包在发布准备分支 dirty working tree 下打印 `VSCE README doc ref: 231b73254fd6867b10e961d549f922024229f6e8`，只能作为 repo-local 分支内验证证据；最终 release checkout 必须在 clean `main` ref 上重跑，或显式设置 `DEV_SESSION_CANVAS_VSCE_DOC_BRANCH=<final-ref>`
- `npm run test:vsix-smoke` 首次尝试曾因 VS Code 主窗口 `CodeWindow: detected unresponsive` 退出，未发现扩展断言失败；同日原命令重跑通过并输出 `VSIX packaged-payload smoke passed.`，因此当前把它记录为本地宿主 flake，最终 `main` release ref 仍需重跑 packaged-payload smoke

当前发布准备分支尚未合并，因此最终发布前必须在发布准备 MR 合入后的最终 `main` ref 上重跑 clean-checkout VSIX、主扩展 / notifier 打包、packaged-payload smoke 与 `publish/v0.21.1` dry-run。`npm run release:publish-tag -- --trigger-tag publish/v0.21.1 --dry-run --package-only --skip-origin-main-check` 需要先创建指向最终 release commit 的本地或远端 `publish/v0.21.1` tag；在发布准备分支未合入、未创建该 tag 前，不把 dry-run 写作已完成。

残余风险：Visual Studio Marketplace public gallery 当前仍不可见，manifest / Release notes 必须继续把 VSM 写成 deferred / `publish-failed`；不得把官方 VS Code Marketplace 安装路径宣称为已可用。真实 GitHub OAuth template publish smoke 仍是发布前 / 升级验证状态前 follow-up；生产服务与插件发布版本线分离，不能把服务 deploy tag 写成插件版本，也不能把插件 `v0.21.1` tag 当成当前线上服务版本。
...
推送 `publish/v0.21.1` 会触发 `.github/workflows/publish-marketplace-release.yml`。该 workflow checkout 临时 tag 指向的 commit，先执行：
...
随后 workflow 会创建或确认正式 `v0.21.1` tag 指向同一 release ref，创建或更新 `v0.21.1` 对应的 GitHub Release，并上传以下 Release assets：

    release-artifacts/release-manifest-0.21.1.json
    dev-session-canvas-0.21.1.vsix
    extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.21.1.vsix

Release assets 上传后，workflow 会继续复用同一批 manifest / VSIX 分别执行 Open VSX 与 Visual Studio Marketplace 发布、验证。如果同一 `publish/v0.21.1` 因任一 marketplace 失败而重跑，workflow 会先下载并校验 `v0.21.1` GitHub Release 中已有的 manifest / VSIX assets，不会重新打包或覆盖 VSIX；若既有 Release 缺少任一必需 asset，则直接失败并要求人工修复不完整状态。
...
注意：`--skip-package` 不再只检查 VSIX 文件存在；它要求已有 `release-artifacts/release-manifest-0.21.1.json`，并校验当前 VSIX sha256 与 manifest 一致，避免复用不属于本次 release ref 的旧包。

若最终版本号不是 `0.21.1`，统一替换 tag、manifest 文件名、release notes 与验证记录中的版本号。
...
`vX.Y.Z` 是正式 lightweight release tag，也是 GitHub Release 绑定的 tag。为了让 Release assets 在 marketplace 暂时不可用时仍能先提供手动安装兜底，workflow 会在打包后、marketplace 发布验证前创建或确认该 tag。单看 `vX.Y.Z` 存在不再足以判断整轮发布完成；`0.21.1` 当前完成条件是 GitHub Release assets 已上传、Open VSX 已发布并验证、Visual Studio Marketplace 已记录为 verified 或 deferred、最终 manifest 已更新，且 `publish/vX.Y.Z` 已删除。如果 Open VSX 发布或验证失败，`vX.Y.Z` 与 Release assets 可能已经存在，但 workflow 会失败并保留临时 tag。需要人工删除临时 tag 时，应先确认两个 tag 指向同一 commit、Release assets 已存在，且 Open VSX 发布验证已经完成：

    git push origin :refs/tags/publish/v0.21.1
    git tag -d publish/v0.21.1
...
1. 确认对应版本的 lightweight tag 已存在于远端，且指向本次实际发布的 commit；若 Open VSX 已验证完成，确认远端 `publish/v0.21.1` 已删除。
...
3. 打开 GitHub Release 页面，确认 `dev-session-canvas-0.21.1.vsix`、`dev-session-canvas-notifier-0.21.1.vsix` 与 `release-manifest-0.21.1.json` 都存在于 Assets 中。
...
6. 定向复核 `0.21.1` 用户可见主路径：主扩展激活、主画布打开、多根 root-scoped arrangement、Pane Gallery active root 占位和 running 扫描提示、serialized terminal supervisor snapshot 恢复不回退到旧画面、当前 Agent 恢复 / 分叉启动意图继承、侧栏 root 顺序，以及模板市场打开 / 空目录读取。
7. 复核生产服务状态时使用 `/api/v1/meta`、deploy tag、Cloudflare deployment id 和 production smoke 证据；不要把插件 `v0.21.1` tag 当成服务当前运行版本。
