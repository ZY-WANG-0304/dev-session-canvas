...
- 主扩展 VSIX：`dev-session-canvas-0.15.2.vsix`，114 files，约 3.43 MB（本轮本地打包观测；最终字节数以合并后最终 ref 复跑为准）
- Notifier VSIX：`extensions/vscode/dev-session-canvas-notifier/dev-session-canvas-notifier-0.15.2.vsix`，10 files，约 144.31 KB（本轮本地打包观测；最终字节数以合并后最终 ref 复跑为准）
