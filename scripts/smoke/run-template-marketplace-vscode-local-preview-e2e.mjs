...
} = require('../../tests/vscode-smoke/template-marketplace-fixture.cjs');

const projectRoot = process.cwd();
const currentScriptPath = fileURLToPath(import.meta.url);
const debugRoot = path.join(projectRoot, '.debug', 'template-marketplace-vscode-local-preview-e2e');
const hostTmpRoot = path.join(projectRoot, '.debug', 'template-marketplace-vscode-local-preview-e2e-host-tmp');
...
async function main() {
  ...
  try {
    const extensionTestsEnv = {
      DEV_SESSION_CANVAS_SMOKE_SCENARIO: 'template-marketplace-local-preview',
    ...
    const runtime = await prepareRuntime({
      projectRoot,
      debugRoot,
      runtimeDirName: 'dsc-template-marketplace-vscode-local-preview-e2e-runtime',
      extensionTestsEnv,
      userSettings: {
    ...
    console.log(`Template marketplace VS Code local preview E2E passed against ${marketplaceSourceUrl}.`);
  } finally {
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      server.close(() => {
        if (!address || typeof address === 'string') {
          reject(new Error('Unable to allocate a local marketplace preview fixture port.'));
          return;
        }
