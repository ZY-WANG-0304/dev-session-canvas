...
async function build(directory) {
  const record = { kind: 'linux-native-failure-v4-build', status: 'preparing', nativeExecutions: 0,
  ...
  try {
    ...
    fs.mkdirSync(inputs);
    const names = ['build-native-failure-v4.mjs', 'unix-native-failure-patch-v4.mjs', 'unix-native-failure-support-v4.h'];
    ...
    const patch = await import(pathToFileURL(path.join(inputs, 'unix-native-failure-patch-v4.mjs')).href);
