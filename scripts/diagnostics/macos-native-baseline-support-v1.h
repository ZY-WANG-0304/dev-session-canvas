#ifndef DSC_MACOS_NATIVE_BASELINE_SUPPORT_V1_H
#define DSC_MACOS_NATIVE_BASELINE_SUPPORT_V1_H
...
struct Ledger {
  ...
  bool kqueueAcquired = false, kqueueRegistered = false, kqueueWaitReturned = false;
...
static Napi::Object MakeSnapshot(Napi::Env env) {
  ...
  out.Set("token", state.token); out.Set("scenario", "U1-0"); out.Set("platform", "darwin");
...
#define DSC_STATUS(field) out.Set(#field, state.field < 0 ? env.Null() : Napi::Number::New(env, state.field).As<Napi::Value>())
  ...
  DSC_BOOL(kqueueAcquired); DSC_BOOL(kqueueRegistered); DSC_BOOL(kqueueWaitReturned);
...
static Napi::Value Configure(const Napi::CallbackInfo& info) {
  ...
  if (token.size() != 32 || token.find_first_not_of("0123456789abcdef") != std::string::npos || scenario != "U1-0")
    throw Napi::TypeError::New(env, "Invalid macOS baseline identity");
  std::lock_guard<std::mutex> lock(ledgerMutex);
...
static void BeforeSpawn(Napi::Env env, const std::string& helper, const std::string& executable) {
  ...
  if (!ledger.configured || ledger.spawnAttempted)
    throw Napi::Error::New(env, "Configure exactly one native spawn before creating resources");
...
static void Wait(napi_threadsafe_function tsfn, pid_t pid) {
  ...
  do {
    ...
    std::lock_guard<std::mutex> lock(ledgerMutex);
    if (kq >= 0) { ledger.kqueueFd = kq; ledger.kqueueAcquired = true; }
    RecordLocked("kqueue-return", kq, error);
  } while (kq == -1 && error == EINTR);
  bool registered = false, eventConfirmed = false;
  if (kq >= 0) {
    struct kevent change = {};
    EV_SET(&change, pid, EVFILT_PROC, EV_ADD, NOTE_EXIT, 0, nullptr);
    do {
      Record("kqueue-register-enter", kq, 0, pid); errno = 0;
      result = kevent(kq, &change, 1, nullptr, 0, nullptr); error = result < 0 ? errno : 0;
      std::lock_guard<std::mutex> lock(ledgerMutex);
      registered = result == 0; ledger.kqueueRegistered = registered;
      RecordLocked("kqueue-register-return", result, error, kq);
    } while (result == -1 && error == EINTR);
    if (registered) {
      struct kevent event = {};
      do {
        Record("kqueue-wait-enter", kq); errno = 0;
        result = kevent(kq, nullptr, 0, &event, 1, nullptr); error = result < 0 ? errno : 0;
        Record("kqueue-wait-return", result, error, kq);
      } while (result == -1 && error == EINTR);
      std::lock_guard<std::mutex> lock(ledgerMutex);
      ledger.kqueueWaitReturned = true;
      if (result == 1) {
        ledger.kqueueEventIdent = event.ident; ledger.kqueueEventFilter = event.filter;
        ledger.kqueueEventFlags = event.flags; ledger.kqueueEventFflags = event.fflags;
        RecordLocked("kqueue-exit-event", event.ident, event.flags, event.fflags);
        eventConfirmed = event.ident == static_cast<uintptr_t>(pid) && event.filter == EVFILT_PROC &&
          (event.fflags & NOTE_EXIT) && !(event.flags & EV_ERROR);
        ledger.kqueueExitEventValid = eventConfirmed;
      }
    }
  }
  ...
  if (eventConfirmed) {
    pid_t waited;
    do {
      Record("wait-enter", pid); errno = 0;
      waited = waitpid(pid, &status, 0); error = waited < 0 ? errno : 0;
      Record("wait-return", waited, error, waited == pid ? status : 0);
  ...
  if (decoded) {
    ...
    if (payload) {
      {
        std::lock_guard<std::mutex> lock(ledgerMutex);
        ledger.payloadAllocated = true; RecordLocked("payload-allocated");
