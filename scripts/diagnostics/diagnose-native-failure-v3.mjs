// One Linux control and three TSFN-created/thread-start-failed attempts.
...
import { assessCase, verifySaved } from './native-failure-verifier-v3.mjs';

const script = fileURLToPath(import.meta.url);
const roleScript = fileURLToPath(new URL('./native-failure-roles-v3.mjs', import.meta.url));
...
const fixed = [{ scenario: 'U1-0', attempt: 1 }, ...[1, 2, 3].map(attempt => ({ scenario: 'U1-2', attempt }))];
...
async function runSchedule(values) {
  ...
  assert.equal(hash(fs.readFileSync(binary)), binaryHash, 'Use the source-bound stage-22 candidate');
  ...
  const sources = [script, fileURLToPath(new URL('./native-failure-verifier-v3.mjs', import.meta.url)),
    fileURLToPath(new URL('./native-failure-v3.test.mjs', import.meta.url)), roleScript, writerScript, v1Verifier,
  ...
  save(path.join(output, 'schedule.json'), { schema: 'linux-native-failure-v3', platform: process.platform,
    ...
    scope: 'One new normal control and three TSFN-created/thread-start-failed attempts; not a rerun of v1 or v2.',
    notScheduled: ['Linux U1-3..5', 'macOS U1-0..7', 'Windows W1'] });
  const configs = [];
  for (const entry of entries) {
    const directory = path.join(output, `${entry.scenario}-${entry.attempt}`);
    fs.mkdirSync(directory);
    const configPath = path.join(directory, 'config.json');
    const config = { ...entry, binary, binaryHash, dependencyRoot, buildDirectory, buildManifestHash, cwd: process.cwd(), configPath,
