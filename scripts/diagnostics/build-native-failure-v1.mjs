...
const officialHeaderArchiveHash = 'daaf13ec5d45a38bbcfcff06d0723f72a6813b89ab27bb5af2ac8e1f6259dde0';
...
try {
  ...
  if (values['download-headers']) {
    assert(!values.output && !values.headers && !values['dependency-root'], 'Header download is a separate operation');
    downloadHeaders(freshDirectory(values['download-headers']));
  } else {
...
function downloadHeaders(directory) {
  const archive = `node-v${nodeVersion}-headers.tar.gz`;
  const base = `https://nodejs.org/dist/v${nodeVersion}`;
  const record = { kind: 'official-node-headers', nodeVersion, base, status: 'preparing', nativeExecutions: 0 };
  try {
    for (const name of ['SHASUMS256.txt', archive]) {
      const result = command('curl', ['--fail', '--show-error', '--location', '--max-time', '120',
        '--dump-header', `${name}.response.txt`, '--output', name, `${base}/${name}`], directory, 125000);
      save(directory, `${name}.download.json`, result);
      successful(result, `Download ${name}`);
    }
    const lines = fs.readFileSync(path.join(directory, 'SHASUMS256.txt'), 'utf8').trim().split(/\r?\n/);
    const expected = lines.map(line => line.trim().split(/\s+/)).filter(parts => parts[1] === archive);
    assert.equal(expected.length, 1, 'Expected exactly one official checksum');
    assert.match(expected[0][0], /^[a-f0-9]{64}$/);
    record.archiveSha256 = digest(path.join(directory, archive));
    record.checksumsSha256 = digest(path.join(directory, 'SHASUMS256.txt'));
    assert.equal(record.archiveSha256, expected[0][0], 'Official archive checksum mismatch');
    assert.equal(record.archiveSha256, officialHeaderArchiveHash, 'Frozen header archive changed');
    const listing = command('tar', ['-tzf', archive], directory);
    save(directory, 'archive-list.json', listing);
    successful(listing, 'List verified archive');
    assert(listing.stdout.trim().split('\n').every(name => name.startsWith(`node-v${nodeVersion}/`) && !name.split('/').includes('..')),
      'Unexpected archive root');
    const extraction = command('tar', ['--no-same-owner', '-xzf', archive], directory);
    save(directory, 'extract.json', extraction);
    successful(extraction, 'Extract verified archive');
    record.headers = path.join(directory, `node-v${nodeVersion}/include/node`);
    assert.equal(versionFromHeaders(record.headers), nodeVersion);
    record.headerTree = tree(record.headers);
    assert.equal(record.headerTree.sha256, officialHeaderTreeHash, 'Frozen header content changed');
    record.status = 'verified';
  } catch (error) {
    record.status = 'failed'; record.error = error.stack ?? String(error);
  }
  save(directory, 'headers.json', record);
  seal(directory);
  console.log(JSON.stringify({ status: record.status, directory, headers: record.headers, sha256: record.archiveSha256, nativeExecutions: 0 }));
  assert.equal(record.status, 'verified', record.error);
}

async function build(directory) {
  const record = { kind: 'linux-native-failure-v1-build', status: 'preparing', nativeExecutions: 0,
  ...
  try {
    ...
    fs.mkdirSync(inputs);
    const names = ['build-native-failure-v1.mjs', 'unix-native-failure-patch-v1.mjs', 'unix-native-failure-support-v1.h'];
    ...
    const patch = await import(pathToFileURL(path.join(inputs, 'unix-native-failure-patch-v1.mjs')).href);
