...
export function patchUnixSource(source) {
  ...
  replace('#include <signal.h>\n', '#include <signal.h>\n#include "unix-native-failure-support-v1.h"\n');
  ...
  replace('    default:\n      if (pty_nonblock(master) == -1) {\n        throw Napi::Error::New(napiEnv, "Could not set master fd to nonblocking.");\n      }',
    '    default:\n' +
    '      if (dsc_failure::InjectNonblockFailure()) {\n' +
    '        dsc_failure::CloseMasterOwned();\n' +
    '        dsc_failure::TerminateBeforeWaiter();\n' +
    '      } else {\n' +
    ...
    '        }\n' +
    '      }');
  replace('  obj.Set("pty", Napi::String::New(napiEnv, ptsname(master)));',
    '  obj.Set("pty", Napi::String::New(napiEnv, dsc_failure::CreationFailed() ? "" : ptsname(master)));');
