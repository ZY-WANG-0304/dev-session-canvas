...
export function assessCase(config, observation, evidence) {
  ...
  try {
    fact(['U1-0', 'U1-2'].includes(config.scenario) && /^[a-f0-9]{32}$/.test(config.token), 'configured scenario and identity');
    ...
    const normal = config.scenario === 'U1-0';
    const enterName = normal ? 'wait-enter' : 'wait-poll-enter';
    const returnName = normal ? 'wait-return' : 'wait-poll-return';
    const enters = named(enterName), waits = named(returnName), wait = waits.at(-1);
    ...
    for (const [index, returned] of waits.entries()) {
      ...
      waitSequence = waitSequence && entered?.value === n.pid && entered.ord < returned.ord &&
        (normal || entered.aux === 1);
      if (index > 0) {
        waitSequence = waitSequence && waits[index - 1].ord < entered?.ord;
        if (!normal) fact(BigInt(entered.monoNs) - BigInt(waits[index - 1].monoNs) >= 500000000n,
          'poll waits at least 500ms after pending return');
      }
      if (index < waits.length - 1) {
        waitSequence = waitSequence && returned.aux === 0 &&
          ((returned.value === -1 && returned.error === 4) || (!normal && returned.value === 0 && returned.error === 0));
      }
    }
    resource(waitSequence && n.waitConfirmed && n.waitError === 0 &&
      wait?.value === n.pid && wait.error === 0, 'exclusive wait returned own terminal child');
    ...
    if (termination) {
      ...
      if (normal) fact(equal(report.callback, decoded), 'raw wait status decode matches notification and callback');
    ...
    if (normal) {
      scenario(report.creationError === null && n.threadConstructCalled && n.threadConstructReturned &&
        !n.threadFailureInjected && !n.threadStartFailed && n.threadStartError === 0, 'normal thread construction succeeds');
      ...
      fact(report.pollWaitCalls === 0 && n.pollWaitCalls === 0 && !n.pollWaitStopped && !n.pollWaitInFlight &&
        ...
        named('wait-poll-return').length === 0, 'normal path does not poll child');
      ...
      const skipped = one('thread-construction-skipped'), failed = one('thread-start-error');
      scenario(n.threadFailureInjected && n.threadStartFailed && n.threadStartError === 11 &&
        skipped?.value === -1 && skipped?.error === 11 && failed?.value === -1 && failed?.error === 11 &&
        report.creationError?.code === 'DSC_THREAD_START_FAILED', 'synthetic EAGAIN skips thread construction after TSFN');
      resource(n.threadConstructCalled === false && n.threadConstructReturned === false &&
        n.threadStarted === false && n.threadFinished === false && n.threadJoined === false &&
        n.payloadAllocated === false && n.payloadFreed === false &&
        n.notificationStatus === null && n.notificationCallbackStatus === null && report.callback === null,
      'uncreated thread payload and notification remain absent');
      for (const name of ['thread-construction-enter', 'thread-construction-return', 'thread-started', 'thread-finished',
        'thread-joined', 'payload-allocated', 'payload-freed', 'notification-enter', 'notification-return',
        'notification-callback', 'wait-enter', 'wait-return'])
        resource(named(name).length === 0, `uncreated or alternate owner event absent: ${name}`);
      const notJoinable = one('thread-not-joinable');
      fact(notJoinable?.value === 0, 'finalizer observed no joinable thread');
      before('tsfn-create-return', 'thread-construction-skipped');
      before('thread-construction-skipped', 'thread-start-error');
      before('thread-start-error', 'master-close-enter');
      before('control-return', 'tsfn-release-enter');
      before('tsfn-finalizer-enter', 'thread-not-joinable');
      before('thread-not-joinable', 'tsfn-finalized');
      resource(n.pollWaitCalls === waits.length && report.pollWaitCalls === waits.length &&
        waits.length >= 1 && waits.length <= 60 && n.pollWaitStopped === true && n.pollWaitInFlight === false &&
        n.pollWaitDisposition === 'terminal', 'driver-owned polling collected child within fixed call budget');
      fact(named('wait-poll-limit').length === 0 && named('wait-poll-rejected').length === 0, 'poll path not exhausted or repeated');
      before('tsfn-release-return', 'wait-poll-enter');
      const pollCalls = report.events.filter(event => event.name === 'poll-wait-call');
      const pollReturns = report.events.filter(event => event.name === 'poll-wait-return');
      fact(pollCalls.length === waits.length && pollReturns.length === waits.length &&
        pollReturns.every((event, index) => event.disposition === (index === waits.length - 1 ? 'terminal' : 'pending') &&
          finiteTime(event.ms) && finiteTime(pollCalls[index]?.ms) && pollCalls[index].ms <= event.ms),
      'JS poll requests agree with native return dispositions');
    }
    ...
    if (config.scenario === 'U1-0') {
    ...
    } else if (config.scenario === 'U1-2') {
      scenario(n.creationFailed && n.nonblockCalled && n.nonblockResult === 0 && n.nonblockError === 0 &&
        (n.masterFlagsBefore & fs.constants.O_NONBLOCK) !== 0 && named('nonblock-return')[0]?.value === 0 &&
        named('nonblock-skipped').length === 0, 'partial path acquired real nonblocking master before TSFN');
      before('owner-registered', 'nonblock-enter'); before('nonblock-return', 'tsfn-create-enter');
      ...
      before('control-return', 'wait-poll-enter');
    }
  } catch (error) {
...
export async function verifySaved(directory) {
  ...
  assert.equal(schedule.schema, 'linux-native-failure-v3');
  ...
  const requiredSources = ['diagnose-native-failure-v3.mjs', 'native-failure-verifier-v3.mjs', 'native-failure-v3.test.mjs',
    'native-failure-roles-v3.mjs', 'diagnose-native-failure-v1.mjs', 'native-failure-verifier-v1.mjs',
    'build-native-failure-v2.mjs', 'unix-native-failure-patch-v2.mjs', 'unix-native-failure-support-v2.h',
    'native-thread-failure-patch-v2.test.mjs'].map(name => fileURLToPath(new URL(name, import.meta.url)));
  ...
  const buildTools = ['build-native-failure-v2.mjs', 'unix-native-failure-patch-v2.mjs', 'unix-native-failure-support-v2.h'];
  ...
  const expectedEntries = [{ scenario: 'U1-0', attempt: 1 }, ...[1, 2, 3].map(attempt => ({ scenario: 'U1-2', attempt }))];
