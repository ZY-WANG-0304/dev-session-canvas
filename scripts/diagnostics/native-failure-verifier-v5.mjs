...
export function assessCase(config, observation, evidence) {
  ...
  try {
    fact(['U1-0', 'U1-4'].includes(config.scenario) && /^[a-f0-9]{32}$/.test(config.token), 'configured scenario and identity');
    ...
    if (config.scenario === 'U1-4') {
      const skipped = one('notification-call-skipped');
      fact(n.notificationFailureInjected === true && n.notificationCallInvoked === false &&
        n.notificationStatus === 16 && skipped?.value === 16 && skipped?.error === 0 && skipped?.aux === 0,
      'notification is synthetic closing without a real queue call');
      fact(n.notificationCallbackStatus === null && report.callback === null &&
        named('notification-enter').length === 0 && named('notification-return').length === 0 &&
        named('notification-callback').length === 0, 'unsubmitted notification has no fabricated callback');
      before('payload-allocated', 'notification-call-skipped');
      before('notification-call-skipped', 'payload-freed');
      before('payload-freed', 'tsfn-release-enter');
    } else {
    ...
    if (config.scenario === 'U1-4') {
      fact(callbacks.length === 0 && report.callback === null, 'skipped notification is not replaced by a JS callback');
    } else {
...
export async function verifySaved(directory) {
  ...
  assert.equal(schedule.schema, 'linux-native-failure-v5');
  ...
  const requiredSources = ['diagnose-native-failure-v5.mjs', 'native-failure-verifier-v5.mjs', 'native-failure-v5.test.mjs',
    'native-failure-roles-v5.mjs', 'diagnose-native-failure-v1.mjs', 'native-failure-verifier-v1.mjs',
  ...
  const expectedEntries = [{ scenario: 'U1-0', attempt: 1 }, ...[1, 2, 3].map(attempt => ({ scenario: 'U1-4', attempt }))];
