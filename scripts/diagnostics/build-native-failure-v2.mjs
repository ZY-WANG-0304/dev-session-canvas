...
async function build(directory) {
  const record = { kind: 'linux-native-failure-v2-build', status: 'preparing', nativeExecutions: 0,
  ...
  try {
    ...
    fs.mkdirSync(inputs);
    const names = ['build-native-failure-v2.mjs', 'unix-native-failure-patch-v2.mjs', 'unix-native-failure-support-v2.h'];
    ...
    const patch = await import(pathToFileURL(path.join(inputs, 'unix-native-failure-patch-v2.mjs')).href);
