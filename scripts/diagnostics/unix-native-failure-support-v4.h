#ifndef DSC_UNIX_NATIVE_FAILURE_SUPPORT_V4_H
#define DSC_UNIX_NATIVE_FAILURE_SUPPORT_V4_H
...
struct Ledger {
  ...
  bool threadConstructCalled = false, threadConstructReturned = false;
  bool threadFailureInjected = false, threadStartFailed = false;
  int threadStartError = 0;
  bool pollWaitStopped = false, pollWaitInFlight = false;
  int pollWaitCalls = 0, pollWaitDisposition = 0;
  bool tsfnCreated = false, tsfnFinalized = false;
  bool notificationCallInvoked = false, notificationFailureInjected = false;
...
#define DSC_STATUS(field) out.Set(#field, state.field < 0 ? env.Null() : Napi::Number::New(env, state.field).As<Napi::Value>())
  ...
  DSC_BOOL(threadFailureInjected); DSC_BOOL(threadStartFailed); DSC_INT(threadStartError);
  DSC_BOOL(pollWaitStopped); DSC_BOOL(pollWaitInFlight); DSC_INT(pollWaitCalls);
  out.Set("threadJoinable", Napi::Boolean::New(env, waiter.joinable()));
  const char* dispositions[] = {"not-started", "pending", "terminal", "unknown"};
  out.Set("pollWaitDisposition", Napi::String::New(env, dispositions[state.pollWaitDisposition]));
  DSC_BOOL(tsfnCreated); DSC_BOOL(tsfnFinalized);
  DSC_BOOL(notificationCallInvoked); DSC_BOOL(notificationFailureInjected);
...
#undef DSC_INT
#undef DSC_STATUS
  if (state.firstAttemptRecorded) {
    Napi::Object first = Napi::Object::New(env);
    first.Set("synthetic", true); first.Set("syscallCalled", false);
    first.Set("result", -1); first.Set("error", ECHILD);
    first.Set("statusValid", false); first.Set("rawStatus", env.Null());
    first.Set("disposition", "unconfirmed");
    first.Set("nativeOrdinal", state.firstAttempt.ord);
    first.Set("monoNs", std::to_string(state.firstAttempt.monoNs));
    out.Set("firstAttempt", first);
  } else out.Set("firstAttempt", env.Null());
...
static void TerminateBeforeWaiter() {
  ...
  {
    std::lock_guard<std::mutex> lock(ledgerMutex);
    if (!ledger.masterAcquired || ledger.pid <= 0 || ledger.waitConfirmed || ledger.threadStarted ||
        ledger.threadConstructReturned || waiter.joinable() || ledger.pollWaitCalls || ledger.controlCalls) {
...
static Napi::Value PollWait(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if (info.Length() != 1 || !info[0].IsString())
    throw Napi::TypeError::New(env, "failurePollWait requires the configured token");
  const std::string token = info[0].As<Napi::String>();
  pid_t pid;
  {
    std::lock_guard<std::mutex> lock(ledgerMutex);
    if (!ledger.configured || token != ledger.token || std::strcmp(ledger.scenario, "U1-2") != 0 ||
        !ledger.threadStartFailed || ledger.threadStarted || ledger.threadConstructReturned || waiter.joinable() ||
        ledger.pid <= 0 || !ledger.masterAcquired || ledger.waitConfirmed ||
        ledger.pollWaitStopped || ledger.pollWaitInFlight || ledger.pollWaitCalls >= 60)
      throw Napi::Error::New(env, "Child polling is not owned or already settled");
    pid = ledger.pid;
    ++ledger.pollWaitCalls; ledger.pollWaitInFlight = true;
    RecordLocked("wait-poll-enter", pid, 0, WNOHANG);
  }
  int status = 0;
  errno = 0;
  ...
  {
    std::lock_guard<std::mutex> lock(ledgerMutex);
    ledger.pollWaitInFlight = false; ledger.waitError = error;
    RecordLocked("wait-poll-return", result, error, result == pid ? status : 0);
    if (result == pid && (WIFEXITED(status) || WIFSIGNALED(status))) {
      ledger.waitConfirmed = true; ledger.pollWaitStopped = true; ledger.pollWaitDisposition = 2;
      ledger.exitCode = WIFEXITED(status) ? WEXITSTATUS(status) : 0;
      ledger.signalCode = WIFSIGNALED(status) ? WTERMSIG(status) : 0;
    } else if (result == 0 || (result == -1 && error == EINTR)) {
      ledger.pollWaitDisposition = 1;
      if (ledger.pollWaitCalls == 60) {
        ledger.pollWaitStopped = true; ledger.pollWaitDisposition = 3;
        RecordLocked("wait-poll-limit", ledger.pollWaitCalls);
      }
    } else {
      ledger.pollWaitStopped = true; ledger.pollWaitDisposition = 3;
    }
...
static void Wait(napi_threadsafe_function tsfn, pid_t pid) {
  {
    ...
    if (std::strcmp(ledger.scenario, "U1-3") == 0) {
      // The substituted first attempt never invokes waitpid or decodes status.
      RecordLocked("first-wait-unconfirmed", -1, ECHILD);
      if (!ledger.overflow) {
        ledger.firstAttempt = ledger.events[ledger.eventCount - 1];
  ...
  bool notificationInjected = false;
  if (decoded) {
    ...
    if (payload) {
      { std::lock_guard<std::mutex> lock(ledgerMutex); ledger.payloadAllocated = true; RecordLocked("payload-allocated"); }
      {
        ...
        if (notificationInjected) {
          notification = napi_closing;
          ledger.notificationFailureInjected = true;
          ledger.notificationStatus = notification;
          RecordLocked("notification-call-skipped", notification);
        } else {
          ledger.notificationCallInvoked = true;
          RecordLocked("notification-enter");
        }
      }
      if (!notificationInjected) {
...
static void SetupExitCallback(Napi::Env env, Napi::Function cb, pid_t pid) {
  ...
  bool inject;
  { std::lock_guard<std::mutex> lock(ledgerMutex); inject = std::strcmp(ledger.scenario, "U1-2") == 0; }
  try {
    if (inject) {
      {
        std::lock_guard<std::mutex> lock(ledgerMutex);
        ledger.threadFailureInjected = true;
        RecordLocked("thread-construction-skipped", -1, EAGAIN);
      }
      throw std::system_error(EAGAIN, std::generic_category(), "Injected diagnostic thread start failure");
    }
    {
      std::lock_guard<std::mutex> lock(ledgerMutex);
      ledger.threadConstructCalled = true; RecordLocked("thread-construction-enter", pid);
    }
    waiter = std::thread(Wait, tsfn, pid);
    {
      std::lock_guard<std::mutex> lock(ledgerMutex);
      ledger.threadConstructReturned = true; RecordLocked("thread-construction-return", 0, 0, pid);
    }
  } catch (const std::system_error& error) {
    {
      std::lock_guard<std::mutex> lock(ledgerMutex);
    ...
    CloseMasterOwned();
    TerminateBeforeWaiter();
    Release(tsfn);
    Napi::Error failure = Napi::Error::New(env, "Could not start diagnostic waiter");
    failure.Set("code", "DSC_THREAD_START_FAILED");
    throw failure;
  }
}
