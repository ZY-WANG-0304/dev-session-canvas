// One Linux control and three held release-receipt attempts using the frozen native v4.
...
import { assessCase, verifySaved } from './native-failure-verifier-v6.mjs';

const script = fileURLToPath(import.meta.url);
const roleScript = fileURLToPath(new URL('./native-failure-roles-v6.mjs', import.meta.url));
...
const releaseBudgets = { observation: 100, unknownObservation: 1000, hold: 100 };
const fixed = [{ scenario: 'U1-0', attempt: 1 }, ...[1, 2, 3].map(attempt => ({ scenario: 'U1-5', attempt }))];
...
async function observe(config) {
  ...
  const mark = (name, detail = {}) => {
    const event = { name, ms: performance.now() - started, ...detail };
    observation.events.push(event);
    return event;
  };
  const child = spawn(process.execPath, [roleScript, '--caller', config.configPath],
    { stdio: ['ignore', 'pipe', 'pipe', 'ipc'] });
  let ready = false, unknown = null, receiptPermitted = false, holdTimer = null, finished = false;
  const send = type => {
    const value = { type, token: config.token, operationId: config.releaseOperationId };
    mark(type, { token: value.token, operationId: value.operationId });
    if (!child.connected) { mark('caller-send-error', { type, message: 'IPC disconnected' }); return; }
    try {
      child.send(value, error => { if (error) mark('caller-send-error', { type, ...errorFact(error) }); });
    } catch (error) { mark('caller-send-error', { type, ...errorFact(error) }); }
  };
  const grantReceipt = () => {
    if (finished || receiptPermitted) return;
    const remaining = unknown.ms + releaseBudgets.hold - (performance.now() - started);
    if (remaining > 0) { holdTimer = setTimeout(grantReceipt, Math.ceil(remaining)); return; }
  ...
  child.on('message', value => {
    const item = { ms: performance.now() - started, value };
    observation.messages.push(item);
    if (!['release-ready', 'release-observation'].includes(value?.type)) return;
    if (value.token !== config.token || value.operationId !== config.releaseOperationId) {
      mark('release-protocol-error', { type: value.type, message: 'Foreign release identity' });
      return;
    }
    if (value.type === 'release-ready') {
      if (ready) { mark('release-protocol-error', { message: 'Duplicate release ready' }); return; }
      ready = true;
      send('release-permit');
    } else if (config.scenario === 'U1-5' && value.first?.disposition === 'observation-unknown') {
      if (!ready || unknown) { mark('release-protocol-error', { message: 'Unexpected release unknown' }); return; }
      unknown = item;
      grantReceipt();
    }
  });
  ...
  await done;
  finished = true;
  clearTimeout(control); clearTimeout(deadline); clearTimeout(holdTimer);
...
async function runSchedule(values) {
  ...
  assert.equal(hash(fs.readFileSync(binary)), binaryHash, 'Use the frozen stage-24 candidate');
  assert.equal(binaryHash, '6e96a9dcd2a06b05cfe09d7bc98e6782838db3a326dd277ab47b0a60260f8217');
  ...
  const sources = [script, fileURLToPath(new URL('./native-failure-verifier-v6.mjs', import.meta.url)),
    fileURLToPath(new URL('./native-failure-v6.test.mjs', import.meta.url)), roleScript, writerScript, v1Verifier,
  ...
  save(path.join(output, 'schedule.json'), { schema: 'linux-native-failure-v6', platform: process.platform,
    arch: process.arch, kernel: os.release(), versions: process.versions, binary, binaryHash,
    dependencyRoot, buildDirectory, buildManifestHash, budgets, releaseBudgets, entries, sources,
    scope: 'One new normal control and three held release-receipt attempts; no native changes and no previous samples rerun.',
    notScheduled: ['macOS U1-0..7', 'Windows W1', 'Real close hang or failure', 'Environment teardown', 'Production integration'] });
  const configs = [];
  for (const entry of entries) {
    const directory = path.join(output, `${entry.scenario}-${entry.attempt}`);
    fs.mkdirSync(directory);
    const configPath = path.join(directory, 'config.json');
    const config = { ...entry, nativeScenario: 'U1-0', fixtureScenario: 'U1-0',
      releaseOperationId: `${entry.token}:master-close:1`, releaseBudgets, binary, binaryHash, dependencyRoot, buildDirectory, buildManifestHash, cwd: process.cwd(), configPath,
