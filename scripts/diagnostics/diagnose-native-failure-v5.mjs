// One Linux control and three synthetic-closing/unqueued-notification attempts.
...
import { assessCase, verifySaved } from './native-failure-verifier-v5.mjs';

const script = fileURLToPath(import.meta.url);
const roleScript = fileURLToPath(new URL('./native-failure-roles-v5.mjs', import.meta.url));
...
const fixed = [{ scenario: 'U1-0', attempt: 1 }, ...[1, 2, 3].map(attempt => ({ scenario: 'U1-4', attempt }))];
...
async function runSchedule(values) {
  ...
  assert.equal(hash(fs.readFileSync(binary)), binaryHash, 'Use the source-bound stage-24 candidate');
  ...
  const sources = [script, fileURLToPath(new URL('./native-failure-verifier-v5.mjs', import.meta.url)),
    fileURLToPath(new URL('./native-failure-v5.test.mjs', import.meta.url)), roleScript, writerScript, v1Verifier,
  ...
  save(path.join(output, 'schedule.json'), { schema: 'linux-native-failure-v5', platform: process.platform,
    ...
    scope: 'One new normal control and three synthetic-closing/unqueued-notification attempts; not a rerun of v1, v2, v3 or v4.',
    notScheduled: ['Linux U1-5', 'macOS U1-0..7', 'Windows W1'] });
  const configs = [];
