// New four-case input; all caller/driver/fixture/writer roles remain frozen v1.
...
import { assessCase, verifySaved } from './native-failure-verifier-v2.mjs';

const script = fileURLToPath(import.meta.url);
const roleScript = fileURLToPath(new URL('./diagnose-native-failure-v1.mjs', import.meta.url));
...
const fixed = [{ scenario: 'U1-0', attempt: 1 }, ...[1, 2, 3].map(attempt => ({ scenario: 'U1-1', attempt }))];
...
async function persist(file, record) {
  const started = performance.now();
  const child = spawn(process.execPath, [roleScript, '--writer', file], { stdio: ['ignore', 'ignore', 'pipe', 'ipc'] });
...
async function runSchedule(values) {
  ...
  assert.equal(hash(fs.readFileSync(binary)), binaryHash, 'Use the unchanged stage-20 raw-status candidate');
  assert.equal(hash(fs.readFileSync(roleScript)), '94e77d2fe45854ba15affb23783d9544313c29da765a1c8b2b7e7869c6e61708');
  ...
  const sources = [script, fileURLToPath(new URL('./native-failure-verifier-v2.mjs', import.meta.url)),
    fileURLToPath(new URL('./native-failure-v2.test.mjs', import.meta.url)), roleScript, v1Verifier,
  ...
  save(path.join(output, 'schedule.json'), { schema: 'linux-native-failure-v2', platform: process.platform,
    arch: process.arch, kernel: os.release(), versions: process.versions, binary, binaryHash,
    dependencyRoot, budgets, entries, sources,
    scope: 'One new normal control and three partial-create attempts; not a completion or reclassification of v1.',
    notScheduled: ['Linux U1-2..5', 'macOS U1-0..7', 'Windows W1'] });
  const configs = [];
  for (const entry of entries) {
    const directory = path.join(output, `${entry.scenario}-${entry.attempt}`);
    fs.mkdirSync(directory);
    const configPath = path.join(directory, 'config.json');
    const config = { ...entry, binary, binaryHash, dependencyRoot, cwd: process.cwd(), configPath,
...
if (process.argv[1] && path.resolve(process.argv[1]) === script) {
  const { values } = parseArgs({ options: { output: { type: 'string' }, binary: { type: 'string' },
    'dependency-root': { type: 'string' }, 'verify-saved': { type: 'string' } } });
  try {
