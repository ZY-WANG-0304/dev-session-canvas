...
const cases = ['natural-zero', 'natural-nonzero', 'paused-tail', 'split-state',
  'consumer-held', 'cancel-read-pending', 'cancel-callback-held'];
const scope = 'Isolated native reader and consumer diagnostics; no production cancellation policy or long-lived native handle no-growth proof.';
...
function payload(scenario) {
  if (isCancellation(scenario)) return 'C'.repeat(settings.cancelPayloadBytes);
...
async function runFixture(config) {
  ...
  const write = part => {
    let offset = 0;
    while (offset < part.length) offset += fs.writeSync(1, part, offset, part.length - offset);
    written += offset;
  };
...
async function runSample(config) {
  ...
  const readNext = () => {
    ...
    const untilPause = scenario === 'paused-tail' && !paused
      ? settings.pauseLine * Buffer.byteLength('DSC_TAIL_00001_0123456789\r\n') - deliveredBytes : settings.readBytes;
    const buffer = Buffer.alloc(cancellation ? settings.cancelReadBytes : Math.min(settings.readBytes, untilPause));
    pending = true;
    readCalls++;
  ...
  const renderedExpected = await render(cancellation ? 'C'.repeat(settings.cancelReadBytes) : expected.replace(/\n/g, '\r\n'));
  ...
  const result = { scenario, run, token: config.token, pid, source, auditSource, exit, error: errorDetail, timedOut,
    ...
    exact: cancellation ? raw.equals(Buffer.from('C'.repeat(settings.cancelReadBytes))) : normalize(raw.toString('utf8')) === expected,
...
function assess(result) {
  ...
  need(!result.error && !result.timedOut, 'driver-error-or-deadline');
  ...
  need(result.receipt?.complete && result.receipt?.written === Buffer.byteLength(payload(result.scenario)) &&
  ...
  need(result.exit?.code === (result.scenario === 'natural-nonzero' ? 7 : 0) && result.exit?.signal === 0, 'main-exit');
  need(result.closed && !result.closeError && result.closeProbe === 'EBADF', 'fd-release');
  need(result.accepted > 0 && result.accepted === result.completed &&
...
async function verifySaved(dir) {
  ...
  const sourceNames = ['diagnose-unix-exit-tail.mjs', 'unixTerminal.js', 'xterm-headless.js', 'pty.node'];
  ...
  for (let i = 0; i < schedule.length; i++) {
    ...
    assert.equal(result.exact, cancellation ? raw.equals(Buffer.from('C'.repeat(settings.cancelReadBytes))) : normalize(raw.toString('utf8')) === payload(entry.scenario));
    ...
    const expected = await render(cancellation ? 'C'.repeat(settings.cancelReadBytes) : payload(entry.scenario).replace(/\n/g, '\r\n'));
    ...
    assert.deepEqual(result.receipt, readJSON(path.join(sampleDir, 'writer-receipt.json')));
    assert.deepEqual(result.owner, readJSON(path.join(sampleDir, 'fixture-owner.json')));
    assert.equal(result.owner.configPath, path.join(config.dir, 'config.json'));
  ...
  console.log(JSON.stringify({ verified: results.length, failures, note: 'Offline evidence verification, not new native execution.' }));
  if (failures.length) process.exitCode = 1;
}

async function selfTest() {
  assert.equal(schedule.length, 21);
  assert.equal(new Set(schedule.map(sampleName)).size, 21);
