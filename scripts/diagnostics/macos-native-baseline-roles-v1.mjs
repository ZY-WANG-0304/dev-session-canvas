// Darwin U1-0 only; the fixture, writer and process budgets remain frozen v1.
...
async function driver(config) {
  ...
  let terminal, native, child, readySocket = null, readyMessage = null;
  report.loaded.platform = process.platform;
  report.loaded.executablePath = process.execPath;
  report.loaded.helper = config.helper;
  report.loaded.helperHash = hash(fs.readFileSync(config.helper));
    ...
    socket.on('data', data => {
      ...
      for (let end; (end = pending.indexOf('\n')) >= 0;) {
        ...
        if (message.type === 'ready') {
          assert.equal(readyMessage, null);
  ...
  try {
    ...
    for (;;) {
      ...
      if (readyMessage && snapshot.kqueueRegistered) {
        report.writeGate = { native: snapshot, ready: readyMessage };
        mark('write-gate-ready', { gate: report.writeGate });
        assert.equal(snapshot.pid, readyMessage.pid);
        assert.equal(snapshot.masterCloseCalls, 0);
        report.permissionSent = true;
        mark('write-permission');
        readySocket.write(JSON.stringify({ type: 'go', token: config.token }) + '\n');
        break;
      }
      if (performance.now() - started > 29000) throw new Error('Fixture or kqueue registration not established');
      await sleep(2);
    }
    {
      terminal = makeTerminal(config.dependencyRoot);
      for (;;) {
        if (performance.now() - started > 29000) throw new Error('Terminal source remains unconfirmed');
        ...
        if (result.error || result.count === 0) {
          report.source = result.error ? 'read-error' : 'darwin-read-zero';
    ...
    for (;;) {
      ...
      if (report.native.tsfnFinalized && report.native.threadJoined && report.native.waitConfirmed &&
          report.native.kqueueCloseReturned) break;
