...
- First-time installs and upgrades from `0.15.2` to `0.16.0` should use the public extension registry configured by the current host. Open VSX should publish the same version for compatible hosts and is the current marketplace completion gate; the official VS Code `Visual Studio Marketplace` path is announced only after the release-day visibility check confirms both the main extension and notifier are public. If VSM is still not visible, GitHub Release assets remain the manual-install fallback for this release
