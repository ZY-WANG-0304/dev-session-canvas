...
本次 `0.1.2` 主要聚焦交互和恢复链路的修复收口：

- Canvas 内的 `Terminal` / `Agent` 节点补齐更接近原生终端的路径拖拽插入与链接打开体验
- 节点标题编辑修复了 Enter 提交旧值回灌与中文输入法重复显示问题
- `defaultSurface` 与 `runtimePersistence.enabled` 的 reload / restart 恢复语义已收口，切换 `panel` / `editor` 后不再恢复旧承载面
- runtime restore reattach 期间的 viewport 保持更稳定，恢复主路径体验更一致

## 安装与升级

- 扩展 ID 为 `devsessioncanvas.dev-session-canvas`
- 首次安装与从 `0.1.1` 升级到 `0.1.2` 都通过 `Visual Studio Marketplace` 获取；后续 `0.1.x` 更新同样通过 Marketplace 升级获取
