...
当前公开的 `0.4.0` 版本以侧栏节点列表与工作区会话历史入口收口为重点：
...
- 收口历史恢复链路：从历史恢复时会沿用当前 provider 命令与默认启动参数，再显式附加 `resume` 参数；侧栏不可见时仍可通过命令面板里的列表入口回退到同能力
- 对会话归属采用保守过滤：只有能明确确认属于当前 workspace 的历史记录才会显示；并继续显式保留 Windows 下 `Codex` 无法向上翻页的已知限制
