...
- Lossless foreground-priority / background-fair output scheduling plus Supervisor-backed checkpoint and journal recovery for persistent execution sessions, with conservative safe compaction and current/previous fallback generations; dead PTYs restore only as bounded historical displays, eligible Agent resume stays explicit, and healthy live streams do not periodically compete with typed input for complete journal projections
...
The project has completed its first round of research, design, and MVP validation, and is now in the public `Preview` phase. The current release-prep target is `0.24.5`: recovery of a PTY known to have ended reads bounded journal metadata and preserves the last persisted terminal display as history; an eligible Agent becomes `resume-ready` and starts a provider resume process only after an explicit `Resume`. A healthy live stream no longer periodically competes with typed input for complete journal projections, and each node keeps input in order with a single write in flight. The last published baseline remains `0.24.4` until this release-prep branch is reviewed, merged, and published. The external version remains explicitly `Preview`, with no stable-release commitment.
...
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The `0.24.5` release-prep branch is responsible for version/package consistency, build, audit, packaging, dead-PTY recovery / terminal-input regressions, packaged-payload smoke, and publish dry-run evidence; final release-day validation still requires the clean merged `main` ref. Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
...
- Restart recovery does not revive PTYs that ended during a host or runtime restart. It restores only a bounded historical display; eligible Agent resume requires an explicit user action. This has repeatable Host-level smoke coverage, while physical device restarts and long Remote SSH disconnects remain outside this release's completed validation claim.
- Healthy live streams no longer periodically request complete journal projections, and each node serializes typed input with one write in flight. New projection attach and detected stream gaps can still retrieve authoritative journal data; full-history browsing remains incomplete.
