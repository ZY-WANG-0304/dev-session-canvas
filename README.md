...
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The `0.18.1` release-prep validation focuses on version/package consistency, pane-gallery coverage, layout-arrangement coverage, Codex abnormal-output footer coverage, release workflow dry-run checks, packaging, and current channel visibility; Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
