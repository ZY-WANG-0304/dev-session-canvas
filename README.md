...
项目已完成首轮研究、设计与 MVP 验证，处于公开 `Preview` 阶段。当前工作重点是在继续推进一轮 UI 修复与交互优化的同时，把 notifier companion 的正式安装路径与桌面通知桥接能力收口到主线实现，并继续按 Marketplace `Preview` 口径迭代。对外版本口径维持 `Preview`，不提供稳定正式版承诺。
...
- notifier 的架构与能力边界见 [`docs/design-docs/notifier-companion-architecture.md`](docs/design-docs/notifier-companion-architecture.md)
