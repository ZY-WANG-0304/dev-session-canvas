...
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The `0.18.2` release validation covered version/package consistency, pane-gallery root ordering, group resize draft cleanup, Webview regression stability, Marketplace README QR-removal checks, release workflow dry-run checks, packaging, GitHub Release assets, Open VSX verification, and current channel visibility; Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
