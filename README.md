...
The project has completed its first round of research, design, and MVP validation, and is now in the public `Preview` phase. The current `0.10.3` release-prep focus is to reissue the `0.10.2` Preview behavior with corrected Open VSX icon asset metadata for compatible hosts, while keeping execution-terminal link behavior, installation topology, support boundaries, and Marketplace `Preview` positioning unchanged. The external version remains explicitly `Preview`, with no stable-release commitment.
...
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The `0.10.3` repo-local validation focuses on version/package consistency, VSIX icon payload checks, Open VSX asset metadata checks, and the existing `0.10.2` execution-terminal regression evidence; Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
