...
The project has completed its first round of research, design, and MVP validation, and is now in the public `Preview` phase. The latest published release is `0.24.2`: persistent Supervisor journals gain conservative safe compaction and fallback recovery, current-node Agent forks gain directed placement, generated nodes share creation-time collision avoidance, and final-state protocol gating no longer depends on a fixed wait. The external version remains explicitly `Preview`, with no stable-release commitment.
...
- The intended primary public distribution channel remains `Visual Studio Marketplace`, with `Open VSX` as a same-version supplemental channel. `0.24.2` completed through GitHub Release assets plus verified Open VSX publication while Visual Studio Marketplace remains recorded as deferred.
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The final `0.24.2` `main` ref passed version/package consistency, build, audit, packaging, journal/Fork/final-state regressions, packaged-payload smoke, and publish validation. Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
...
Public distribution is intended to happen through public extension registries. Official VS Code is still intended to use the `Visual Studio Marketplace` as the primary path, while `Open VSX` is the supplemental path for compatible hosts. For `0.24.2`, GitHub Release assets provide the artifact mirror and manual-install fallback, and both Open VSX extensions are published and verified; Visual Studio Marketplace publication was attempted but remains a deferred channel because public visibility is unavailable. `.vsix` files are otherwise kept as build artifacts and release-verification inputs rather than ordinary distribution files.
...
- `Visual Studio Marketplace` remains the intended official VS Code installation path, but it must not be announced as available until both the main extension and notifier are publicly visible there; `0.24.2` completed through GitHub Release assets plus verified Open VSX with VSM deferred
...
- The public `Preview` distribution path is intended to consolidate around `Visual Studio Marketplace` with same-version `Open VSX` mirroring. `0.24.2` is available through its GitHub Release assets and verified main/notifier Open VSX entries, while Visual Studio Marketplace publication remains deferred; future release-day publication still requires manual execution and review.
