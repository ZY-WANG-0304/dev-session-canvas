...
The product has entered the public `Preview` phase. The latest published baseline is the `0.16.1` Preview patch through GitHub Releases and verified Open VSX; Visual Studio Marketplace remains deferred until the public gallery exposes both the main extension and notifier. Current work is focused on preparing the `0.17.0` Preview milestone and follow-up `0.17.x` regression verification. It is aimed at advanced users who accept early limitations and can prepare their local CLI runtime environment themselves.
...
- Sidebar workspace-folder actions for adding folders, creating git worktrees, distinguishing repository / linked-worktree roots, and removing workspace folders or linked worktrees through explicit confirmations
...
The project has completed its first round of research, design, and MVP validation, and is now in the public `Preview` phase. The current `0.17.0` release-prep scope is a Preview milestone for sidebar workspace-folder and git-worktree management, repository / linked-worktree root distinction, and input-prioritized Host output scheduling for multi-Agent terminal output. It preserves the `0.16.1` root-watermark readability, clipboard-diagnostic restore cleanup, production dependency audit cleanup, Codex / Claude Code Agent Fork from current nodes and history, sidebar pending-attention triage, GitHub Release assets automation, Marketplace metadata, installation topology, support boundaries, and Marketplace `Preview` positioning. The external version remains explicitly `Preview`, with no stable-release commitment.
...
- The intended primary public distribution channel remains `Visual Studio Marketplace`, with `Open VSX` as a same-version supplemental channel; for the `0.17.0` release-day gate, GitHub Release assets plus verified Open VSX publication remain the allowed completion path while Visual Studio Marketplace visibility is still recorded as deferred.
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The `0.17.0` release-prep validation focuses on version/package consistency, sidebar worktree command coverage, execution output scheduler coverage, release workflow dry-run checks, packaging, and current channel visibility; Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
...
- The public `Preview` distribution path is intended to consolidate around `Visual Studio Marketplace` with same-version `Open VSX` mirroring. `0.16.1` completed through GitHub Release assets plus verified Open VSX while Visual Studio Marketplace visibility remains deferred; `0.17.0` keeps the same release-day completion gate unless VSM visibility recovers; future release-day publication still requires manual execution and review.
