...
The product has entered the public `Preview` phase and already completed its first external release. Current work is focused on preparing the `0.12.0` Preview milestone release and tightening follow-up `0.12.x` capabilities, release materials, and regression verification. It is aimed at advanced users who accept early limitations and can prepare their local CLI runtime environment themselves.
...
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The `0.12.0` repo-local validation focuses on version/package consistency, Explorer cwd-scoped execution-node creation, execution context propagation, workspace-relative path display, Panel Webview lifecycle identity, extension manifest checks, VSIX payload checks, and publish dry-run consistency; Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
...
- `Visual Studio Marketplace` is already the primary public installation path; later `0.12.x` updates still need the final git ref to be locked, the same version published to both `Visual Studio Marketplace` and `Open VSX`, and post-release verification completed
