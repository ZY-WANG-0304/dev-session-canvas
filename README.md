...
The product has entered the public `Preview` phase and already completed its first external release. Current work is focused on preparing the `0.13.0` Preview milestone release and tightening follow-up `0.13.x` capabilities, release materials, and regression verification. It is aimed at advanced users who accept early limitations and can prepare their local CLI runtime environment themselves.
...
The project has completed its first round of research, design, and MVP validation, and is now in the public `Preview` phase. The current `0.13.0` release-prep focus is to ship VS Code multi-root workspace composition: each workspace folder appears as a system root section in one shared canvas, while root-local canvas state remains visible when the folder is opened by itself. It preserves the `0.12.0` File Explorer cwd-scoped creation milestone, the `0.11.0` canvas groups milestone, Marketplace metadata, installation topology, support boundaries, and Marketplace `Preview` positioning. The external version remains explicitly `Preview`, with no stable-release commitment.
...
- The main path already has public `Preview` validation evidence across Linux, macOS, Windows local workspaces, and `Remote SSH`. The `0.13.0` repo-local validation focuses on version/package consistency, multi-root composition, workspace-root section behavior, cross-root edge rejection, file-activity namespace isolation, runtime reattach preservation, extension manifest checks, VSIX payload checks, and publish dry-run consistency; Windows still keeps one explicit known limitation: when using `Codex`, embedded session history cannot page upward yet.
...
- `Visual Studio Marketplace` is already the primary public installation path; later `0.13.x` updates still need the final git ref to be locked, the same version published to both `Visual Studio Marketplace` and `Open VSX`, and post-release verification completed
