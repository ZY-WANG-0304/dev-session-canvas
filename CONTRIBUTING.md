...
5. 若本机默认 profile 中已安装该扩展，确认 Development Host 里实际生效的是 `.debug/vscode-extension-main-only/` 下生成的 debug-only 主扩展，而非已安装副本。

如果本轮改动涉及 notifier companion，还应额外做一次“远端主扩展 + 本机 notifier”人工验收：

1. 在 `Remote - SSH` 打开的仓库窗口中按 `F5` 运行 `Run Dev Session Canvas + Notifier (Remote Window)`。
2. 输入：
