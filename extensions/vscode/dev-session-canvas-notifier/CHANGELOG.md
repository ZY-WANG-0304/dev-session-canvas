...
- Aligns with the `0.24.4` main-extension patch: Supervisor restart recovery now remains available while historical journals hydrate, error attribution is more accurate, Pane Gallery bottom-thumbnail titles are visible again, and the public multi-root Preview media is refreshed. The notifier companion itself adds no notification-delivery behavior, protocol, backend-selection, click-return, or localization change.
...
- 与主扩展 `0.8.0` 对齐：本轮 notifier companion 不引入新的通知行为变更，继续作为主扩展 `system` attention bridge 的本机 UI 侧投递组件。
- 继续沿用 companion 自动安装关系与版本对齐策略，保持 notifier 与主扩展的发布输入一致。

## 0.7.1

- 与主扩展 `0.7.1` 对齐：本轮 notifier companion 不引入新的通知行为变更，继续作为主扩展 `system` attention bridge 的本机 UI 侧投递组件。
- 继续沿用 companion 自动安装关系与版本对齐策略，保持 notifier 与主扩展的发布输入一致。

## 0.7.0

- 与主扩展 `0.7.0` 对齐：本轮 notifier companion 不引入新的通知行为变更，继续作为主扩展 `system` attention bridge 的本机 UI 侧投递组件。
- 最低 VS Code 版本要求与主扩展一起调整为 `1.80.0`。
- 继续沿用 companion 自动安装关系与版本对齐策略，保持 notifier 与主扩展的发布输入一致。
