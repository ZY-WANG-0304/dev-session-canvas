...
- 与主扩展 `0.16.1` 对齐：本轮主扩展是 `0.16.x` Preview 线内的可读性与诊断噪音补丁，聚焦 multi-root workspace root section 水印可读性、snapshot restore 窗口内剪贴板诊断抑制，以及用户输入前刷新恢复诊断抑制状态；notifier companion 不引入新的通知投递行为变更。
