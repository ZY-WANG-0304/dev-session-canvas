...
The public `0.24.5` release continues the `0.24.x` Preview line by separating Runtime Supervisor restart from Window Reload and keeping terminal input responsive. A new Supervisor starts with an empty namespace instead of scanning an old registry or journal. A Window Reload that reconnects to the same healthy Supervisor restores each surface and node from its complete retained projection in chunks and displays those chunks as they arrive.

- After a Supervisor restart, an Agent with a trusted provider session id becomes `resume-ready` and starts a new PTY only after you select `Resume`. A Terminal offers existing durable history when available and `Restart`; a screen snapshot or recent-output hint is optional, not guaranteed
- Restart does not hydrate an old runtime namespace or publish a canvas-wide hydration notification. New sessions can start immediately
- Window Reload uses independent, bounded bulk projections for each surface and node. The selected node receives priority, background nodes still make progress, and one restoring node does not block ready nodes, Notes, or node creation
- Per-node terminal input remains FIFO, but the next item can dispatch after the previous request is written to its fixed socket instead of waiting for that response. Every response is still observed, diagnosed, and reported independently
- The Supervisor writes input to the PTY and sends its small response before publishing compact lifecycle. Bulk connections carry terminal stream events and do not drive shared lifecycle state
- During an upgrade, sessions genuinely still owned by an older Supervisor can continue output, input, resize, stop, and delete while the current generation serves new sessions
- Codex-style OSC 10/11 color queries no longer disqualify an otherwise safe checkpoint. Color changes, resets, and unknown events remain fail-closed; safe current/previous checkpoint generations may compact complete journal prefixes
- The extension ID, VS Code minimum version, provider commands, journal format, desktop-notifier behavior, notifier auto-install relationship, Open VSX gate, Visual Studio Marketplace deferred stance, Template Marketplace service version line, and Preview support matrix stay unchanged
...
- Reconnect Window Reload to the same live Supervisor with per-surface, per-node chunked retained-history projection, while keeping restart semantics honest when the old PTY is gone
...
- Runtime-restart handling has repeatable Host-level smoke coverage for Host startup, the explicit Agent Resume action, and Terminal history state. That smoke does not click a real Resume/Restart button or promise that a physical device restart or long Remote SSH disconnect preserves an old PTY
...
- Journal compaction is deliberately conservative: unsafe or oversized checkpoints retain the journal prefix still required by the current/previous safety proof, so this release does not promise a fixed disk cap, a complete long-term retention policy, or cross-version journal rollback compatibility
...
- If an older Runtime Supervisor still owns running sessions during upgrade, those sessions continue through their original runtime with output, input, resize, stop, and delete available while new sessions start immediately on the current generation. Older sessions do not migrate PTY ownership and may need a node resize to redraw stale terminal pixels
- Supervisor-backed persistence still depends on `runtimePersistence.enabled` and backend availability. Eligible live journals can compact complete prefixes with current/previous fallback generations; unsafe checkpoints retain the required prefix, local PTYs do not gain a cross-Host lifetime guarantee, and Preview releases do not promise rollback compatibility for runtime journals
- A replacement Supervisor starts an empty namespace and does not scan or open the prior registry/journal. A trusted Agent resume id enables explicit `Resume` into a new PTY; a Terminal offers existing durable history when available and `Restart`. No canvas-wide hydration notification is emitted, and screen/recent-output hints are optional
- A Window Reload against the same live Supervisor restores complete retained history per surface and node over a bounded bulk stream, showing chunks immediately. Input and compact lifecycle remain on control: FIFO advances after fixed-socket dispatch, each response settles independently, response precedes lifecycle, and bulk does not publish shared lifecycle
