...
function resolveRelativeExecutionPath(
  ...
  const cwdComparable = cwd.caseInsensitive || workspaceFolderPath.caseInsensitive
    ? cwd.normalized.toLowerCase()
    : cwd.comparable;
  const workspaceComparable = cwd.caseInsensitive || workspaceFolderPath.caseInsensitive
    ? workspaceFolderPath.normalized.toLowerCase()
    : workspaceFolderPath.comparable;
