...
产品已进入公开 `Preview` 阶段，并已完成首个对外版本发布；当前主要工作是准备 `0.10.1` Preview 更新，并围绕后续 `0.10.x` 迭代持续收口能力、发布材料与回归验证。面向愿意接受早期限制、并能自行准备本地 CLI 运行环境的高级用户。
...
项目已完成首轮研究、设计与 MVP 验证，处于公开 `Preview` 阶段。当前 `0.10.1` 发布准备重点是收口 Note 与 Markdown 文件工作流、Remote SSH 拖拽边界、Markdown metadata / 图片预览、关联 Note 模板保存策略、停止后 Agent 重启动作和发布材料，并继续按 Marketplace `Preview` 口径迭代。对外版本口径维持 `Preview`，不提供稳定正式版承诺。
...
- Linux、macOS、Windows 本地工作区以及 `Remote SSH` 主路径已有公开 `Preview` 验证证据；`0.10.1` 的 repo-local 验证重点收口关联 Markdown Note 模板、Remote Markdown 拖拽 host 身份、YAML metadata、安全图片预览、停止后 Agent 重启动作、UI 颜色对齐、Marketplace 英文默认文案和打包材料，但 Windows 下使用 `Codex` 时仍保留“执行节点内历史无法向上翻页”的已知限制。
