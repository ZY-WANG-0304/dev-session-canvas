...
产品已进入公开 `Preview` 阶段；最新已发布基线是通过 GitHub Releases 发布并在 Open VSX 验证通过的 `0.17.0` Preview 里程碑，Visual Studio Marketplace 仍需等 public gallery 同时暴露主扩展与 notifier 后再解除 deferred。当前主要工作是准备 `0.18.0` Preview 里程碑，并围绕后续 `0.18.x` 迭代持续回归验证。面向愿意接受早期限制、并能自行准备本地 CLI 运行环境的高级用户。
...
项目已完成首轮研究、设计与 MVP 验证，处于公开 `Preview` 阶段。当前 `0.18.0` 发布准备范围是一个 Preview 里程碑：multi-root workspace 窗格画廊呈现、一次性画布布局整理，以及 Codex 异常输出尾部状态栏兼容。它保留 `0.17.0` 的侧栏 workspace folder / git worktree 管理、Host 输出投递输入优先调度、Codex / Claude Code Agent 从当前节点和历史会话分叉、侧栏待处理提醒汇总、publish tag / GitHub Release assets 发布自动化、Marketplace 元数据、安装拓扑、支持边界和 Marketplace `Preview` 定位。对外版本口径维持 `Preview`，不提供稳定正式版承诺。
...
- 公开发布主渠道目标仍以 `Visual Studio Marketplace` 为主，`Open VSX` 作为同版本补充渠道；`0.18.0` release-day 完成门禁继续允许在 Visual Studio Marketplace 仍不可见时，依赖 GitHub Release assets 加已验证的 Open VSX 完成本轮发布，并把 VSM 记录为 deferred channel。
- Linux、macOS、Windows 本地工作区以及 `Remote SSH` 主路径已有公开 `Preview` 验证证据；`0.18.0` 发布准备验证重点覆盖版本 / 打包一致性、窗格画廊、布局整理、Codex 异常输出 footer 兼容、发布 workflow dry-run、打包和当前渠道可见性；Windows 下使用 `Codex` 时仍保留“执行节点内历史无法向上翻页”的已知限制。
...
对外分发目标是通过公开扩展市场发布；官方 VS Code 仍计划以 `Visual Studio Marketplace` 为主路径，`Open VSX` 作为 VS Code 兼容宿主的补充渠道。`0.18.0` 中，GitHub Release assets 继续作为 release-day 工件镜像和手动安装兜底入口，Open VSX 是当前必须验证通过的 marketplace 完成门禁；Visual Studio Marketplace 仍会尝试发布，但 public visibility 若仍不可用，则作为 deferred channel 记录而不阻塞本轮完成。除此之外，`.vsix` 仍仅保留为构建工件和发布验证输入。

- 公开 `Preview` 用户应通过当前宿主配置的扩展市场安装，而非手动分发 `.vsix`
- `Visual Studio Marketplace` 仍是官方 VS Code 安装主路径目标，但只有在主扩展和 notifier 均公开可见后才对外宣称可用；`0.18.0` 可在 VSM deferred 的状态下，通过 GitHub Release assets 加已验证的 Open VSX 完成本轮发布
...
- 公开 `Preview` 的分发主路径目标仍是 `Visual Studio Marketplace`，并补充 `Open VSX` 同版本发布；`0.17.0` 已在 Visual Studio Marketplace 可见性 deferred 时依赖 GitHub Release assets 加 Open VSX verified 完成；`0.18.0` 沿用同一完成门禁，后续 release-day 仍需手工执行与复核。
