...
产品已进入公开 `Preview` 阶段，并已完成首个对外版本发布；当前主要工作是准备 `0.10.3` Preview 重发补丁，并围绕后续 `0.10.x` 迭代持续收口能力、发布材料与回归验证。面向愿意接受早期限制、并能自行准备本地 CLI 运行环境的高级用户。
...
项目已完成首轮研究、设计与 MVP 验证，处于公开 `Preview` 阶段。当前 `0.10.3` 发布准备重点是在保持 `0.10.2` Preview 行为不变的前提下重发补丁版本，修复 Open VSX 兼容宿主读取主扩展图标 asset metadata 的问题，并继续按 Marketplace `Preview` 口径迭代。对外版本口径维持 `Preview`，不提供稳定正式版承诺。
...
- Linux、macOS、Windows 本地工作区以及 `Remote SSH` 主路径已有公开 `Preview` 验证证据；`0.10.3` 的 repo-local 验证重点收口版本 / 打包一致性、VSIX 图标 payload、Open VSX asset metadata 检查，并沿用 `0.10.2` 的执行终端回归证据；Windows 下使用 `Codex` 时仍保留“执行节点内历史无法向上翻页”的已知限制。
