...
产品已进入公开 `Preview` 阶段，并已完成首个对外版本发布；当前主要工作是准备 `0.13.0` Preview 里程碑发布，并围绕后续 `0.13.x` 迭代持续收口能力、发布材料与回归验证。面向愿意接受早期限制、并能自行准备本地 CLI 运行环境的高级用户。
...
项目已完成首轮研究、设计与 MVP 验证，处于公开 `Preview` 阶段。当前 `0.13.0` 发布准备重点是发布 VS Code multi-root workspace 组合画布：每个 workspace folder 在同一张画布中显示为系统 root section，而单独打开某个 folder 时仍看到该 root 自己的 root-local 画布状态。它保留 `0.12.0` 的 File Explorer 绑定 cwd 创建里程碑、`0.11.0` 的画布分组里程碑、Marketplace 元数据、安装拓扑、支持边界和 Marketplace `Preview` 定位。对外版本口径维持 `Preview`，不提供稳定正式版承诺。
...
- Linux、macOS、Windows 本地工作区以及 `Remote SSH` 主路径已有公开 `Preview` 验证证据；`0.13.0` 的 repo-local 验证重点收口版本 / 打包一致性、多根组合、workspace-root section 行为、跨 root 连线拒绝、文件活动命名空间隔离、runtime reattach 信号保留、扩展 manifest 检查、VSIX payload 检查与发布 dry-run 一致性；Windows 下使用 `Codex` 时仍保留“执行节点内历史无法向上翻页”的已知限制。
...
- `Visual Studio Marketplace` 已是当前公开安装主路径；后续 `0.13.x` 更新仍需按发布手册锁定最终 git ref、同步发布到 `Visual Studio Marketplace` 与 `Open VSX`，并完成发布后验证
